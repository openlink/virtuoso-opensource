...
function writeRichText(rte, html, width, height, buttons, readOnly) {
  ...
  if (buttons == true)
  {
    ...
    if (!isSafari)
    {
      ...
      document.writeln('    <td><div id="emotion_' + rte + '"><img class="rteImage" src="' + imagesPath + 'smiley.gif" width="25" height="24" alt="Insert Emotion" title="Insert Emotion" onClick="dlgEmotion(\'' + rte + '\', \'emotion\', \'\')"></div></td>');
...
function getWebDAVImage (rte)
{
  ...
  if (OAT.WebDav == null || user_name == null)
  {
  ...
  var options = { imagePath: '/ods/images/oat/',
                  imageExt: 'png',
                  path: '/DAV/home/'+user_name.value+'/',
                  user: user_name.value,
                  width: 600,
                  height: 400
                };
  OAT.WebDav.init(options);
  var oWebDAV = OAT.WebDav;

  var options = { mode: 'browser',
                  onConfirmClick: function(path, fname) {$('rteTmpImage').value = path + fname; addImage(rte);}
                };
  oWebDAV.open(options);
}

function getWebDAVImage2 (rte)
{
  var tpath, sid, realm;
  var path, fsid;
...
function openWebDAV (rte)
{
  ...
  var user_name = document.forms[0]['user_name'];
  var encl  = document.forms[0][enclosure];
  if (OAT.WebDav == null || user_name == null || encl == null)
  {
    alert ("Cannot initialize the WebDAV browser");
    return;
  }
  ...
  var options = { mode: 'browser',
                  onConfirmClick: function(path, fname) {$(enclosure).value = path + fname; insertMedia(rte, false);}
                };
  oWebDAV.open(options);
}

function openWebDAV2 (rte)
{
...
function insertMedia(rte, prom) {
  ...
  if ((mediaPath != null) && (mediaPath != "") && (mediaPath != "http://")) {
    ...
    currentRTE = rte;
    insertHTML (
      '<img src="/ods/rte/images/'+ img +'" title="Play Media" alt="Play Media" border="0" style="width:320px; height:240px;"' +
