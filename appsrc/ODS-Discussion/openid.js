...
function cbSuccess (url, idserver, oid_ret, sig, nam, mail) {
    ...
    if($('post_from_n'))
    $('post_from_n').value = mail;
    
    if($('post_from'))
    $('post_from').innerHTML=nam+' <'+mail+'>';

    if($('virified_openid_url'))
      $('virified_openid_url').value=url;
...
function cbNeedPermissions (url) {
    ...
    if(typeof(url)!='undefined')
    {
     setup_URL = url;
      ...
      setup_URL = document.getElementById('openid_url').value;
     
    if($('virified_openid_url'))
      $('virified_openid_url').value='';

    setNote("<b>Note:</b> You need to <a onclick='return popSetupURL();' target='user_setup_url_win' href='" + setup_URL + "'>grant permission</a> for this site to know who you are.  Once you do so, press Login again.", '#FBFFC6');
...
{
  var msg = document.getElementById('openIdStatus');
  msg.src = imagePath + img;
  if (img != '')
