...
sticker_init() {
  ...
  echo "  </name>" >> $STICKER
  echo "  <version package=\"1.01\">" >> $STICKER
  ...
  echo "<ddls>" >> $STICKER
  echo "    <sql purpose=\"pre-install\">if (lt (sys_stat ('st_dbms_ver'), '$NEED_VERSION')) { result ('ERROR', 'The blog package requires server version $NEED_VERSION or greater'); signal ('FATAL', 'The blog package requires server version $NEED_VERSION or greater'); } </sql>" >> $STICKER
