...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:v="http://www.openlinksw.com/vspx/" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:vm="http://www.openlinksw.com/vspx/macro" xmlns:ods="http://www.openlinksw.com/vspx/ods/">
    ...
    <v:form name="F1" method="POST" type="simple" xhtml_enctype="multipart/form-data">
      ...
      <div style="background-color: #fff;">
        <div style="float: left;">
            <?vsp
              http (sprintf ('<a alt="Polls Home" title="Polls Home" href="%s"><img src="image/pollsbanner_sml.jpg" border="0" alt="Polls Home" /></a>', POLLS.WA.utf2wide (POLLS.WA.domain_sioc_url (self.domain_id, self.sid, self.realm))));
            ?>
        </div>
        ...
        <v:template type="simple" enabled="--case when (self.account_role in ('public', 'guest')) then 0 else 1 end">
          <v:url url="settings.vspx" value="Preferences" xhtml_title="Preferences"/>
            ...
            <a href="about" onclick="javascript: POLLS.aboutDialog(); return false;" title="About">About</a>
      </div>
      ...
      <div id="FT">
        ...
        <div id="FT_R">
          <a href="<?V POLLS.WA.wa_home_link () ?>faq.html">FAQ</a> |
          <a href="<?V POLLS.WA.wa_home_link () ?>privacy.html">Privacy</a> |
          <a href="<?V POLLS.WA.wa_home_link () ?>rabuse.vspx">Report Abuse</a>
    ...
    <?vsp
      ...
      select WAUI_LAT, WAUI_LNG into lat, lng from DB.DBA.WA_USER_INFO where WAUI_U_ID = self.account_id;
        if (not is_empty_or_null(lat) and not is_empty_or_null (lng) and exists (select 1 from ODS..SVC_HOST, ODS..APP_PING_REG where SH_NAME = 'GeoURL' and AP_HOST_ID = SH_ID and AP_WAI_ID = self.domain_id)) {
          http (sprintf('<a href="http://geourl.org/near?p=%U" title="GeoURL link" alt="GeoURL link" class="gems"><img src="http://i.geourl.org/geourl.png" border="0"/></a>', POLLS.WA.polls_url (self.domain_id)));
        ...
        http ('<div style="border-top: 1px solid #7f94a5;"></div>');
        http (sprintf ('<a href="%s" target="_blank" title="FOAF export" alt="FOAF export" class="gems"><img src="image/foaf.png" border="0" alt="FOAF export" /> FOAF</a>', POLLS.WA.foaf_url (self.domain_id)));
        http ('<div style="border-top: 1px solid #7f94a5;"></div>');

        S := sprintf ('http://%s/dataspace/%U/polls/%U/', DB.DBA.wa_cname (), POLLS.WA.domain_owner_name (self.domain_id), POLLS.WA.utf2wide (POLLS.WA.domain_name (self.domain_id)));
  ...
  <xsl:template name="vm:splash">
    <div style="padding: 1em; font-size: 0.70em;">
      ...
      Polls version: <?V registry_get('_polls_version_') ?><br/>
      Polls build date: <?V registry_get('_polls_build_') ?><br/>
    </div>
  </xsl:template>
