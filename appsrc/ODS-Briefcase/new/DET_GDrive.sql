  ...
  in auth_uid integer) returns any
{
  ...
  declare retValue, save, items, entries, colEntry, xmlItems, xmlEntries, xmlEntry, davEntry, davResouresId any;
  declare exit handler for sqlstate '*'
  {
    connection_set ('dav_store', save);
    resignal;
  ...
  if (not isinteger (entries))
  {
    ...
    declare resourceId, ETag, title varchar;
    {
      declare exit handler for sqlstate '*'
      {
        -- dbg_obj_print ('DB.DBA.GDrive_DAV_DIR_LIST', __SQL_STATE, __SQL_MESSAGE);
        goto _exitSync;
      };
      ...
      items := DB.DBA.GDrive__davList (detcol_id, colId);
      foreach (any item in items) do
      {
        connection_set ('dav_store', 1);
        resourceId := DB.DBA.GDrive__paramGet (item[4], item[1], 'resourceId', 0);
        if (resourceId is not null)
        {
          ...
          if (xmlEntry is not null)
          {
            ...
            ETag := DB.DBA.GDrive__entryXPath (xmlEntries, sprintf ('/@gd:etag', resourceId), 1);
            davEntry := DB.DBA.GDrive__paramGet (item[4], item[1], 'Entry', 0);
            if (DAV_HIDE_ERROR (davEntry) is not null)
            {
              ...
              {
                DB.DBA.GDrive__paramSet (item[4], item[1], 'Entry', DB.DBA.GDrive__xml2string (xmlEntry), 0);
                DB.DBA.GDrive__paramSet (item[4], item[1], ':getlastmodified', DB.DBA.GDrive__entryXPath (xmlEntry, sprintf ('/updated', resourceId), 1), 0, 0);
                DB.DBA.GDrive__download (detcol_id, item[4], item[1]);
              }
              goto _continue;
            }
          }
        }
        DAV_DELETE_INT (item[0], 1, null, null, 0, 0);
      ...
      foreach (any xmlItem in xmlItems) do
      {
        ...
        if (not position (resourceId, davResouresId))
        {
          ...
          if (DAV_HIDE_ERROR (_id) is not null)
          {
            ...
            DB.DBA.GDrive__download (detcol_id, _id, _what);
          }
        }
      }
    }
  ...
  return DB.DBA.GDrive__davList (detcol_id, colId);
  ...
  in colId integer)
{
  ...
  params := vector (
    'authentication',   DB.DBA.GDrive__paramGet (colId, 'C', 'Authentication', 0),
...
create function DB.DBA.GDrive__exec (
  ...
  in content varchar := '')
{
  -- dbg_obj_princ ('DB.DBA.GDrive__exec', colId, method, url, header, ')');
  ...
  declare tmp, _json, _reqHeader, _resHeader, _body, _xml any;
  declare exit handler for sqlstate '*'
  {
    dbg_obj_princ ('GDrive__exec: ', __SQL_STATE, __SQL_MESSAGE);
    return -28;
  };

  params := DB.DBA.GDrive__params (colId);
  ...
  if (dateadd ('second', _expires_in, _access_timestamp) < now ())
  {
    ...
    if (not DB.DBA.GDrive__exec_error (_resHeader, 0))
      return -28;

    tmp := subseq (ODS..json2obj(_json), 2);
    DB.DBA.GDrive__paramSet (colId, 'C', 'access_timestamp', datestring (now ()), 0);
    DB.DBA.GDrive__paramSet (colId, 'C', 'access_token', get_keyword ('access_token', tmp), 0);
    DB.DBA.GDrive__paramSet (colId, 'C', 'token_type', get_keyword ('token_type', tmp), 0);
    DB.DBA.GDrive__paramSet (colId, 'C', 'expires_in', cast (get_keyword ('expires_in', tmp) as varchar), 0);
    params := DB.DBA.GDrive__params (colId);
  }
  ...
  if (not DB.DBA.GDrive__exec_error (retHeader, 0))
    return -28;
  ...
  inout subPath_parts varchar)
{
  ...
  declare exit handler for sqlstate '*'
  {
    dbg_obj_print ('DB.DBA.GDrive__list', __SQL_STATE, __SQL_MESSAGE);
    return -28;
  };
...
-------------------------------------------------------------------------------
--
create function DB.DBA.GDrive__download (
  in detcol_id any,
  in id any,
  in what varchar)
{
...
-------------------------------------------------------------------------------
--
create function DB.DBA.GDrive__download_internal (
  in detcol_id any,
  in id any,
  in what varchar)
{
  ...
  if (DAV_HIDE_ERROR (davEntry) is not null)
  {
    ...
    if (not isnull (url))
    {
      ...
      if (DAV_HIDE_ERROR (retValue) is not null)
        update WS.WS.SYS_DAV_RES set RES_CONTENT = retValue where RES_ID = DB.DBA.GDrive__davId (id);
    }
  }
}
;
