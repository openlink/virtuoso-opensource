...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="vm:dav_browser">
    ...
    <v:browse-button value="Browse..."
                         selector="popup_browser.vspx"
                         child-window-options="scrollbars=auto, resizable=yes, menubar=no, height=600, width=800" browser-options="ses_type={@ses_type}&amp;list_type={@list_type}&amp;flt={@flt}&amp;flt_pat={@flt_pat}&amp;path={@path}&amp;browse_type={@browse_type}&amp;style_css={@style_css}&amp;w_title={@w_title}&amp;title={@title}&amp;advisory={@advisory}&amp;lang={@lang}&amp;retname={@return_box}&amp;view={@view}">
  ...
  function chkbx(bx1, bx2)
    {
      if (bx1.checked == true &amp;&amp; bx2.checked == true)
        ...
        <v:template name="template_auth_browser" type="simple" enabled="-- case when (self.sid is not null) then 1 else 0 end">
          ...
          <v:variable name="initPath" persist="0" type="varchar" default="null" />
  ...
  if (isnull(self.initPath))
    self.initPath := self.curpath;
  if (self.curpath not like concat(self.initPath, '%'))
    self.curpath := self.initPath;
...
if (length(rowset) > 2 and not isnull(rowset[2]))
  ...
  if (rowset[0] <> 0)
    {
      imgname := 'image/dav_browser/foldr_16.png';
      http(sprintf('<td><input type="checkbox" name="CBC_%s"/></td>', rowset[1]));
    }
  else
    {
      imgname := 'image/dav_browser/file_gen_16.png';
      http(sprintf('<td><input type="checkbox" name="CBR_%s"/></td>', rowset[1]));
                          ...
                          <xsl:choose>
                            <xsl:when test="@view='popup'">
                              <v:button name="item_view2_button"
                                        style="image"
                                        action="simple"
                                        value="image/dav_browser/open_16.png"
                                        xhtml_title="View"
                                        xhtml_alt="View">
                                <v:on-post>
                                    <![CDATA[
            ...
            or (length ((control as vspx_row_template).te_rowset) > 5 and (control as vspx_row_template).te_rowset[5] like 'text/%'))
                          ...
                          <v:button name="b_item_edit2"
                                    style="image"
                                    action="simple"
                                    value="--'image/dav_browser/edit_16.png'"
                                    xhtml_alt="Edit"
                                    xhtml_title="Edit">
                            <v:on-post>
                                <![CDATA[
  ...
  while (i < len)
    ...
    select COL_OWNER, COL_GROUP, COL_MOD_TIME, COL_PERMS
      ...
      nf3:;
                ...
                ?>
                <tr>
                  <td><img src="image/dav_browser/foldr_16.png"/></td>
                  <td><?V full_path ?></td>
                  <td>N/A</td>
                  <td><?V modtime ?></td>
                  <td>folder</td>
        ...
        nf4:
                ...
                ?>
                <tr>
                  <td><img src="image/dav_browser/file_gen_16.png"/></td>
                  <td><?V full_path ?></td>
                  <td><?V ressize ?></td>
                  <td><?V modtime ?></td>
                  <td><?V restype ?></td>
  ...
  <script type="text/javascript"><![CDATA[
    ...
    document.forms["form1"].t_path.onkeydown = handleEnter;
                ...
                <v:button style="image" name="b_go_path" value="--'image/dav_browser/go_16.png'" xhtml_alt="Go" xhtml_title="Go" action="simple">
                  <v:on-post>
                        <v:script>
                          <![CDATA[
          ...
          if (length(path) > 0 and path[length(path) - 1] <> ascii('/'))
                ...
                <v:button name="b_up" style="image" value="--'image/dav_browser/up_16.png'" xhtml_alt="Up" xhtml_title="Up" action="simple">
                  <v:before-render>
                        <v:script>
                          <![CDATA[
                ...
                <v:button name="b_create" style="image" value="image/dav_browser/foldr_new_16.png" xhtml_alt="New folder" xhtml_title="New folder" action="simple">
                  <v:on-post>
                    <v:script>
                      <![CDATA[
        ...
        <th>Perms</th>
                    ...
                    <tr class="vdir_listrow">
                      ...
                      <td>
                        <v:button name="b_up2"
                                  style="image"
                                  value="image/dav_browser/up_16.png"
                      ...
                      <table>
                        ...
                        <td nowrap="nowrap">
                          ...
                          <v:button name="b_item" style="url" action="simple" value="--(control.vc_parent as vspx_row_template).te_rowset[1]" format="%s">
                 			      <v:before-render>
                 			        <![CDATA[
                      				  if ((control.vc_parent as vspx_row_template).te_rowset[0] = 0 and (self.dir_select = 0 or self.dir_select = 2) and self.browse_type = 2)
                      			    {
                      				    declare file any;
  ...
  self.vc_data_bind(e);
            ...
            or (length ((control as vspx_row_template).te_rowset) > 5 and (control as vspx_row_template).te_rowset[5] like 'text/%'))
                          ...
                          <v:button name="b_item_prop_edit"
                                    style="image"
                                    action="simple"
                                    value="--'image/dav_browser/confg_16.png'"
           ...
           WS.WS.SYS_DAV_RES where RES_NAME = _resname and RES_COL = DAV_SEARCH_ID(_source_dir, 'c');
                ...
                <xsl:choose>
                  ...
                  <xsl:otherwise>
                    <tr>
                      <td>
                        <label for="t_filter"><img src="image/icons/filter_16.png" alt="Filter" title="Filter"/></label>
                      </td>
                      <td>
