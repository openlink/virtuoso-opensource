...
public class VirtuosoRepositoryConnection implements RepositoryConnection {
	 ...
	 *             if the transaction state can not be determined. This can
	 *             happen for instance when communication with a repository
	 *             fails or times out.
	 * @throws RepositoryException
	 */
	...
	public void add(Reader reader, String baseURI, RDFFormat format, final Resource... contexts) throws IOException, RDFParseException, RepositoryException {
		verifyIsOpen();
		sendDelayAdd();
		final boolean useStatementContext = (contexts != null && contexts.length == 0); // If
																						// no
																						// context
																						// are
																						// specified,
		...
		try {
			...
			parser.parse(reader, baseURI); // parse out each tripled to be
	...
	public void add(Statement statement, Resource... contexts) throws RepositoryException {
		if (contexts != null && contexts.length == 0 && statement.getContext() != null) {
			contexts = new Resource[] { statement.getContext() }; // try the
																	// context
																	// given by
																	// the
																	// statement
	...
	public void add(Iterable<? extends Statement> statements, Resource... contexts) throws RepositoryException {
		...
		boolean useStatementContext = (contexts != null && contexts.length == 0); // If
																					// no
																					// context
																					// are
																					// specified,
																					...
																					// statement
																					// context
		Resource[] _contexts = checkDMLContext(contexts); // otherwise, either
															// use all contexts,
															// or do not specify
															// a context
		...
		try {
			while (it.hasNext()) {
				...
				if (contexts != null && contexts.length == 0 && st.getContext() != null)
					_contexts = new Resource[] { st.getContext() }; // try the
																	// context
																	// given by
																	// the
																	// statement
	...
	public RepositoryResult<Namespace> getNamespaces() throws RepositoryException {
		...
		return createRepositoryResult(namespaceList);// new
														// RepositoryResult<Namespace>(new
														// IteratorWrapper(v.iterator()));
	}
	 ...
	 * private synchronized void addToQuadStore(Resource subject, URI predicate,
	 * Value object, Resource... contexts) throws RepositoryException {
	 * verifyIsOpen();
	 * 
	 * try { boolean isAutoCommit = getQuadStoreConnection().getAutoCommit();
	 * 
	 * if (!isAutoCommit && useLazyAdd) { for (int i = 0; i < contexts.length;
	 * i++) { String ctx = contexts[i].stringValue(); StringBuilder data =
	 * batchData.get(ctx); if (data == null) data = new StringBuilder(256);
	 * 
	 * append(subject, data); data.append(' '); append(predicate, data);
	 * data.append(' '); append(object, data); data.append(" .\n");
	 * 
	 * batchData.put(ctx, data); psInsertCount++; } if (psInsertCount >=
	 * BATCH_SIZE) flushDelayAdd(); } else { HashMap<String, StringBuilder> map
	 * = new HashMap<String,StringBuilder>();
	 * 
	 * for (int i = 0; i < contexts.length; i++) { String ctx =
	 * contexts[i].stringValue(); StringBuilder data = map.get(ctx); if (data ==
	 * null) data = new StringBuilder(256);
	 * 
	 * append(subject, data); data.append(' '); append(predicate, data);
	 * data.append(' '); append(object, data); data.append(" .\n");
	 * 
	 * map.put(ctx, data); } flushDelayAddMap(null, map); map.clear(); } } catch
	 * (Exception e) { throw new RepositoryException(e); } }
	 * 
	 * private synchronized void flushDelayAdd() throws RepositoryException {
	 * try { if (psInsertCount > 0) { flushDelayAddMap(null, batchData);
	 * batchData.clear(); psInsertCount = 0; } } catch (Exception e) { throw new
	 * RepositoryException(e); } }
	 * 
	 * private synchronized void dropDelayAdd() throws RepositoryException { try
	 * { if (psInsertCount > 0) { batchData.clear(); psInsertCount = 0; } }
	 * catch (Exception e) {} }
	 ***/
	...
	private void append(Value value, StringBuilder sb) throws RepositoryException {
	...
	private void append(Resource resource, StringBuilder sb) throws RepositoryException {
	...
	private void append(URI uri, StringBuilder sb) throws RepositoryException {
		sb.append("<");
	...
	private void append(Literal lit, StringBuilder sb) throws RepositoryException {
		sb.append("\"");
	...
	private void removeContext(Resource subject, URI predicate, Value object, Resource context) throws RepositoryException {
		...
		try {
			...
			if (subject == null && predicate == null && object == null && context != null) {
				...
				// context should not be null at this point, at the least, it
				// will be a wildcard
				String query = "sparql delete from graph <" + context + "> { " + S + " " + P + " " + O + " } from <" + context + "> where { " + S + " " + P + " " + O + " }";
	...
	/**
	 * protected void verifyTxnActive() throws StoreException { if
	 * (isAutoCommit()) { throw new
