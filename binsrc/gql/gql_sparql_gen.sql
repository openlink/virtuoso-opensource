              ...
              if (aref (elem, 0) = 'NODE')
                {
                  ...
                  if (nvar_name is not null)
                    {
                      -- New INSERT nodes are materialized as fresh IRIs. The
                      -- variable name remains parser context only here.
          ...
          if (isarray (ditem) and aref (ditem, 0) = 'VAR')
            {
              ...
              -- Delete all triples where this node appears
              del_body := concat (del_body, '  ', dvar, ' ?p', cast (di as varchar), ' ?o', cast (di as varchar), ' .\n');
              if (aref (_delete_ast, 1) = 1)  -- DETACH
                {
                  del_body := concat (del_body, '  ?s', cast (di as varchar), ' ?q', cast (di as varchar), ' ', dvar, ' .\n');
                }
            }
        }
  ...
  if (_set_ast is not null)
    {
      ...
      if (set_ins <> '' or set_del <> '')
        {
          declare set_kind varchar;
          if (set_del <> '')
            set_kind := 'DELETE/INSERT';
          else
            set_kind := 'INSERT';
          ...
          sparql_text := concat (sparql_text, set_kind, ' {\n  GRAPH <', graph, '> {\n');
          if (set_del <> '')
