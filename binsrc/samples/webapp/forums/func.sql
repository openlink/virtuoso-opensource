...
DROP PROCEDURE LIST_MESSAGES;
DROP PROCEDURE INSERT_MESSAGES;
DROP PROCEDURE INSERT_USERS;
DROP PROCEDURE INS_PARENT_MESSAGES;
DROP PROCEDURE SESS_CREATE;
DROP PROCEDURE SESS_RESTORE;
DROP PROCEDURE SESS_EXPIRE;
DROP PROCEDURE SESS_EXPIRE_OLD;
DROP PROCEDURE SESS_EXPIRE_ALL;
DROP PROCEDURE MISC_REDIRECT;
DROP PROCEDURE COUNT_MESSAGES;
DROP PROCEDURE CONVERT_DATE ;
DROP PROCEDURE MAX_MSG;
DROP PROCEDURE MAX_DATE;
DROP PROCEDURE FORI_SEARCH_FORM;
DROP PROCEDURE FORI_SEARCH_NAVIGATION;
DROP PROCEDURE PARENT_MESSAGE;
DROP PROCEDURE FORI_SEARCH_RES;
DROP PROCEDURE FORI_SEARCH_GET_MESSAGE_PTITLE;
DROP PROCEDURE MISC_XML_TO_STR;
DROP PROCEDURE CNTNEW_MSG;
...
CREATE PROCEDURE LIST_MESSAGES (in amsg_id integer, in m_level integer)
{
  ...
  SELECT M.MSG_ID, M.MSG_TEXT, M.TIME_CHANGED, A.AUTHOR_NICK INTO m_id, _bk_source, m_time, m_author
   FROM MESSAGES M, AUTHORS A WHERE M.MSG_ID = amsg_id AND M.AUTHOR_ID = A.AUTHOR_ID;
  ...
  declare sr CURSOR FOR SELECT MSG_ID FROM MESSAGES WHERE PARENT_ID = amsg_id;
    ...
    in ins_nick varchar, in ins_title varchar, in ins_text varchar)
{
  ...
  declare ar CURSOR FOR SELECT AUTHOR_ID FROM AUTHORS WHERE AUTHOR_NICK = ins_nick;
  WHENEVER NOT FOUND GOTO nf;
  OPEN  ar;
  FETCH ar INTO m_aut_id;
  CLOSE ar;

  declare fr CURSOR FOR SELECT FORUM_ID FROM MESSAGES WHERE MSG_ID = ins_id;
  OPEN  fr;
  ...
  INSERT INTO MESSAGES (MSG_ID, ANS_NUM, LAST_VISIT, PARENT_ID, AUTHOR_ID, FORUM_ID, MSG_TEXT, TIME_CHANGED)
  ...
  in ins_pass  varchar )
{
  ...
  IF (EXISTS (SELECT 1 FROM AUTHORS WHERE AUTHOR_NICK = ins_nick))
    {
      ac := 'No';
      RETURN ac;
    };
  ...
  lps := md5 (concat (ins_nick, ins_mail, ins_pass));
    INSERT INTO AUTHORS (AUTHOR_ID, AUTHOR_NICK, E_MAIL, FIRST_NAME, FATHER_NAME, AUTH_PASD)
  ...
  in aip_addr    varchar(50))
{
  ...
  --get the author_id
  declare cr CURSOR FOR SELECT AUTHOR_ID, E_MAIL, AUTH_PASD FROM AUTHORS WHERE AUTHOR_NICK = alogin_name;
  ...
  if (uid = lps)
    {
      INSERT INTO SESSIONS (SID, USER_ID, LOGIN_TIME, EXPIRE_TIME, IP_ADDR)
	  VALUES (lsid, luser_id, now(), dateadd('minute', 10, now()), aip_addr);
      UPDATE AUTHORS SET LAST_LOGIN = now() WHERE AUTHOR_ID = luser_id;
...
CREATE PROCEDURE SESS_RESTORE (in realm varchar)
{
  ...
  -- get the user_id
  declare cr CURSOR FOR SELECT USER_ID, SES_VARS FROM SESSIONS WHERE SID = asid;
  ...
  UPDATE SESSIONS SET EXPIRE_TIME = dateadd ('minute', 10, now()) WHERE SID = asid;
...
{
  DELETE FROM SESSIONS WHERE SID = asid;
  COMMIT WORK;
};

CREATE PROCEDURE SESS_EXPIRE_OLD ()
{
  DELETE FROM SESSIONS WHERE EXPIRE_TIME <= now();
  COMMIT WORK;
};
...
CREATE PROCEDURE COUNT_MESSAGES(in amsg_id integer)
{
  ...
  FOR (SELECT MSG_ID FROM MESSAGES WHERE PARENT_ID = amsg_id)
      DO
	{
  ...
  in atime   datetime)
{
  ...
  result := atime;
  for (SELECT MSG_ID, TIME_CHANGED FROM MESSAGES WHERE PARENT_ID = amsg_id)
      do
       {
...
CREATE PROCEDURE PARENT_MESSAGE(in amsg_id integer)
{
  ...
  SELECT MSG_ID, MSG_TEXT, PARENT_ID INTO m_id, _bk_source, m_parent FROM MESSAGES WHERE MSG_ID = amsg_id;
...
CREATE PROCEDURE FORI_SEARCH_GET_MESSAGE_PTITLE (in aeid integer, in apos integer)
{
  ...
  SELECT M.MSG_ID, M.MSG_TEXT, M.TIME_CHANGED, A.AUTHOR_NICK, M.FORUM_ID, F.PARENT_ID
      INTO pid, _bk_source, ltime, lnick, tid, fid FROM MESSAGES M, AUTHORS A, FORUMS F
...
CREATE PROCEDURE CNTNEW_MSG(IN amsg_id integer)
{
  ...
  FOR (select MSG_ID FROM MESSAGES WHERE PARENT_ID = amsg_id and datediff ('hour', TIME_CHANGED, now()) <= 24)
    DO
      {
  ...
  if (http_map_get ('persist_ses_vars') and connection_is_dirty ())
    {
      ...
      if (sid is not null)
        update SESSIONS set SES_VARS = serialize (vars) where SID = sid;
    }
