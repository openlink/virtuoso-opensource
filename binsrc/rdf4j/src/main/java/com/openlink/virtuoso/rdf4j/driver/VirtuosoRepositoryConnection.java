...
/**
 * Main interface for updating data in and performing queries on a Sesame repository. By default, a RepositoryConnection
 * is in autoCommit mode, meaning that each operation corresponds to a single transaction on the underlying store.
 * autoCommit can be switched off in which case it is up to the user to handle transaction commit/rollback. Note that
 * care should be taking to always properly close a RepositoryConnection after one is finished with it, to free up
 * resources and avoid unnecessary locks.
 * <p>
 * Several methods take a vararg argument that optionally specifies a (set of) context(s) on which the method should
 * operate. Note that a vararg parameter is optional, it can be completely left out of the method call, in which case a
 * method either operates on a provided statements context (if one of the method parameters is a statement or collection
 * of statements), or operates on the repository as a whole, completely ignoring context. A vararg argument may also be
 * 'null' (cast to Resource) meaning that the method operates on those statements which have no associated context
 * only.
 * <p>
 * Examples:
 *
...
public class VirtuosoRepositoryConnection implements RepositoryConnection {
    ...
    public VirtuosoRepositoryConnection(VirtuosoRepository repository, Connection connection)
        ...
        this.concurrencyMode = repository.concurencyMode;
     ...
     *
     * @param parserConfig a Rio RDF Parser configuration.
     */
    ...
    /**
     * Closes the connection, freeing resources. If the connection is not in autoCommit mode, all non-committed
     ...
     * that need to be resolved against an external base URI, one should use {@link #prepareQuery(QueryLanguage, String,
     * String)} instead.
     *
     * @param ql    The query language in which the query is formulated.
     ...
     * @throws UnsupportedQueryLanguageException If the supplied query language is not supported.
     */
    public TupleQuery prepareTupleQuery(QueryLanguage ql, final String query, final String baseURI)
            throws RepositoryException, MalformedQueryException {
        if (ql != QueryLanguage.SPARQL)
            throw new UnsupportedQueryLanguageException(" : Only SPARQL queries are supported");
        ...
        return new VirtuosoTupleQuery() {
            public TupleQueryResult evaluate() throws QueryEvaluationException {
                return executeSPARQLForTupleResult(baseURI, query, getDataset(), getIncludeInferred(), getBindings(),
                        getMaxExecutionTime());
            }

            public void evaluate(TupleQueryResultHandler handler)
                    throws QueryEvaluationException, TupleQueryResultHandlerException {
                executeSPARQLForHandler(baseURI, handler, query, getDataset(), getIncludeInferred(), getBindings(),
                        getMaxExecutionTime());
            }
        };
    }
     ...
     * @throws UnsupportedQueryLanguageException If the supplied query language is not supported.
     */
    public GraphQuery prepareGraphQuery(QueryLanguage ql, final String query, final String baseURI)
            throws RepositoryException, MalformedQueryException {
        if (ql != QueryLanguage.SPARQL)
            throw new UnsupportedQueryLanguageException(" : Only SPARQL queries are supported");
        ...
        return new VirtuosoGraphQuery() {
            public GraphQueryResult evaluate() throws QueryEvaluationException {
                return executeSPARQLForGraphResult(baseURI, query, getDataset(), getIncludeInferred(), getBindings(),
                        getMaxExecutionTime());
            }
    ...
    /**
     * Prepares <tt>true</tt>/<tt>false</tt> queries. In case the query contains relative URIs that need to be resolved
     * against an external base URI, one should use {@link #prepareBooleanQuery(QueryLanguage, String, String)}
     * instead.
     *
     * @param ql    The query language in which the query is formulated.
     * @param query The query string.
     ...
     * @throws UnsupportedQueryLanguageException If the supplied query language is not supported.
     */
    public BooleanQuery prepareBooleanQuery(QueryLanguage ql, final String query, final String baseURI)
            throws RepositoryException, MalformedQueryException {
        if (ql != QueryLanguage.SPARQL)
            throw new UnsupportedQueryLanguageException(" : Only SPARQL queries are supported");
        ...
        return new VirtuosoBooleanQuery() {
            public boolean evaluate() throws QueryEvaluationException {
                return executeSPARQLForBooleanResult(baseURI, query, getDataset(), getIncludeInferred(), getBindings(),
                        getMaxExecutionTime());
            }
        };
    }
    ...
    public Update prepareUpdate(QueryLanguage ql, final String query, final String baseURI)
            throws RepositoryException, MalformedQueryException {
        if (ql != QueryLanguage.SPARQL)
            throw new UnsupportedQueryLanguageException(" : Only SPARQL queries are supported");
    ...
    /**
     * Gets all resources that are used as content identifiers. Care should be taken that the returned {@link
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    throw new RepositoryException(
                            "VirtuosoRepositoryConnection.getContextIDs() Non-URI context encountered: " + obj);
                }
            }
            rs.close();
            stmt.close();
     ...
     * lazy Iterator-like object containing {@link Statement}s and optionally throwing a {@link RepositoryException}
     * when an error when a problem occurs during retrieval.
     */
    ...
    public void exportStatements(Resource subj, IRI pred, Value obj, boolean includeInferred, RDFHandler handler,
        ...
        try (RepositoryResult<Namespace> nsIt = getNamespaces()) {
            while (nsIt.hasNext()) {
    ...
    /**
     * Begins a transaction requiring {@link #commit()} or {@link #rollback()} to be called to end the transaction.
     *
     * @throws RepositoryException If the connection could not start a transaction.
     * @see #isActive()
     * @see #commit()
     * @see #rollback()
     * @since 2.7.0
    ...
    /**
     * Begins a new transaction with the supplied {@link IsolationLevel}, requiring {@link #commit()} or {@link
     ...
     *              not supported by the underlying store, it will attempt to use a supported {@link
     *              IsolationLevel#isCompatibleWith(IsolationLevel) compatible level} instead.
     ...
     *
     * @throws RepositoryException If the connection could not be committed.
     */
     ...
     *
     * @throws RepositoryException If the connection could not be rolled back.
     */
     ...
     *                                      repository is not writable.
     */
    public synchronized void add(Reader reader, String baseURI, RDFFormat dataFormat, final Resource... contexts)
            throws RDFParseException, RepositoryException {
        verifyIsOpen();
        flushDelayAdd();

        final boolean useStatementContext = (contexts != null &&
                contexts.length == 0); // If no context are specified, each statement is added to statement context
     ...
     *                   value of {@link URL#toExternalForm() url.toExternalForm()} if the value is set to
     *                   <tt>null</tt>.
     * @param dataFormat The serialization format of the data.
     * @param contexts   The contexts to add the data to. If one or more contexts are specified the data is added to
     *                   these contexts, ignoring any context information in the data itself.
     ...
     * @param baseURI    The base URI to resolve any relative URIs that are in the data against. This defaults to the
     *                   value of {@link File#toURI() file.toURI()} if the value is set to
     *                   <tt>null</tt>.
     * @param dataFormat The serialization format of the data.
     ...
     *                             repository is not writable.
     */
    public synchronized void add(Iterable<? extends Statement> statements, Resource... contexts)
            throws RepositoryException {
        verifyIsOpen();
        flushDelayAdd();

        Iterator<? extends Statement> it = statements.iterator();
        boolean useStatementContext = (contexts != null &&
                contexts.length == 0); // If no context are specified, each statement is added to statement context
        Resource[] _contexts = checkDMLContext(
                contexts); // otherwise, either use all contexts, or do not specify a context
     ...
     * Adds the supplied statements to this repository, optionally to one or more named contexts.
     *
     * @param statements The statements to add. In case the iterator is a {@link CloseableIteration}, it will be closed
     ...
     *                             repository is not writable.
     */
    public synchronized <E extends Exception> void add(Iteration<? extends Statement, E> statements,
     ...
     * @param contexts   The context(s) to remove the data from. Note that this parameter is a vararg and as such is
     *                   optional. If no contexts are supplied the method operates on the contexts associated with the
     *                   statement itself, and if no context is associated with the statement, on the entire
     *                   repository.
     * @throws RepositoryException If the statements could not be added to the repository, for example because the
     *                             repository is not writable.
     */
     ...
     *
     * @param statements The statements to remove. In case the iterator is a {@link CloseableIteration}, it will be
    ...
    public RepositoryResult<Namespace> getNamespaces() throws RepositoryException {
        ...
        return createRepositoryResult(
                namespaceList);// new RepositoryResult<Namespace>(new IteratorWrapper(v.iterator()));
    }
    ...
    protected TupleQueryResult executeSPARQLForTupleResult(String baseURI, String query,
                                                           Dataset dataset, boolean includeInferred,
                                                           BindingSet bindings,
                                                           int maxQueryTime) throws QueryEvaluationException {

        List<String> names = new ArrayList<>();
        try {
            PreparedStatement stmt = executeSPARQL(baseURI, query, dataset, includeInferred, bindings, maxQueryTime,
                    false);
    ...
    protected void executeSPARQLForHandler(String baseURI, TupleQueryResultHandler tqrh,
                                           ...
                                           BindingSet bindings, int maxQueryTime)
            throws QueryEvaluationException, TupleQueryResultHandlerException {
    ...
    protected void executeSPARUL(String baseURI, String query, Dataset dataset,
                                 boolean includeInferred, BindingSet bindings, int maxQueryTime)
     ...
     * @throws RepositoryException If the <tt>prepareQuery</tt> method is not supported by this repository.
     */
     ...
     *
     * @param quadStoreConnection The Repository Connection.
     */
    ...
    private java.sql.Statement createStatement(int maxQueryTime, boolean isIUD) throws SQLException {
        java.sql.Statement stmt = quadStoreConnection
                .createStatement(ResultSet.TYPE_FORWARD_ONLY, getJdbcConcurrency(isIUD));
    ...
    private PreparedStatement prepareStatement(String sql, boolean isIUD) throws SQLException {
        PreparedStatement stmt = quadStoreConnection
                .prepareStatement(sql, ResultSet.TYPE_FORWARD_ONLY, getJdbcConcurrency(isIUD));
        int timeout = queryTimeout;
        if (timeout > 0)
            stmt.setQueryTimeout(timeout);
        stmt.setFetchSize(prefetchSize);
    ...
    private String substBindings(String query, BindingSet bindings, List<Value> pstmtParams, boolean isSPARUL)
            throws RepositoryException {
        boolean use_setParams = !insertBNodeAsVirtuosoIRI;
    ...
    private String fixQuery(boolean isSPARUL, String query, Dataset dataset, boolean includeInferred,
                            BindingSet bindings, List<Value> pstmtParams, String baseURI) throws RepositoryException {
        ...
        if (baseURI != null && baseURI.length() > 0)
            ret.append(" define input:graph-base <").append(baseURI).append("> \n");

        if (includeInferred && ruleSet != null && ruleSet.length() > 0)
            ret.append("define input:inference '").append(ruleSet).append("'\n ");

        if (includeInferred && macroLib != null && macroLib.length() > 0)
            ret.append("define input:macro-lib <").append(macroLib).append(">\n ");

        if (dataset != null) {
            list = dataset.getDefaultGraphs();
            if (list != null) {
                for (IRI v : list) {
                    ret.append(" define input:default-graph-uri <").append(v.toString()).append("> \n");
            ...
            if (list != null) {
                for (IRI v : list) {
                    ret.append(" define input:named-graph-uri <").append(v.toString()).append("> \n");
                }
            }
        }

        if (!added_def_graph && useDefGraphForQueries)
            ret.append(" define input:default-graph-uri <").append(defGraph).append("> \n");
    ...
    private String SubstGraphs(String query, String gInsert, String gDelete) {
        ArrayList<String> lst = new ArrayList<>(16);
        StringBuilder buf = new StringBuilder();
        String delim = "{} \t\n\r\f";
        int i = 0;
        char ch;
    ...
    /**
     * NOTES: if WITH exists, it doesn't use addTerm if GRAPH exists, it doesn't use addTerm if INSERT INTO exists, it
     * doesn't use addTerm if DELETE FROM exists, it doesn't use addTerm
     **/
    ...
    private synchronized void addToQuadStore(Resource subject, IRI predicate, Value object, Resource... contexts)
            throws RepositoryException {
    ...
    private void append(Literal lit, StringBuilder sb, boolean inTriplePattern)
        ...
        } else if (lit.getLanguage().isPresent()) {
    ...
    private CloseableIteration<Statement, RepositoryException> selectFromQuadStore(Resource subject, IRI predicate,
                                                                                   Value object,
        ...
        if (includeInferred && ruleSet != null && ruleSet.length() > 0)
            query.append("define input:inference '").append(ruleSet).append("' ");

        if (includeInferred && macroLib != null && macroLib.length() > 0)
            query.append("define input:macro-lib <").append(macroLib).append(">\n ");
        ...
        try {
            ...
            throw new RepositoryException(
                    getClass().getCanonicalName() + ": SPARQL execute failed." + "\n" + query.toString() + "[" + e +
                            "]", e);
        }
    ...
    private void removeData(Map<String, StringBuilder> data) throws RepositoryException {
        try (java.sql.Statement stmt = createStatement(-1, true)) {
    ...
    private void removeContext(Resource subject, IRI predicate, Value object, Resource context)
            throws RepositoryException {
        try {
    ...
    private void bindValue(PreparedStatement ps, int col, Value n) throws SQLException {
        ...
        if (n instanceof IRI) {
            ...
            ps.setNull(col + 2, Types.VARCHAR);
        ...
        if (val instanceof ExtendedString) {
            ...
            if (ves.getIriType() == ExtendedString.IRI && (ves.getStrType() & 0x01) == 0x01) {
                ...
                try {
                    ...
                    throw new RepositoryException(
                            "VirtuosoRepositoryConnection().castValue() Invalid value from Virtuoso: \"" + valueString +
                                    "\"", iaex);
                }
            ...
            if (rb.getLang() != null) {
                ...
                if (rb_val.length() == 1 && (rb_val.charAt(0) == '1' || rb_val.charAt(0) == '0')) {
                    if (rb_type.equals("http://www.w3.org/2001/XMLSchema#boolean"))
                        return valueFactory.createLiteral(rb_val.charAt(0) == '1' ? "true" :
                                                          "false"); //  return getRepository().getValueFactory().createLiteral(rb_val.charAt(0)=='1'?"true":"false", this.getRepository().getValueFactory().createURI(rb_type));
                }
            ...
            return valueFactory.createLiteral((Long) val);
        } else if (val instanceof Integer) {
            return valueFactory.createLiteral((Integer) val);
        } else if (val instanceof Short) {
            return valueFactory.createLiteral(((Short) val).intValue());
        } else if (val instanceof Float) {
            return valueFactory.createLiteral((Float) val);
        } else if (val instanceof Double) {
            return valueFactory.createLiteral((Double) val);
            ...
            IRI type = valueFactory.createIRI("http://www.w3.org/2001/XMLSchema#dateTime");
            return valueFactory.createLiteral(Timestamp2String((Timestamp) val), type);
            ...
            try {
                ...
                throw new RepositoryException(
                        "VirtuosoRepositoryConnection().castValue() Could not parse resource: " + val, iaex2);
            }
        }
    }
    ...
    /**
     * protected void verifyTxnActive() throws StoreException { if (isAutoCommit()) { throw new
     * RepositoryException("Connection does not have an active transaction"); } }
     **/

    public abstract static class CloseableIterationBase<E, X extends Exception> implements CloseableIteration<E, X> {
        E v_row;
        ...
        public CloseableIterationBase(java.sql.Statement stmt, ResultSet rs, Resource subject, IRI predicate,
                                      Value object) {
            v_stmt = stmt;
            v_rs = rs;
    ...
    public class CloseableIterationStmt extends CloseableIterationBase<Statement, RepositoryException> {
        ...
        public CloseableIterationStmt(java.sql.Statement stmt, ResultSet rs, Resource subject, IRI predicate,
                                      Value object)
                throws RepositoryException {
        ...
        protected void extractRow() throws Exception {
            ...
            if (_subject == null && col_s != -1)
                try {
                    ...
                    throw new RepositoryException(
                            "Unexpected resource type encountered. Was expecting Resource: " + val, ccex);
                }
