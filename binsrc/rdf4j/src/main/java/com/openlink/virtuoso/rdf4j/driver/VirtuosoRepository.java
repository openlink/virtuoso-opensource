...
/**
 * A Sesame repository that contains RDF data that can be queried and updated. Access to the repository can be acquired
 * by openening a connection to it. This connection can then be used to query and/or update the contents of the
...
public class VirtuosoRepository implements Repository {
     ...
     *
     * @param url_hostlist the Virtuoso JDBC URL connection string or hostlist for poolled connection. Virtuoso database
     *                     hostlist
     *                     <pre>
     ...
     * @param defGraph     a default Graph name, used for Sesame calls, when contexts list is empty, exclude
     *                     <tt>exportStatements, hasStatement, getStatements</tt> methods
     * @param useLazyAdd   set <tt>true</tt>  to enable using batch optimization for sequence of
     *                     <pre>
     *                     	  add(Resource subject, URI predicate, Value object, Resource... contexts);
     *                            add(Statement statement, Resource... contexts);
     *                            </pre>
     *                     methods, when autoCommit mode is off. The triples will be sent to DBMS on commit call or when
     *                     batch size become more than predefined batch max_size. LazyAdd will be set false for
     *                     XADataSource connection
     */
    ...
    /**
     * Construct a VirtuosoRepository with a specified parameters. useLazyAdd will be set to <tt>false</tt>.
     *
     * @param url_hostlist the Virtuoso JDBC URL connection string or hostlist for poolled connection. Virtuoso database
     *                     hostlist
     *                     <pre>
    ...
    public RepositoryConnection getConnection() throws RepositoryException {
        if (_xads != null) {
            ...
            try {
                ...
                if (roundrobin && !url.toLowerCase().contains("roundrobin=")) {
                    if (url.charAt(url.length() - 1) != '/')
                ...
                if (!url.toLowerCase().contains("log_enable=")) {
                    if (url.charAt(url.length() - 1) != '/')
            ...
            try {
                ...
                pds.setCharset(StandardCharsets.UTF_8.name());
     ...
     *
     * @param seconds queryTimeout seconds, 0 - unlimited.
     */
    ...
    /**
     * Set the UseLazyAdd state for connection(default true) for XADataSource connection set false and can't be changed
     *
     * @param v true - useLazyAdd
     */
     ...
     *
     * @param v true - insert BNode as Virtuoso IRI false - insert BNode as Virtuoso Native BNode
     */
     ...
     *
     * @param v true - insert String Literals as Simple Literals false - insert String Literals as is
     */
     ...
     *
     * @param v true - useDefGraphForQueries
     */
