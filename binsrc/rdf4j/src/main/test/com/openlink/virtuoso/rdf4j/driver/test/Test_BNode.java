...
public class Test_BNode {
	...
	public static void Test_OneRow(String[] args, boolean insertBNodeAsIRI) {
		...
		try {
			...
			System.out.println("\n\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
			if (insertBNodeAsIRI) System.out.println("Test with One Row operation (BNode as Virtuoso IRI)");
			else System.out.println("Test with One Row operation (BNode as Virtuoso Native BNode)");
			System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			...
			try {
				...
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				Object o = st.getObject();
				if (!(o instanceof BNode)) throw new Exception("Value must be BNode");
				bn1 = (BNode) o;
				log("Got bn1=" + bn1);
				...
				if (bn1.equals(bn2)) throw new Exception("Error bn1 == bn2");
			...
			try {
				...
				if (!rc) throw new Exception("hasStatement return FALSE for bn1");
				log("hasStatement with bn1 OK");

				rc = con.hasStatement(bn2, np, no, false, context);
				if (!rc) throw new Exception("hasStatement return FALSE for bn2");
			...
			try {
				...
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				if (!st.getSubject().equals(ns1)) throw new Exception("Subject must be :" + ns1);
				if (!st.getPredicate().equals(np)) throw new Exception("Predicate must be :" + np);
				if (!st.getObject().equals(bn1)) throw new Exception("Subject must be :" + bn1);
				log("getStatements with bn1 OK");

				rs = con.getStatements(bn2, np, no, false, context);
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				if (!st.getSubject().equals(bn2)) throw new Exception("Subject must be :" + ns1);
				if (!st.getPredicate().equals(np)) throw new Exception("Predicate must be :" + np);
				if (!st.getObject().equals(no)) throw new Exception("Subject must be :" + no);
			...
			try {
				...
				rc = con.hasStatement(ns1, np, bn1, false, context);
				if (rc) throw new Exception("Triple with bn1 wasn't removed");
				...
				rc = con.hasStatement(bn2, np, no, false, context);
				if (rc) throw new Exception("Triple with bn2 wasn't removed");
			...
			if (!insertBNodeAsIRI) {
				...
				try {
					...
					if (bn1.equals(bnew)) throw new Exception("Error bn1 == bnew");
	...
	public static void Test_Batch(String[] args, boolean insertBNodeAsIRI) {
		...
		try {
			 ...
			 * Try batch insert :
			 * 
			 * _:a t:p1 t:o1 _:a t:p2 _:b _:b t:p3 t:o3
			 * 
			 ***************/

			System.out.println("\n\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
			if (insertBNodeAsIRI) System.out.println("Test with Batch Operation (BNode as Virtuoso IRI)");
			else System.out.println("Test with Batch Operation (BNode as Virtuoso Native BNode)");
			System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
			...
			try {
				...
				if (ba.equals(bb)) throw new Exception("Error ba == bb");
			...
			try {
				...
				if (!rc) throw new Exception("hasStatement return FALSE for ba");
				log("hasStatement with ba OK");

				rc = con.hasStatement(ba, np2, bb, false, context);
				if (!rc) throw new Exception("hasStatement return FALSE for ba & bb");
				log("hasStatement with ba & bb OK");

				rc = con.hasStatement(bb, np3, no3, false, context);
				if (!rc) throw new Exception("hasStatement return FALSE for bb");
				log("hasStatement with ba OK");
			...
			try {
				...
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				if (!st.getSubject().equals(ba)) throw new Exception("Subject must be :" + ba);
				if (!st.getPredicate().equals(np1)) throw new Exception("Predicate must be :" + np1);
				if (!st.getObject().equals(no1)) throw new Exception("Subject must be :" + no1);
				...
				st = rs.next();
				if (!st.getSubject().equals(ba)) throw new Exception("Subject must be :" + ba);
				if (!st.getPredicate().equals(np2)) throw new Exception("Predicate must be :" + np2);
				if (!st.getObject().equals(bb)) throw new Exception("Subject must be :" + bb);
				log("getStatements with ba & bb OK");

				rs = con.getStatements(bb, np3, no3, false, context);
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				if (!st.getSubject().equals(bb)) throw new Exception("Subject must be :" + bb);
				if (!st.getPredicate().equals(np3)) throw new Exception("Predicate must be :" + np3);
				if (!st.getObject().equals(no3)) throw new Exception("Subject must be :" + no3);
				log("getStatements with bn2 OK");
			...
			try {
				...
				rc = con.hasStatement(ba, np1, no1, false, context);
				if (rc) throw new Exception("Triple with ba wasn't removed");
				log("remove with ba OK");

				rc = con.hasStatement(ba, np2, bb, false, context);
				if (rc) throw new Exception("Triple with ba & bb wasn't removed");
				...
				rc = con.hasStatement(bb, np3, no3, false, context);
				if (rc) throw new Exception("Triple with bb wasn't removed");
	...
	public static void Test_ImportFromFile(String[] args, boolean insertBNodeAsIRI) {
		...
		try {
			...
			try {
				...
				if (!rs.hasNext()) throw new Exception("ResultSet is empty");
				st = rs.next();
				if (!st.getSubject().equals(ns)) throw new Exception("Subject must be :" + ns);
				if (!st.getPredicate().equals(np)) throw new Exception("Predicate must be :" + np);

				BNode[] bn_arr = new BNode[3];
				bn_arr[0] = (BNode) st.getObject();
				if (!rs.hasNext()) throw new Exception("ResultSet have not 2 row");

				st = rs.next();
				bn_arr[1] = (BNode) st.getObject();
				if (!rs.hasNext()) throw new Exception("ResultSet have not 3 row");
				...
				o_val = st.getObject();
				if (!(o_val instanceof Literal)) throw new Exception("" + o_val + " Must be Literal");
				l_val = (Literal) o_val;
				if (!data.contains(l_val.getLabel())) throw new Exception("Wrong data was received =" + o_val);
