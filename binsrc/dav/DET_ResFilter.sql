...
{
  -- dbg_obj_princ ('ResFilter_FNMERGE (', path, id, ')');
  declare pairs any;
  declare res varchar;
...
{
  -- dbg_obj_princ ('ResFilter_DAV_SEARCH_ID_IMPL (', detcol_id, path_parts, what, ')');
...
create function "ResFilter_DAV_PROP_SET" (
  ...
  in auth_uid integer) returns any
{
  -- dbg_obj_princ ('ResFilter_DAV_PROP_SET (', id, what, propname, propvalue, overwrite, auth_uid, ')');
...
create function "ResFilter_DAV_DIR_LIST" (
  in detcol_id any,
  in path_parts any,
  ...
  in auth_uid integer) returns any
{
  ...
  else
  {
    ...
    tmp_cond := vector ('', vector_concat (vector (vector ('RES_NAME', 'like', name_mask)), get_keyword ('', rfc_list_cond)));
...
create function "ResFilter_DAV_DIR_FILTER" (
  in detcol_id any,
  in path_parts any,
  in detcol_path varchar,
...
create function "ResFilter_DAV_SEARCH_ID" (
  in detcol_id any,
  in path_parts any,
...
create function "ResFilter_DAV_SEARCH_PATH" (
  in id any,
  in what char(1)) returns any
{
  ...
  if (isarray (id[2]))
  {
    rc := call (cast (id[2][0] as varchar) || '_DAV_SEARCH_PATH') (id[2], what);
    if (DAV_HIDE_ERROR (rc) is null)
      return rc;

    return DAV_CONCAT_PATH (DAV_SEARCH_PATH (id[1], 'C'), DB.DBA.DAV_DET_PATH_NAME (rc));
  }
...
create function "ResFilter_DAV_RES_CONTENT" (
  in id any,
  inout content any,
  out type varchar,
  in content_mode integer) returns integer
{
  ...
  if (isarray (id[2]))
  {
    return call (cast (id[2][0] as varchar) || '_DAV_RES_CONTENT') (id[2], content, type, content_mode);
  }
  else if ((content_mode = 0) or (content_mode = 2))
  {
...
create function "ResFilter_DAV_IS_LOCKED" (
  inout id any,
  inout type char(1),
