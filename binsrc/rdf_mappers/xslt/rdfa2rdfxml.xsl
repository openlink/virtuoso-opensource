...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*[@property and not (@href)]">
	  ...
	  <rdf:Description rdf:about="{$about}">
		  ...
		  <xsl:choose>
		      ...
		      <xsl:otherwise><xsl:call-template name="elem-cont"/></xsl:otherwise>
		  </xsl:choose>
	      </xsl:element>
	  </rdf:Description>
      </xsl:if>
  ...
  <xsl:template name="dt-attr">
	  ...
	  <xsl:when test="@datatype and @datatype != ''">
		  ...
		  <xsl:attribute name="rdf:datatype">
		      <xsl:value-of select="string ($nss//namespace[@prefix = $elem-ns])"/><xsl:value-of select="substring-after (@datatype, ':')"/>
		  </xsl:attribute>
	      </xsl:if>
	  </xsl:when>
  ...
  <xsl:template name="rev-uri">
      <xsl:variable name="qname">
	  <xsl:choose>
	      <xsl:when test="@rev"><xsl:value-of select="@rev"/></xsl:when>
	      <!--xsl:when test="@property"><xsl:value-of select="@property"/></xsl:when-->
	  </xsl:choose>
      </xsl:variable>
      <xsl:variable name="elem-ns" select="substring-before ($qname, ':')"/>
  ...
  <xsl:template name="about-ancestor-or-self">
	  ...
	  <xsl:with-param name="uri">
	      <xsl:choose>
		  <xsl:when test="@about and ancestor-or-self::*/@xml:base"><xsl:value-of select="resolve-uri (@about, ancestor-or-self::*/@xml:base)"/></xsl:when>
