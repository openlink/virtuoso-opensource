...
OAT.Declarative = {
	...
	shouldBeProcessed:function(domNode) {
		/*
			...
			<div oaType="oat:tab" oaParams="'p1',{p2}" >
		*/
		if (domNode.getAttribute("oaType") && domNode.getAttribute("oaType").toString().split(":")[0].toLowerCase() == "oat") { return true; }
		return false;
	},
	...
	scanningFinished:function() { /* process deferred elements */
		...
		*/
		...
		for (var i=0;i<OAT.Declarative.toBeProcessed.length;i++) {
			...
			var type = elm.getAttribute("oatype").toString().split(":");
			var params = [];
			if (elm.getAttribute("oaParams")) { params = elm.getAttribute("oaParams").toString().split(";"); }
			var name = type[1];
			var parentArr = queue[0];
			if (partTwo.find(name) != -1) { parentArr = queue[1]; }
			if (partThree.find(name) != -1) { parentArr = queue[2]; }
			var obj = {name:name,params:[],elm:elm};
			for (var j=0;j<3;j++) { /* JSON params */
				var str = (j < params.length ? params[j] : "false");
				obj.params.push(eval('('+str+')'));
			}
		...
		for (var i=0;i<queue[0].length;i++) {
			...
			switch (o.name) {
				case "drag":
					var target = o.params[0] ? o.params[0] : o.elm;
					var params = o.params[1] ? o.params[1] : o.params[0];
				...
				break;
				case "tree":
					OAT.Tree.assign(o.elm,o.params[0]);
				break;
				case "rounddiv":
					OAT.SimpleFX.roundDiv(o.elm,o.params[0]);
				break;
				case "roundimg":
					OAT.SimpleFX.roundDiv(o.elm,o.params[0]);
				break;
				case "shadow":
					OAT.SimpleFX.shadow(o.elm,o.params[0]);
				break;
				case "shader":
					var target = o.params[0] ? o.params[0] : o.elm;
				...
				break;
				case "validation":
					OAT.Validation.create(o.elm,o.params[0], o.params[1]);
				break;
				case "grid":
					var g = new OAT.Grid(o.elm,o.params[1],o.params[2]);
					g.fromTable(o.params[0]);
					addObject("grid",g);
				break;
				case "anchor":
					OAT.Anchor.assign(o.elm,o.params[0]);
				break;
			} /* switch */
		}
		...
		for (var i=0;i<queue[1].length;i++) {
			...
			switch (o.name) {
				...
				break;
				case "dock":
					var d = new OAT.Dock(o.elm,o.params[0]);
					addObject("dock",d);
				break;
				case "panelbar":
					var p = new OAT.Panelbar(o.elm,o.params[0]);
			...
			switch (o.name) {
				case "tab_content":
					...
					for (var j=0;j<OAT.Declarative.objects["tab"].length;j++) {
						...
						if (t.div == $(pid)) { t.add(o.params[1],o.elm); }
					}
				break;
				case "dock_content":
					var pid = o.params[0];
					for (var j=0;j<OAT.Declarative.objects["dock"].length;j++) {
						...
						if (d.div == $(pid)) { d.addObject(o.params[1],o.params[2],o.elm); }
					}
				break;
				case "panelbar_content":
					...
					for (var j=0;j<OAT.Declarative.objects["panelbar"].length;j++) {
						...
						if (p.div == $(pid)) { p.addPanel(o.params[1],o.elm); }
					}
				break;
			} /* switch */
		}
