...
public class VirtuosoQueryExecution implements QueryExecution {
    ...
    public void setInitialBinding(QuerySolution arg) {
        if (arg != null) {
          m_arg = BindingLib.toBinding(arg);
        }
    }
 ...
 ***/
        ...
        while (i < qlen) {
            ...
            if (ch == '\\') {
                ...
                if (j != i) {
                    ...
                    Node val = m_arg.get(varName);
                    if (val != null) {
                        varData = graph.Node2Str(val);
                        i = j;
                    }
                }
                if (varData != null)
    ...
    public class VResultSet implements org.apache.jena.query.ResultSet {
        ...
        protected VResultSet(VirtGraph _g, java.sql.Statement _stmt, java.sql.ResultSet _rs) {
            ...
            try {
                rsmd = rs.getMetaData();
                cols = new Var[rsmd.getColumnCount()+1];

                for (int i = 1; i <= rsmd.getColumnCount(); i++)
                    cols[i] = Var.alloc(rsmd.getColumnLabel(i));

                cols[0] = Var.alloc("graph");
            } catch (Exception e) {
        ...
        protected void extractRow() throws Exception {
            ...
            try {
                for (int i = 1; i <= rsmd.getColumnCount(); i++) {
                    ...
                    if (n != null)
                        builder.add(cols[i], n);
                }

                if (virt_graph != null && !virt_graph.equals("virt:DEFAULT"))
                    builder.add(cols[0], NodeFactory.createURI(virt_graph));
            } catch (Exception e) {
                throw new JenaException("ViruosoResultBindingsToJenaResults has FAILED.:" + e);
            }
            v_row = builder.build();
