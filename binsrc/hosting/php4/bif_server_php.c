...
int sapi_virtuoso_ub_write(const char *str, uint str_length TSRMLS_DC)
{
...
static char * ap_lines_get_http_ver(caddr_t *lines)
{
  char *http_ver, *p;
...
static char * ap_lines_get_req_mtd(caddr_t *lines)
{
...
static char * ap_lines_get_line0(caddr_t *lines)
{
  char *header_content, *p;
...
static char *lookup_header (caddr_t *lines, char *key_int)
{
  int len, i;
  char *header_content, *p;
...
static char * ap_lines_get(caddr_t *lines, char *key)
{
...
zend_op_array *virt_php_compile_file(zend_file_handle *file_handle, int type TSRMLS_DC)
{
  thr_atrp *thra =  (thr_atrp *) THR_ATTR (THREAD_CURRENT_THREAD, VIRT_PRINT_OUT);
...
static PHP_MINIT_FUNCTION(virt)
{
...
check_php_version ()
{
  ...
  if (zend_get_constant ("PHP_VERSION", sizeof ("PHP_VERSION") - 1, &val TSRMLS_CC)
     && Z_TYPE (val) == IS_STRING)
    {
...
/*    if (php_version_compare (PHP_VERSION, &php_dll_version) == 1) */
        {
           log_info (
              "PHP " VIRT_PHP_RUNTIME_NAME " version %s does not match the compilation version %s; "
              "unpredictable behaviour may result.",
              php_dll_version, PHP_VERSION);
        }
    }
  else
    log_info (
       "PHP " VIRT_PHP_RUNTIME_NAME " version unknown. "
       "If your PHP " VIRT_PHP_RUNTIME_NAME " version is not %s, unpredictable behaiviour may follow",
       PHP_VERSION);
}
...
caddr_t
http_handler_php (char* file, caddr_t* params, caddr_t* lines, caddr_t string, caddr_t ** head_ret,
    query_instance_t * qi)
{
...
bif_php_str (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  return http_handler_php (NULL, params, lines, in_string, NULL, (query_instance_t *)qst);
}
#endif
