...
ins_blob (long row_no, int n_blocks)
{
  long w_start;
  long rc, n_param, c;
  SQLLEN l1 = SQL_DATA_AT_EXEC, l2 = SQL_DATA_AT_EXEC, l3 = SQL_DATA_AT_EXEC, l4 = SQL_DATA_AT_EXEC;
  static char temp[10000], btemp[10000];
  static wchar_t wtemp[10000];
  if (!ins_stmt)
    {
      int inx;
	...
	{
	  wtemp [inx] = (wchar_t)(temp[inx] = 'A' + (inx % ('Z' + 1 - 'A')));
  ...
  IS_ERR (ins_stmt,
      SQLSetParam (ins_stmt, 3, SQL_C_BINARY, SQL_LONGVARBINARY, 10, 0, (void *) 3L, &l2));
  IS_ERR (ins_stmt,
      SQLSetParam (ins_stmt, 4, SQL_C_WCHAR, SQL_WLONGVARCHAR, 10, 0, (void *) 4L, &l3));
  IS_ERR (ins_stmt,
      SQLSetParam (ins_stmt, 5, SQL_C_CHAR, SQL_LONGVARBINARY, 10, 0, (void *) 5L, &l4));
...
upd_blob (long row_no, int n_blocks)
{
  ...
  if (!upd_stmt)
    {
      ...
      for (inx = 0; inx < sizeof (temp); inx++)
	{
	  wtemp [inx] = (wchar_t)(temp[inx] = 'A' + (inx % ('Z' - 'A')));
	  if (inx < sizeof (temp) / 2)
	    {
	      btemp [inx * 2] = ((temp[inx] & 0xF0) >> 4) + ((((temp[inx] & 0xF0) >> 4) < 10) ? '0' : 'A' - 10);
	      btemp [inx * 2 + 1] = (temp[inx] & 0x0F) + (((temp[inx] & 0x0F) < 10) ? '0' : 'A' - 10);
  ...
  SQLSetParam (upd_stmt, 2, SQL_C_BINARY, SQL_LONGVARBINARY, 0, 0, (void *) 2L, &l2);
  SQLSetParam (upd_stmt, 3, SQL_C_WCHAR, SQL_WLONGVARCHAR, 0, 0, (void *) 3L, &l3);
  SQLSetParam (upd_stmt, 4, SQL_C_CHAR, SQL_LONGVARBINARY, 0, 0, (void *) 4L, &l4);
...
read_bound_blobs ()
{
  ...
  IF_ERR_GO (st, err,
      SQLExecDirect (st, (UCHAR *) "select B1, B2, B3, B4 from BLOBS", SQL_NTS));
  ...
  for (;;)
      ...
      if (rc == SQL_ERROR)
	{
      ...
      temp[10] = 0;
      wtemp[10] = 0;
      printf (" Bound clob len %ld  %s\n", (long) len1, temp);
      printf (" Bound blob len %ld  %s\n", (long) len2, wtemp);
      printf (" Bound nlob len %ld\n", (long) len3);
      printf (" Bound cblob len %ld  %s\n", (long) len4, wtemp4);
    }
  ...
  if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
    {
      ...
      check_blob_col (sel_stmt, 2, expect_n_blocks * 5000, SQL_C_BINARY);
      check_blob_col (sel_stmt, 3, expect_n_blocks * 10000 * sizeof (wchar_t), SQL_C_WCHAR);
