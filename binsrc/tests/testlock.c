...
static long nbrea = 0;
static long loop = 0;
static SQLINTEGER isolation = 0;

void print_error(struct thr_t* t)
{
  ...
  while(SQLError(t->henv, t->hdbc, t->hstmt, sqlstate, NULL,
     buf, sizeof(buf), NULL) == SQL_SUCCESS)
  {
     printf("%s, SQLSTATE=%s\n", buf, sqlstate);
  }

  while(SQLError(t->henv, t->hdbc, SQL_NULL_HSTMT, sqlstate, NULL,
     buf, sizeof(buf), NULL) == SQL_SUCCESS)
  {
     printf("%s, SQLSTATE=%s\n", buf, sqlstate);
  }

  while(SQLError(t->henv, SQL_NULL_HDBC, SQL_NULL_HSTMT, sqlstate, NULL,
     buf, sizeof(buf), NULL) == SQL_SUCCESS)
  {
     printf("%s, SQLSTATE=%s\n", buf, sqlstate);
  }
...
#endif
{
  ...
  if(isolation)
  {
    if(SQLSetConnectOption(thr->hdbc, SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_OFF) == SQL_ERROR)
       { print_error(thr); goto end; }
    if(SQLSetConnectOption(thr->hdbc, SQL_ATTR_TXN_ISOLATION, (SQLPOINTER)isolation) == SQL_ERROR)
       { print_error(thr); goto end; }
  }
  /* Make a loop */
  for(i = 0 ; i<loop ; i++)
  {
    ...
    {
      case SQL_ERROR: print_error(thr); goto end;
      default: while(SQLFetch(thr->hstmt) == SQL_SUCCESS);
    }
...
#endif
{
  ...
  for(i = 0 ; i<loop ; i++)
  {
    if(SQLAllocStmt(thr->hdbc, &thr->hstmt) == SQL_ERROR)
       { print_error(thr); goto end; }
    sprintf(szout, "INSERT INTO LOCKTEST(PK,SOME_CHAR,SOME_DATETIME) VALUES('%ld','CHAR:%ld',NULL)", thr->num*(loop+1) + i, thr->num*(loop+1) + i);
    ...
    if(isolation)
    {
      if(SQLTransact(SQL_HANDLE_DBC, thr->hdbc, SQL_COMMIT) == SQL_ERROR)
         { print_error(thr); goto end; }
    }
    if(SQLFreeStmt(thr->hstmt, SQL_CLOSE) == SQL_ERROR)
      { print_error(thr); goto end; }
...
int main(int argc, char **argv)
{
  ...
  if(argc >= 5)
  {
...
#endif
      /* Set some variables */
      array[i].done = 0; array[i].henv = henv;
      array[i].hdbc = NULL; array[i].hstmt = NULL;
      array[i].type = 0; array[i].num = i;
#ifdef WINDOWS
    ...
    for ( ; i<nbrea+nbupd ; i++)
    {
      ...
      array[i].done = 0; array[i].henv = henv;
      array[i].hdbc = NULL; array[i].hstmt = NULL;
      array[i].type = 1; array[i].num = i - nbrea;
#ifdef WINDOWS
...
#endif
        printf("Problem creating thread number : %d/%d\n", i+1, nbupd);
    }
