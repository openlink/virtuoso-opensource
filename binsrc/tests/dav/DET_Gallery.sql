...
create function "Gallery_DAV_AUTHENTICATE" (in id any, in what char(1), in req varchar, in auth_uname varchar, in auth_pwd varchar, in auth_uid integer)
{
  -- dbg_obj_princ ('Gallery_DAV_AUTHENTICATE (', id, what, req, auth_uname, auth_pwd, auth_uid, ')');
  ...
  pgid := coalesce (
    ( select G_ID from WS.WS.SYS_DAV_GROUP
      where G_NAME = 'Gallery_' || coalesce ((select COL_NAME from WS.WS.SYS_DAV_COL where COL_ID=id[1] and COL_DET='Gallery'), '')
      ), http_admin_gid() );
  pperms := '110100100RR';
...
create function "Gallery_DAV_AUTHENTICATE_HTTP" (in id any, in what char(1), in req varchar, in can_write_http integer, inout a_lines any, inout a_uname varchar, inout a_pwd varchar, inout a_uid integer, inout a_gid integer, inout _perms varchar) returns integer
{
  declare rc integer;
  declare puid, pgid integer;
...
create function "Gallery_DAV_GET_PARENT" (in id any, in st char(1), in path varchar) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_GET_PARENT (', id, st, path, ')');
  return -20;
}
;

create function "Gallery_DAV_COL_CREATE" (in detcol_id any, in path_parts any, in permissions varchar, in uid integer, in gid integer, in auth_uid integer) returns any
{
  declare ospath varchar;
  -- dbg_obj_princ ('Gallery_DAV_COL_CREATE (', detcol_id, path_parts, permissions, uid, gid, auth_uid, ')');
  ...
  return vector (UNAME'Gallery', detcol_id, ospath);
}
;

create function "Gallery_DAV_COL_MOUNT" (in detcol_id any, in path_parts any, in full_mount_path varchar, in mount_det varchar, in permissions varchar, in uid integer, in gid integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_COL_MOUNT (', detcol_id, path_parts, full_mount_path, mount_det, permissions, uid, gid, auth_uid, ')');
  return -20;
}
;

create function "Gallery_DAV_COL_MOUNT_HERE" (in parent_id any, in full_mount_path varchar, in permissions varchar, in uid integer, in gid integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_COL_MOUNT (', parent_id, full_mount_path, permissions, uid, gid, auth_uid, ')');
  return -20;
}
;

create function "Gallery_DAV_DELETE" (in detcol_id any, in path_parts any, in what char(1), in silent integer, in auth_uid integer) returns integer
{
  declare ospath varchar;
  -- dbg_obj_princ ('Gallery_DAV_DELETE (', detcol_id, path_parts, what, silent, auth_uid, ')');
  ospath := DAV_CONCAT_PATH ("Gallery_ID_TO_OSPATH" (detcol_id), path_parts);
...
create table "Gallery_DAV_RES_UPLOAD" (ID varchar primary key, DT datetime, CNT long varchar)
;

create function "Gallery_DAV_RES_UPLOAD" (in detcol_id any, in path_parts any, inout content any, in type varchar, in permissions varchar, in uid integer, in gid integer, in auth_uid integer) returns any
{
  declare ospath varchar;
  declare rc integer;
  ...
  if (__tag (content) = 126)
    {
      ...
      insert into "Gallery_DAV_RES_UPLOAD" values (p, now(), content);
-- in the next string, '1' is an invalid argument for string_to_file to guarantee 'Internal Server Error' visible by client
      string_to_file (ospath, coalesce ((select CNT from "Gallery_DAV_RES_UPLOAD" where ID=p), 1), -2);
    }
  else
...
create function "Gallery_DAV_PROP_REMOVE" (in id any, in what char(0), in propname varchar, in silent integer, in auth_uid integer) returns integer
{
  -- dbg_obj_princ ('Gallery_DAV_PROP_REMOVE (', id, what, propname, silent, auth_uid, ')');
  return -20;
}
;

create function "Gallery_DAV_PROP_SET" (in id any, in what char(0), in propname varchar, in propvalue any, in overwrite integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_PROP_SET (', id, what, propname, propvalue, overwrite, auth_uid, ')');
  if (propname[0] = 58)
    {
      return -16;
    }
...
create function "Gallery_DAV_PROP_GET" (in id any, in what char(0), in propname varchar, in auth_uid integer)
{
  declare ospath varchar;
  ospath := id[2];
  -- dbg_obj_princ ('Gallery_DAV_PROP_GET (', id, what, propname, auth_uid, ')');
...
create function "Gallery_DAV_PROP_LIST" (in id any, in what char(0), in propmask varchar, in auth_uid integer)
{
  declare ospath varchar;
  ospath := id[2];
  -- dbg_obj_princ ('Gallery_DAV_PROP_LIST (', id, what, propmask, auth_uid, ')');
  if (not isstring (file_stat (ospath)))
    {
      WS.WS.HOSTFS_COL_DISAPPEARS (ospath);
      WS.WS.HOSTFS_RES_DISAPPEARS (ospath);
...
create function "Gallery_ID_TO_OSPATH" (in col any)
{
  declare res varchar;
...
create function "Gallery_DAV_DIR_SINGLE" (in id any, in what char(0), in path any, in auth_uid integer) returns any
{
  declare fullname, name, tmp, mimetype, ft_mode varchar;
  declare cr_time, mod_time datetime;
  declare puid, pgid, flen, rc integer;
  -- dbg_obj_princ ('Gallery_DAV_DIR_SINGLE (', id, what, path, auth_uid, ')');
  fullname := id[2];
  ...
  if (path is null)
    path := "Gallery_DAV_SEARCH_PATH" (id, what);
  puid := http_dav_uid();
  pgid := coalesce (
    ( select G_ID from WS.WS.SYS_DAV_GROUP
      where G_NAME = 'Gallery_' || coalesce ((select COL_NAME from WS.WS.SYS_DAV_COL where COL_ID=id[1] and COL_DET='Gallery'), '')
...
create function "Gallery_DAV_DIR_LIST" (in detcol_id any, in path_parts any, in detcol_path varchar, in name_mask varchar, in recursive integer, in auth_uid integer) returns any
{
  ...
  -- dbg_obj_princ ('Gallery_DAV_DIR_LIST (', detcol_id, path_parts, detcol_path, name_mask, recursive, auth_uid, ')');
  ospath := DAV_CONCAT_PATH ("Gallery_ID_TO_OSPATH" (detcol_id), path_parts);
  ...
  pgid := coalesce (
    ( select G_ID from WS.WS.SYS_DAV_GROUP
      where G_NAME = 'Gallery_' || coalesce ((select COL_NAME from WS.WS.SYS_DAV_COL where COL_ID = detcol_id and COL_DET='Gallery'), '')
      ), puid+1);
  vectorbld_init (res);
  len := length (dirs);
  ctr := 0;
      ...
      if ((name <> '.') and (name <> '..'))
        {
          ...
          -- dbg_obj_princ ('Gallery_DAV_DIR_LIST makes ', fullname);
          rc := WS.WS.HOSTFS_PATH_STAT (fullname, flen, cr_time, mod_time);
          if (rc < 0)
            {
              WS.WS.HOSTFS_COL_DISAPPEARS (fullname);
          ...
          else
            {
              vectorbld_acc (res, vector (
                ...
                flen,
                mod_time,
                vector (UNAME'Gallery', detcol_id, fullname),
              ...
              if (recursive > 0)
                vectorbld_concat_acc (res,
                  "Gallery_DAV_DIR_LIST" (detcol_id,
  ...
  vectorbld_final (res);
  -- dbg_obj_princ ('Gallery_DAV_DIR_LIST returns ', res);
  return res;
...
create function "Gallery_DAV_DIR_FILTER" (in detcol_id any, in path_parts any, in detcol_path varchar, in compilation varchar, in recursive integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_DIR_FILTER (', detcol_id, path_parts, detcol_path, compilation, recursive, auth_uid, ')');
  return vector ();
}
;


create function "Gallery_DAV_SEARCH_ID" (in detcol_id any, in path_parts any, in what char(1)) returns any
{
  declare ospath, stat varchar;
  -- dbg_obj_princ ('Gallery_DAV_SEARCH_ID (', detcol_id, path_parts, what, ')');
  ospath := DAV_CONCAT_PATH ("Gallery_ID_TO_OSPATH" (detcol_id), path_parts);
...
create function "Gallery_DAV_SEARCH_PATH" (in id any, in what char(1)) returns any
{
  declare ospath varchar;
  declare slash_pos, detcol_fullpath integer;
  -- dbg_obj_princ ('Gallery_DAV_SEARCH_PATH (', id, what, ')');
  ospath := id[2];
  slash_pos := strchr (ospath, '/');
  detcol_fullpath := coalesce ((select WS.WS.COL_PATH (COL_ID) from WS.WS.SYS_DAV_COL where COL_ID = id[1] and COL_DET='Gallery'));
...
create function "Gallery_DAV_RES_UPLOAD_COPY" (in detcol_id any, in path_parts any, in source_id any, in what char(1), in overwrite integer, in permissions varchar, in uid integer, in gid integer, in auth_uid integer) returns any
{
  declare ospath varchar;
  -- dbg_obj_princ ('Gallery_DAV_RES_UPLOAD_COPY (', detcol_id, path_parts, source_id, what, overwrite, permissions, uid, gid, auth_uid, ')');
  ospath := DAV_CONCAT_PATH ("Gallery_ID_TO_OSPATH" (detcol_id), path_parts);
  if (what = 'R')
    {
      declare cnt any;
...
create function "Gallery_DAV_RES_UPLOAD_MOVE" (in detcol_id any, in path_parts any, in source_id any, in what char(1), in overwrite integer, in auth_uid integer) returns any
{
  declare ospath, src_path varchar;
  -- dbg_obj_princ ('Gallery_DAV_RES_UPLOAD_MOVE (', detcol_id, path_parts, source_id, what, overwrite, auth_uid, ')');
  ospath := DAV_CONCAT_PATH ("Gallery_ID_TO_OSPATH" (detcol_id), path_parts);
  if (what = 'R')
    {
      declare cnt any;
...
create function "Gallery_DAV_RES_CONTENT" (in id any, inout content any, out type varchar, in content_mode integer) returns integer
{
  -- dbg_obj_princ ('Gallery_DAV_RES_CONTENT (', id, ', [content], [type], ', content_mode, ')');
...
no_res:
  -- dbg_obj_princ ('Gallery_DAV_RES_CONTENT (', id, ', [content], [type], ', content_mode, ') catched an error ', __SQL_STATE, __SQL_MESSAGE);
  WS.WS.HOSTFS_RES_DISAPPEARS (id[2]);
  return -1;
}
;

create function "Gallery_DAV_SYMLINK" (in detcol_id any, in path_parts any, in source_id any, in what char(1), in overwrite integer, in uid integer, in gid integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_SYMLINK (', detcol_id, path_parts, source_id, overwrite, uid, gid, auth_uid, ')');
  return -20;
}
;

create function "Gallery_DAV_LOCK" (in path any, in id any, in type char(1), inout locktype varchar, inout scope varchar, in token varchar, inout owner_name varchar, inout owned_tokens varchar, in depth varchar, in timeout_sec integer, in auth_uid integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_LOCK (', path, id, type, locktype, scope, token, owner_name, owned_tokens, depth, timeout_sec, auth_uid, ')');
  return -20;
}
;

create function "Gallery_DAV_UNLOCK" (in id any, in type char(1), in token varchar, in auth_uid integer)
{
  -- dbg_obj_princ ('Gallery_DAV_UNLOCK (', id, type, token, auth_uid, ')');
  return -27;
}
;

create function "Gallery_DAV_IS_LOCKED" (inout id any, inout type char(1), in owned_tokens varchar) returns integer
{
  declare rc integer;
  declare orig_id any;
  declare orig_type char(1);
  -- dbg_obj_princ ('Gallery_DAV_IS_LOCKED (', id, type, owned_tokens, ')');
  orig_id := id;
  id := orig_id[1];
  orig_type := type;
  type := 'C';
...
create function "Gallery_DAV_LIST_LOCKS" (in id any, in type char(1), in recursive integer) returns any
{
  -- dbg_obj_princ ('Gallery_DAV_LIST_LOCKS" (', id, type, recursive);
  return vector ();
}
;

create procedure "Gallery_CF_LIST_PROP_DISTVALS" (in detcol_id integer, in cfc_id integer, in rfc_spath varchar, inout rfc_list_cond any, in schema_uri varchar, inout filter_data any, inout distval_dict any, in auth_uid integer)
{
...
--      exists (select top 1 1 from WS.WS.SYS_DAV_RES where RES_ID = HRI_RES_ID and case (DAV_CHECK_PERM (RES_PERMS, '1__', auth_uid, auth_gid, RES_GROUP, RES_OWNER)) when 0 then WS.WS.ACL_IS_GRANTED (RES_ACL, auth_uid, acl_bits) else 1 end)
  ...
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS (', detcol_id, cfc_id, rfc_spath, rfc_list_cond, schema_uri, filter_data, auth_uid, ')');
  topcol_name := coalesce ((select COL_NAME from WS.WS.SYS_DAV_COL where COL_ID = detcol_id and COL_DET = 'Gallery'));
  if (topcol_name is null)
    {
      -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: can not find the specified mount point', detcol_id);
      return;
    }
  ...
  if (filter_length = 2) -- distinct propvals with no filtering in front -- a special case
    {
      ...
      -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: distinct propvals of ', plast_id, ' in ', cfc_id);
      open c_last1 (prefetch 1);
      while (1)
        {
          fetch c_last1 into v_last;
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: v_last is ', v_last, ' v_max is ', v_max);
      ...
      while (1)
        {
          ...
          res_id_max := res0_id;
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: put hit ', res_id_max);
  ...
  if (filter_length = 14) -- distinct propvals with 3 fixed property
    {
      ...
      while (1)
        {
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: res_id_max is ', res_id_max);
--        res_id_max := 0;
          while (res0_id <= res_id_max) fetch c0 into res0_id;
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: res0_id is ', res0_id);
          if (res0_id > res_id_max) res_id_max := res0_id;
          while (res1_id < res_id_max) fetch c1 into res1_id;
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: res1_id is ', res1_id);
          if (res1_id > res_id_max) res_id_max := res1_id;
          while (res2_id < res_id_max) fetch c2 into res2_id;
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: res2_id is ', res2_id);
...
get_distincts_4:
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: close c4');
  close c4;
get_distincts_3:
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: close c3');
  close c3;
get_distincts_2:
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: close c2');
  close c2;
get_distincts_1:
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: close c1');
  close c1;
get_distincts_0:
  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: close c0');
  close c0;

  -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: now search in all values of ', plast_id);
  ...
  while (1)
    {
      ...
      if (v_max is null or (v_last > v_max))
        {
          -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: next value ', v_last, ' at ', res_last_id);
          if (dict_get (hit_ids, res_last_id, 0))
            {
              -- dbg_obj_princ ('Gallery_CF_LIST_PROP_DISTVALS: full hit at ', res_last_id);
              v_max := v_last; -- note that vectorbld_acc() will destroy the value of v_last so this assignment should be before vectorbld_acc().
              dict_put (distval_dict, v_last, 1);
            }
        }
...
create function "Gallery_CF_GET_RDF_HITS" (in detcol_id integer, in cfc_id integer, in rfc_spath varchar, inout rfc_list_cond any, in schema_uri varchar, inout filter_data any, in detcol_path varchar, in make_diritems integer, in auth_uid integer) returns any
{
  declare topcol_name varchar;
...
--    exists (select top 1 1 from WS.WS.SYS_DAV_RES where RES_ID = HRI_RES_ID and case (DAV_CHECK_PERM (RES_PERMS, '1__', auth_uid, auth_gid, RES_GROUP, RES_OWNER)) when 0 then WS.WS.ACL_IS_GRANTED (RES_ACL, auth_uid, acl_bits) else 1 end)
  ...
  declare c3 cursor for select HRI_RES_ID from WS.WS.HOSTFS_RDF_INVERSE where HRI_TOPCOL_ID = topcol_id and HRI_PROP_CATID = p3_id and HRI_CATVALUE = p3_val and HRI_RES_ID >= res_id_max;
  declare c4 cursor for select HRI_RES_ID from WS.WS.HOSTFS_RDF_INVERSE where HRI_TOPCOL_ID = topcol_id and HRI_PROP_CATID = p4_id and HRI_CATVALUE = p4_val and HRI_RES_ID >= res_id_max;
  -- dbg_obj_princ ('Gallery_CF_GET_RDF_HITS (', detcol_id, cfc_id, rfc_spath, rfc_list_cond, schema_uri, filter_data, make_diritems, auth_uid, ')');
  topcol_name := coalesce ((select COL_NAME from WS.WS.SYS_DAV_COL where COL_ID = detcol_id and COL_DET = 'Gallery'));
  if (topcol_name is null)
    {
      -- dbg_obj_princ ('Gallery_CF_GET_RDF_HITS: can not find the specified mount point', detcol_id);
      return vector ();
    }
  topcol_id := coalesce ((select COL_ID from WS.WS.HOSTFS_COL where COL_PARENT_ID is null and COL_NAME = topcol_name), WS.WS.HOSTFS_FIND_COL (topcol_name));
  filter_length := length (filter_data);
      ...
      while (1)
        {
          while (res0_id <= res_id_max)
            fetch c0 into res0_id;
          res_id_max := res0_id;
          -- dbg_obj_princ ('Gallery_CF_GET_RDF_HITS: put hit ', res_id_max);
      ...
      while (1)
        {
          ...
          if ((res0_id = res_id_max) and (res1_id = res_id_max) and (res2_id = res_id_max))
            {
              -- dbg_obj_princ ('Gallery_CF_GET_RDF_HITS_RES_IDS: put hit ', res_id_max);
              vectorbld_acc (acc, res0_id);
              res_id_max := res_id_max + 1;
            }
        }
...
finalize:
  ...
  while (acc_ctr < acc_len)
    {
      ...
      full_id := vector (UNAME'Gallery', detcol_id, fullname);
      if (make_diritems = 1)
        {
          diritm := "Gallery_DAV_DIR_SINGLE" (full_id, 'R', '(fake path)', auth_uid);
  ...
  -- dbg_obj_princ ('Gallery_CF_GET_RDF_HITS (', detcol_id, cfc_id, rfc_spath, rfc_list_cond, schema_uri, filter_data, make_diritems, auth_uid, ') returns ', acc);
  return acc;
}
;


create procedure "Gallery_RF_ID2SUFFIX" (in id any, in what char(1))
{
  if (what='C')
    {
  ...
  signal ('OBLOM', 'Invalid arguments for Gallery_RF_ID2SUFFIX');
}
;
...
create procedure "HostFile_RF_SUFFIX2ID" (in suffix varchar, in what char(1))
{
  ...
  select vector (UNAME'Gallery', detcol_id, COL_FULL_PATH || RES_NAME) into r_id
...
create procedure "HostDir_RF_SUFFIX2ID" (in suffix varchar, in what char(1))
{
  ...
  detcol_id := cast (subseq (suffix, pairs[2], pairs[3]) as integer);
  whenever not found goto oblom;
  select vector (UNAME'Gallery', detcol_id, COL_FULL_PATH) into c_id
    from WS.WS.HOSTFS_RES join WS.WS.HOSTFS_COL on (RES_COL = COL_ID);
  return c_id;
oblom:
  return NULL;
