...
char *kubl_default_conv_types[(2*(MAX_CONVERSIONS+1))+16] =
 {
/*         SRC TYPE  ->   KUBL TYPE */
/*  0 */   "DUMMY",       "RUMMY",   /* Pair 0-1, not used. */
/*  2 */   "TIME",        "DATE",
/*  4 */   "BIT",         "SMALLINT",
/*  6 */   "BIT VARYING", "INTEGER", /* Really? */
/*  8 */   "BINARY",      "CHAR",
/* 10 */   "VARBINARY",   "VARCHAR",
/* 12 */   "BIGINT",      "CHAR",
/* 14 */   "TINYINT",     "SMALLINT", /* SEE THE NOTE BELOW ABOUT conv_types_table ! */
/* 16 */   NULL,          NULL /* The rest should be NULL's */
 };
...
char *chemalloc(int size, char *where) /* where can be NULL or empty */
{
   FILE *error_stream;
   int i=0;
   char *result;
   ...
   if(NO (result = (char *) malloc(size)))
    {
      ...
      sprintf(num,"%d",size);
print_again:
      if(0 == i) { error_stream = stderr; }
      else if(1 == i) { error_stream = stdout; }
      else { exit(1); }
      ...
      if(where && *where) /* Not null, not empty. */
       {
         fputs(" (",error_stream); fputs(where,error_stream);
         fputs(")",error_stream);
       }
      fputs(": chemalloc failed when tried to allocate ",
                  stderr);
      fflush(error_stream);
...
char *chestrdup(char *str, char *where) /* where can be NULL or empty */
{
    FILE *error_stream;
    int i=0;
    char *res;
...
/* Don't use fprintf if memory is on the red! */
       ...
       fputs(": chestrdup failed when tried to make a copy of: ",
                  stderr);
       fflush(error_stream);
...
unsigned char *strncasestr(unsigned char *string1, unsigned char * string2,
                           int maxbytes)
{
   ...
   if(!str2len) { return(string1); } /* If string2 is an empty string "" */

   first = iso_to_lower(*string2);

   for(maxbytes-=(str2len-1); (maxbytes>0) && (d = *string1); maxbytes--)
    {
      if(is_a_uc_letter(d)) { d = iso_to_lower(d); }
...
/* e have to be fetched and checked before d in and-clause, otherwise
   we won't find substrings from the end of string1: */
         for(s1=string1,s2=string2;((e = *++s2) && (d = *++s1));)
          {
            if(is_a_uc_letter(d)) { d = iso_to_lower(d); }
            if(is_a_uc_letter(e)) { e = iso_to_lower(e); }
            if(d != e) { break; } /* Found first differing character. */
          }
...
/*
   ...
   simply deleted.
 */
unsigned char *str_replace(unsigned char *src_str, unsigned char *from_str,
                           unsigned char *to_str, int only_n)
{
  char *me = "str_replace";
...
/* Don't know actually what we should do when from is an empty string.
  ...
  if(0 == from_len) { return(UCP(chestrdup(SCP(src_str),me))); }
  ...
  if(0 != difference) /* The resulting string would be longer or shorter. */
   {
     ...
     occurrences = 0;
     ptr = src_str;
     while((!only_n_flag || (occurrences < only_n))
             && (ptr = strncasestr(ptr,from_str,(src_len-(ptr-src_str)))))
      {
        occurrences++;
        ptr++;
      }
     ...
     if(0 == occurrences)
      { return(UCP(chestrdup(SCP(src_str),me))); } /* Nothing to replace. */
   }
 ...
 */
  ...
  prevptr = src_str;
  n_changes = 0;
  while((!only_n_flag || (n_changes < only_n))
        && (ptr = strncasestr(prevptr,from_str,(src_len-(prevptr-src_str)))))
   {
...
UCHAR *substitute_tablename_expr(UCHAR *tablename_expr,
                                 UCHAR *whole_tablename_org);

char *escapify(char *resbuf,char *name)
{
    char *retptr = resbuf;

    if(!prefix_with_flag && !surround_with_flag) { return(name); }
 ...
 */
char *escapify_and_form_dest_name(char *resbuf,int resbuf_maxsize,
                                  char *whole_table_name)
{
    char *resptr=resbuf;
    ...
    if(resbuf_maxsize >= 0) { *resbuf = '\0'; }
    if(prefix_with_flag)
     {
       if(resbuf_maxsize >= 1) { *resptr++ = '\\'; }
     }
    if(new_name)
     {
       whole_table_name = SCP(substitute_tablename_expr(UCP(new_name),
                                                 UCP(whole_table_name)));
     }

    strncpy(resptr,whole_table_name,(resbuf_maxsize-(resptr-resbuf)));
    *(resptr+(resbuf_maxsize-(resptr-resbuf))) = '\0';
    if(new_name) { free(whole_table_name); } /* malloced by subst... */
    return(resbuf);
}



char *get_converted_type(char *type)
{
    int i;
    ...
    for(i=(2*n_conv_types); i; i -= 2)
     {
       ...
       if(conv_types_table[i] && conv_types_table[i+1] &&
           !strcasecmp(conv_types_table[i],type))
...
int length_of_tablelist(struct tabledeflist *tl)
{
    int len = 0;
...
struct tabledeflist *find_tabledef(char *table_name,
                                   struct tabledeflist *tl)
{
    while(tl)
     {
       if(0 == strcmp(table_name,tl->name)) { return(tl); }
       tl = tl->next;
     }
...
struct tabledeflist *get_ancestor(struct tabledeflist *tl)
{
    if(tl->super_tl) { return(get_ancestor(tl->super_tl)); }
    else { return(tl); }
}


int index_is_found_from_indexdeflist(char *index_name,
                                     struct indexdeflist *il)
{
    while(il)
     {
       if(0 == strcmp(index_name,il->index_name)) { return(1); }
       il = il->next;
     }
...
struct indexdeflist *find_first_index_like(struct indexdeflist *il,
                                           char *like_what)
{
    while(il)
     {
 ...
 */
int index_has_been_defined_for_supertable(char *index_name,
                                          struct tabledeflist *tl)
{
    struct tabledeflist *suptl;

    if(tl->super_table == NULL) { return(0); }
    else
     {
       suptl = tl->super_tl;
...
"-- Can't find the super table '%s' from the table definitions.\n",
       ...
       if(suptl->primary_key_name &&
           !strcmp(index_name,suptl->primary_key_name))
        { return(1); }
 ...
 */
int add_to_subtables(struct tabledeflist *subtl, struct tabledeflist *tl)
{
    struct subtablelist *sl,*newnode;
    int count=1;
    char where[CHARCOL_LEN+80];

    sprintf(where,"add_to_subtables(\"%s\",\"%s\")",
             (subtl->name ? subtl->name : "NULL"),
             (tl->name ? tl->name : "NULL"));

    /* First create a new node with a name subname. */
    newnode = ((struct subtablelist *)
                            chemalloc(sizeof(struct subtablelist),where));
    newnode->next = NULL;
...
int reorder_subtables(struct tabledeflist *tl)
{
...
int construct_where_clause_aux(char *resbuf,
                               char *father,
                               char *child,
                               char *primcols,
                               char *termstr, /* A right parenthesis */
                               int maxlength)
{
    int tmplen;
    ...
    while(oldptr) /* Loop over the comma-separated column names of pk */
     {
       ...
       if(ptr) { *ptr = '\0'; } /* Overwrite temporarily with zero. */
       sprintf(tmpbuf,"%s.%s = %s.%s%s",
                      escapify(temp1,child),oldptr,
                      escapify(temp2,father),oldptr,
                      (ptr ? " and " : termstr));
       if(ptr) { *ptr = ','; oldptr = ptr+1; } /* Restore the comma. */
       else { oldptr = NULL; } /* Will stop at the next time on loop top */
       tmplen = strlen(tmpbuf);
       if(tmplen > maxlength) { return(0); }
       else
        {
          maxlength -= tmplen;
...
int   construct_where_clause_for_a_subtable(char *resbuf,
                                            struct tabledeflist *father,
                                            struct tabledeflist *child,
                                            char *primcols,
                                            int maxlength)
{
    int tmplen;
    ...
    sprintf(tmpbuf," NOT EXISTS (select %s from %s where ",
                     primcols,escapify(temp,child->name));
    tmplen = strlen(tmpbuf);
    if(tmplen > maxlength) { return(0); }
    else
     {
    ...
    tmplen = construct_where_clause_aux(endptr,father->name,child->name,
                                        primcols,")",maxlength);
    if(0 == tmplen) { return(0); }
    maxlength -= tmplen;
    endptr += tmplen;
...
int make_where_clause_for_table_with_children(char *text,
                                                struct tabledeflist *tl,
                                                int maxlength)
{
    ...
    while(sl)
     {
       tmplen =
        construct_where_clause_for_a_subtable(endptr,tl,sl->sub_table,
                                              primcols,maxlength);
       if(0 == tmplen) { return(0); }
       maxlength -= tmplen;
       endptr += tmplen;
...
void print_error (HENV e1, HDBC e2, HSTMT e3)
{
  short len;
  SQLError (e1, e2, e3, (UCHAR *) SQL_error_state, NULL,
        (UCHAR *) & SQL_error_message, sizeof (SQL_error_message),
        (SWORD *) & len);
  printf ("\n-- *** Error %s: %s\n", SQL_error_state, SQL_error_message);
  if (0 == strcmp (SQL_error_state, "08S01")) exit (2);
}
 ...
 */
char *get_sql_type_title(int type)
{
    switch(type)
     {
#ifdef SQL_CHAR
       case SQL_CHAR:        { return("CHAR"); }     /*  1 */
#endif
#ifdef SQL_NUMERIC
       case SQL_NUMERIC:     { return("NUMERIC"); }  /*  2 */
#endif
#ifdef SQL_DECIMAL
       case SQL_DECIMAL:     { return("DECIMAL"); }  /*  3 */
#endif
#ifdef SQL_INTEGER
       case SQL_INTEGER:     { return("INTEGER"); }  /*  4 */
#endif
#ifdef SQL_SMALLINT
       case SQL_SMALLINT:    { return("SMALLINT"); } /*  5 */
#endif
#ifdef SQL_FLOAT
       case SQL_FLOAT:       { return("FLOAT"); }    /*  6 */
#endif
#ifdef SQL_REAL
       case SQL_REAL:        { return("REAL"); }     /*  7 */
#endif
#ifdef SQL_DOUBLE
       case SQL_DOUBLE:      { return("DOUBLE PRECISION"); }   /*  8 */
#endif
#ifdef SQL_DATETIME
       case SQL_DATETIME:    { return("DATE"); } /*  9 */
#endif
#if defined  (SQL_DATE) && NOT defined (SQL_DATETIME)
       case SQL_DATE:        { return("DATE"); } /* 9 in sqlext.h */
#endif
#ifdef SQL_TIME
       case SQL_TIME:        { return("TIME"); } /* 10 in sqlext.h */
#endif
#if defined (SQL_INTERVAL) && ((NOT defined (SQL_TIME)) || (SQL_INTERVAL != SQL_TIME))
       case SQL_INTERVAL:    { return("TIME"); } /* 10 */
#endif
#ifdef SQL_TIMESTAMP
       case SQL_TIMESTAMP:   { return("TIMESTAMP"); }   /* 11 */
#endif
#ifdef SQL_VARCHAR
       case SQL_VARCHAR:     { return("VARCHAR"); }  /* 12 */
#endif
#ifdef SQL_BIT
       case SQL_BIT:         { return("BIT"); }      /* 14 */
#endif
#ifdef SQL_BIT_VARYING
       case SQL_BIT_VARYING: { return("BIT VARYING"); } /* 15 */
#endif
#ifdef SQL_LONGVARCHAR
       case SQL_LONGVARCHAR: { return("LONG VARCHAR"); } /* (-1) */
#endif
#ifdef SQL_BINARY
       case SQL_BINARY:      { return("BINARY"); } /* (-2) in sqlext.h */
#endif
#ifdef SQL_VARBINARY
       case SQL_VARBINARY:   { return("VARBINARY"); } /* (-3) in sqlext.h */
#endif
#ifdef SQL_LONGVARBINARY
       case SQL_LONGVARBINARY: { return("LONG VARBINARY"); } /* (-4) */
#endif
#ifdef SQL_BIGINT
       case SQL_BIGINT:      { return("BIGINT"); } /* (-5) in sqlext.h */
#endif
#ifdef SQL_TINYINT
       case SQL_TINYINT:     { return("TINYINT"); } /* (-6) in sqlext.h */
#endif
...
/* Makes a new, safe copy of the stuff it returns as a result,
   if it is necessary. */
char *get_sql_col_type_def(int type,unsigned long int precision,
                           int scale,int nullable)
{
    char result[181],more[181];
    char *type_string;
    if ( kubl_default_conversions_flag
	 && (SQL_DECIMAL == type || SQL_NUMERIC == type)) {
      if (scale == 0)
	type = SQL_INTEGER;
      else
	type = SQL_DOUBLE;
...
UCHAR *divide_tablename_aux(UCHAR *tablename,SWORD *cbTableName,
                            UCHAR **p_tablequalifier,SWORD *cbTableQualifier,
                            UCHAR **p_tableowner,SWORD *cbTableOwner)
{
  ...
  if(tablename && (ptr = UCP(strchr(SCP(tablename),'.'))))
   {
     ...
     if(cbTableQualifier) { *cbTableQualifier = piece1len; }
     else { *ptr = '\0'; }

     if(ptr2 = (UCHAR*) strchr( (char*) (ptr+1),'.'))
      {
        ...
        if(cbTableOwner) { *cbTableOwner = SWD(ptr2-(ptr+1)); }
        else { *ptr2 = '\0'; }

        if(*(ptr2+1)) { tablename = (ptr2+1); } /* Something following? */
        else { tablename = NULL; } /* If nothing after second period. */
      }
     else /* If only one period found. */
      {
        *p_tableowner = *p_tablequalifier;
        *p_tablequalifier = NULL;
        if(cbTableOwner) { *cbTableOwner = piece1len; }
        if(cbTableQualifier) { *cbTableQualifier = 0; }
        /* Check that it's not dangling in the end: */
        if(*(ptr+1)) { tablename = ptr+1; }
        else { tablename = NULL; }
      }
   }
   ...
   {
     if(cbTableQualifier) { *cbTableQualifier = 0; }
     if(cbTableOwner) { *cbTableOwner = 0; }
   }
 ...
 */
UCHAR *divide_tablename(UCHAR *tablename,
                        UCHAR **p_tablequalifier,
                        UCHAR **p_tableowner)
{
/* Has any effect only if tablequalifier and tableowner are not given
   elsewhere, i.e. they are still NULL. (Stupid rule for compatibility) */
    if(tablename && NO *p_tablequalifier && NO *p_tableowner)
     {
       return(divide_tablename_aux(tablename,NULL,p_tablequalifier,NULL,
                                                  p_tableowner,NULL));
     }
...
UCHAR *repair_tablename(UCHAR *tablequalifier,
                        UCHAR *tableowner,
                        UCHAR *tablename)
{
    if(tablequalifier)
...
#define TN_SQL_QUOTE \
  my_strncat(dest_buf, quote_char, (dest_size-2))




char *compose_whole_tablename(struct tabledeflist *tl,char *dest_buf,
                              int dest_size)
{
    static int SQLGetInfo_called = 0;
    static UDWORD owner_usage, qualifier_usage;
    static UDWORD dummy1; /* Some extra-space before and ... */
    static UWORD  qualifier_location;
    static UDWORD dummy2; /* ... after qualifier_location, for pissy drivers. */
    ...
    if(0 == SQLGetInfo_called)
     {
       ...
       if (' ' == quote_char [0])
	 quote_char [0] =0;
       SQLGetInfo(*ptr_to_hdbc, SQL_OWNER_TERM, sp_owner_term,
                  (sizeof (sp_owner_term)-1), ((SWORD *)&cb_owner_term));
       if(SQL_NULL_DATA == cb_owner_term) { owner_term = NULL; }
       else { owner_term = sp_owner_term; }

       SQLGetInfo(*ptr_to_hdbc, SQL_QUALIFIER_TERM, sp_qualifier_term,
                  (sizeof (sp_qualifier_term)-1), ((SWORD *)&cb_qualifier_term));
       if(SQL_NULL_DATA == cb_qualifier_term) { qualifier_term = NULL; }
       else { qualifier_term = sp_qualifier_term; }
       ...
       if(SQL_NULL_DATA == cb_qualifier_name_separator) { qualifier_name_separator = NULL; }
       else { qualifier_name_separator = sp_qualifier_name_separator; }

       owner_usage = qualifier_usage = 0;
       qualifier_location = 0;

       SQLGetInfo (*ptr_to_hdbc, SQL_OWNER_USAGE, &owner_usage,
                   ((SWORD)4), ((SWORD *)&cb_owner_usage));

       SQLGetInfo (*ptr_to_hdbc, SQL_QUALIFIER_USAGE, &qualifier_usage,
                   ((SWORD)4), ((SWORD *)&cb_qualifier_usage));

       SQLGetInfo (*ptr_to_hdbc, SQL_QUALIFIER_LOCATION, &qualifier_location,
                   ((SWORD)2), ((SWORD *)&cb_qualifier_location));
...
"-- tl->tl_qual_only=\"%s\", tl->tl_own_only=\"%s\", tl->tl_name_only=\"%s\"\n",
             ...
             ((NULL == tl->tl_own_only)  ? "NULL" : tl->tl_own_only),
             ((NULL == tl->tl_name_only) ? "NULL" : tl->tl_name_only)
                 );
          fflush(stdout);
        }
 ...
 */
    ...
    if(tl->tl_name_only)
      {
	TN_SQL_QUOTE;my_strncat(dest_buf,tl->tl_name_only,(dest_size-2));
	TN_SQL_QUOTE;
      }
 ...
 */
UCHAR *new_whole_tablename(UCHAR *tablequalifier,UCHAR *tableowner,
                           UCHAR *tablename, char *where)
{
...
char * dsn_to_owner_name (char * dsn)
{
  int inx;
  dsn = strdup (dsn);
                                 ...
                                 UCHAR *whole_tablename_org)
{
  char * dsn2;
    ...
    UCHAR *whole_tablename
            = UCP(chestrdup(SCP(whole_tablename_org),where));
    UCHAR *res1=NULL, *res2=NULL, *res3=NULL, *res4=NULL;

    rem_tablenam = divide_tablename(whole_tablename,
                                    &rem_tablequal,
                                    &rem_tableown);
    ...
    if(whole_tablename) { free(whole_tablename); }
    if(res1) { free(res1); }
    if(res2) { free(res2); }
    if(res3) { free(res3); }
 ...
 */
char *belongs_to(char *table_name,char *column_name,char *tablecol_names)
{
...
try_again:
    if((ptr = strstr(tablecol_names,table_et_column_name))
        && ((ptr == tablecol_names) /* In the beginning? Or the prev char: */
           || ((isspace(*(ptr-1)) || (',' == *(ptr-1)) || (';' == *(ptr-1)))))
        && (ptr2 = ptr + strlen(table_et_column_name)) /* to following */
        && (!*ptr2 || isspace(*ptr2) || (',' == *ptr2) || (';' == *ptr2)
                   || ('=' == *ptr2))) /* For future additions. */
     { return(ptr2); }
    else if(table_et_column_name = strchr(table_et_column_name,'.'))
     {
       table_et_column_name++; /* Skip the first part and the dot. */
       goto try_again;
     }
    else { return(NULL); } /* No luck this time. */
}
...
struct tabledeflist *new_tabledeflist_node(struct tabledeflist *next,
                                           char *name,
                                           char *tl_qual_only,
                                           char *tl_own_only,
                                           char *tl_name_only,
                                           ...
                                           struct subtablelist *sub_tables)
{
     ...
     tl = (struct tabledeflist *)
              chemalloc(sizeof(struct tabledeflist),where);
...
struct tabledeflist *get_all_tables(char *global_tablequalifier,
                                    char *global_tableowner,
                                    char *tablepatterns,
                                    char *tabletype,
                                    int *pcount)
{
  ...
  for(tabletok = strtok_r(tablepatterns,tokseps,&toksave); /* The 1st token. */
      (tabletok != NULL);
      tabletok = strtok_r(NULL,tokseps,&toksave) /* The next token. */
     )
   {
creep_into_loop:
     if(verbose_flag || dbdump_debug_level)
      {
        fprintf(stdout,
"-- %s: tablepattern=\"%s\",%d\n",
             where,(SCP(tabletok) ? SCP(tabletok) : "NULL"),
                   ((NULL != tabletok) ? strlen(SCP(tabletok)) : 0));
      }
 ...
 */
     tablename = divide_tablename_aux(UCP(tabletok),&scbTableName,
                                      &tablequalifier,&scbTableQualifier,
                                      &tableowner,&scbTableOwner);
     tableowner = ((tableowner || tablequalifier)
                     ? tableowner : UCP(global_tableowner));
     tablequalifier = (tablequalifier
                     ? tablequalifier : UCP(global_tablequalifier));
...
"-- tablequalifier=%s,%d  tableowner=%s,%d  tablename=%s,%d  tabletype=%s\n",
          ...
          (SCP(tableowner) ? SCP(tableowner) : "NULL"),
              scbTableOwner,
          (SCP(tablename) ? SCP(tablename) : "NULL"),
              scbTableName,
          (SCP(tabletype) ? SCP(tabletype) : "NULL"));
      }
     ...
     rc = SQLTables(stmt,UCP(tablequalifier),scbTableQualifier,
                         ...
                         UCP(tablename),scbTableName,
                         UCP(tabletype),SWD(tabletype ? SQL_NTS : 0));
     if(rc == SQL_SUCCESS)
      {
...
"TableName=\"%s\",%ld TableType=\"%s\",%ld  patterntoken=\"%s\"\n",
            ...
            ((SQL_NULL_DATA == cbTableName) ? "Skipping, because: " : ""),
                          (SCP(szTableQualifier) ? SCP(szTableQualifier) : "NULL"),
                            cbTableQualifier,
                          (SCP(szTableOwner) ? SCP(szTableOwner) : "NULL"),
                            cbTableOwner,
                          (SCP(szTableName) ? SCP(szTableName) : "NULL"),
                            cbTableName,
                          (SCP(szTableType) ? SCP(szTableType) : "NULL"),
                            cbTableType,
                          (SCP(tabletok) ? SCP(tabletok) : "NULL"));
               }
           ...
           if(find_tabledef(SCP(wholetablename),orgtl))
            ...
            {
              new_node = new_tabledeflist_node
                       (NULL,  /* struct tabledeflist *next, */
                        SCP(wholetablename), /* char *name, */
                        ((SQL_NULL_DATA == cbTableQualifier) ? NULL
                          : SCP(chestrdup(SCP(szTableQualifier),where))),
                        ((SQL_NULL_DATA == cbTableOwner) ? NULL
                          : SCP(chestrdup(SCP(szTableOwner),where))),
                        ((SQL_NULL_DATA == cbTableName) ? NULL
                          : SCP(chestrdup(SCP(szTableName),where))),
                        "",    /* char *columns, */
 ...
 error: ;
     ...
     if(NULL == tabletok) { break; } /* In case tablepatterns was NULL */
...
int get_table_columns(struct tabledeflist *tl,
                      int n_cols_with_describe_col)
                         ...
                         SQLDescribeCol instead of SQLColumns */
{
    ...
    sprintf(where,"get_table_columns(\"%s\",%d)",
              (tl->name ? tl->name : "NULL"),
              n_cols_with_describe_col);
 ...
 */
       ...
       rc = SQLColumns(stmt, tablequalifier,scbTableQualifier,
                             tableowner,scbTableOwner,
                             tablename,scbTableName,
                             NULL, 0); /* All columns. */

       if(rc == SQL_SUCCESS)
        {
          SQLBindCol(stmt, 4,
                SQL_C_CHAR,  Column_Name, CHARCOL_LEN,&l_cbColumnName);
          SQLBindCol(stmt, 5, /* DATA TYPE is SMALLINT NOT NULL */
                SQL_C_SSHORT, &Type, 0, &cbType);
          SQLBindCol(stmt, 7,
                SQL_C_SLONG,  &Precision, 0, &cbPrecision); /* INTEGER */
          SQLBindCol(stmt, 9,
                SQL_C_SSHORT, &Scale, 0, &cbScale); /* SMALLINT */
          SQLBindCol(stmt, 11, /* SMALLINT NOT NULL */
                SQL_C_SSHORT, &Nullable, 0, &cbNullable);
        }
       else { IF_ERR_GO (stmt, error, rc); }
     }
    ...
    for(tmp1[0] = '\0', inx = 1; TRUE; inx++)
     {
       if(n_cols_with_describe_col)
        {
          if(inx > n_cols_with_describe_col) { break; }
       ...
       else /* Using SQLColumns with variables bound above. */
        {
          rc = SQLFetch (stmt);
          if(rc == SQL_NO_DATA_FOUND) { break; }
          if(cbScale == SQL_NULL_DATA) { Scale = 0; }
          if(cbPrecision == SQL_NULL_DATA) { Precision = 0; }
      ...
      (NOT n_cols_with_describe_col && (l_cbColumnName == SQL_NULL_DATA)))
       ...
       if(tl->col_defs == NULL) /* First column definition? */
        { /* Start constructing the column definition list. */
          cl = tl->col_defs
             = (struct coldeflist *) chemalloc(sizeof(struct coldeflist),
                                               where);
        }
       else
        { /* Update the next link of the prev node. */
          cl->next = (struct coldeflist *)
                         chemalloc(sizeof(struct coldeflist),where);
          cl = cl->next;
        }

       cl->col_name      = chestrdup(SCP(Column_Name),where);
       cl->col_type      = Type;
       cl->col_unquoted  = !!belongs_to(SCP(tl->name ? tl->name : "NULL"),
                                        SCP(Column_Name),
                                        SCP(unquoted_columns));
          ...
          if(tl->contains_blobs)
           { /* There is already at least one blob column in this table? */
             if(use_foreach_for_blobs_flag && verbose_flag)
              {
                printf(
"-- Table %s has more than one blob column.\n"
"-- The column %s of type %s might not get properly inserted.\n",
                       tl->name, cl->col_name,
                       get_sql_type_title(cl->col_type)
                      );
              }
    ...
    if(n_cols_with_describe_col) /* all the columns, including those of */
     { tl->all_columns = chestrdup(tmp1,where); } /* the possible supertables */
    tl->columns = chestrdup(tmp1,where);

    /* If we have used SQLDescribeCol, return here, don't free anything. */
    if(n_cols_with_describe_col) { return(n_cols_with_describe_col); }
error: ;
 ...
 */
int get_table_bestrow_columns(struct tabledeflist *tl,
                              UWORD scope_specified,
                              UWORD nullability)
{
    UCHAR *tablequalifier, *tableowner, *tablename;
    SWORD scbTableQualifier=SQL_NTS,scbTableOwner=SQL_NTS,scbTableName=SQL_NTS;
    ...
    sprintf(where,"get_table_bestrow_columns(\"%s\",%d,%d)",
                     (tl->name ? tl->name : "NULL"),
                     scope_specified,nullability);
    ...
    rc = SQLSpecialColumns(stmt,
                        ...
                        UCP(tableowner), scbTableOwner,
                        UCP(tablename), scbTableName,
                        ((UWORD) (scope_specified ? scope_specified
                                                  : SQL_SCOPE_CURROW)),
                        ((UWORD) nullability));
    ...
    if(rc == SQL_SUCCESS)
     {
       SQLBindCol(stmt, 1,
          SQL_C_SSHORT, &SCOPE, 0, &cbSCOPE);
       SQLBindCol(stmt, 2,
                    SQL_C_CHAR, Column_Name, CHARCOL_LEN,&cbColumnName);
       SQLBindCol(stmt, 8,
          SQL_C_SSHORT, &PSEUDO_COLUMN, 0, &cbPSEUDO_COLUMN);
 ...
 */
       ...
       debug_fprintf(stdout,
           ...
           (tl->name ? tl->name : "NULL"),
           scope_specified,nullability,
           (tl->bestrow_columns ? tl->bestrow_columns : "NULL"));
     } /* if(rc == SQL_SUCCESS) */
...
int get_table_primarykey(struct tabledeflist *tl)
{
    UCHAR *tablequalifier, *tableowner, *tablename;
    SWORD scbTableQualifier=SQL_NTS,scbTableOwner=SQL_NTS,scbTableName=SQL_NTS;
...
/* These are for Bind: */
    ...
    sprintf(where,"get_table_primarykey(\"%s\")",
                     (tl->name ? tl->name : "NULL"));
    ...
    if(rc == SQL_SUCCESS)
     {
       ...
       if(kubl_mode) /* The 7th column is a Non-Standard Kubl Extension */
        {
          SQLBindCol(stmt, 7,
             SQL_C_CHAR, SuperTabQual, CHARCOL_LEN, &cbSuperTabQual);
          SQLBindCol(stmt, 8,
             SQL_C_CHAR, SuperTabOwn,  CHARCOL_LEN, &cbSuperTabOwn);
          SQLBindCol(stmt, 9,
             SQL_C_CHAR, SuperTabName, CHARCOL_LEN, &cbSuperTabName);
        }
       ...
       while(TRUE)
        {
          ...
          if(kubl_mode && (cbSuperTabName != SQL_NULL_DATA) &&
                   (strcmp(SCP(TabName),SCP(SuperTabName)) || /* Some of these */
             ...
             if(tl->super_table == NULL)  /* Insert only once. */
              {
                if(use_table_prefixes_flag)
                 {
                   tl->super_table
                     = SCP(new_whole_tablename(SuperTabQual,
                                               SuperTabOwn,
                                               SuperTabName,where));
                 }
                else
           ...
           { /* column of it, that has to be concatenated to a string. */
             ...
             if((cbIndexName != SQL_NULL_DATA) &&
                (tl->primary_key_name == NULL)) /* Do this only once. */
              {
...
int get_real_supertables(struct tabledeflist *tl)
{
    UCHAR *tablequalifier, *tableowner, *tablename;
    SWORD scbTableQualifier=SQL_NTS,scbTableOwner=SQL_NTS,scbTableName=SQL_NTS;
...
/* We should use name_part here, get those three parts! */
 ...
 UCP("select v2.KEY_TABLE from SYS_KEYS v1, SYS_KEY_SUBKEY, SYS_KEYS v2 "
     ...
     "and v2.KEY_ID = SUPER and v1.KEY_TABLE <> v2.KEY_TABLE"),
                    SQL_NTS);
    IF_ERR_GO (grs_stmt, error, rc);

    rc = SQLSetParam (grs_stmt, 1, SQL_C_CHAR, SQL_CHAR, 0,0, TableName,
                      &cbTableName);
    IF_ERR_GO (grs_stmt, error, rc);

    rc = SQLBindCol  (grs_stmt, 1, SQL_C_CHAR, SuperTabNamePtr, CHARCOL_LEN,
                      &cbSuperTabName);
    IF_ERR_GO (grs_stmt, error, rc);
       ...
       if(rc == SQL_SUCCESS)
        {
          ...
          if(rc == SQL_NO_DATA_FOUND) { goto not_found; }
          IF_ERR_GO (grs_stmt, error, rc);
...
/* Was here: count++; */
          ...
          if(0 != strcmp(SCP(SuperTabName),SCP(tl->super_table)))
           { /* If changed, then free the old one and alloc new one. */
             if(tl->super_table) { free(tl->super_table); }
             tl->super_table = chestrdup(SCP(SuperTabName),where);
           }
        } /* if(rc == SQL_SUCCESS) */
       else
...
"-- to the output, or include its name explicitly in tablename argument.\n",
             ...
             supertl = new_tabledeflist_node
                      (tl->next,  /* struct tabledeflist *next, */
...
int get_table_indices(struct tabledeflist *tl)
{
    UCHAR *tablequalifier, *tableowner, *tablename;
    SWORD scbTableQualifier=SQL_NTS,scbTableOwner=SQL_NTS,scbTableName=SQL_NTS;
    ...
    sprintf(where,"get_table_indices(\"%s\")",
                     (tl->name ? tl->name : "NULL"));
    debug_fprintf(stdout,"-- %s\n",where);
    ...
    if(rc == SQL_SUCCESS)
     {
       ...
       SQLBindCol(stmt, 6,
          SQL_C_CHAR,   Index_Name, CHARCOL_LEN,&cbIndexName);
...
backwards:
          switch(Type)
           { /* Skip rows for table itself, because other cols are NULLs */
             case SQL_TABLE_STAT:      /* 0 */ { continue; }
             case SQL_INDEX_CLUSTERED: /* 1 */
              { my_strncat(indextype," CLUSTERED", CHARCOL_LEN); break; }
             case SQL_INDEX_HASHED:    /* 2 */
              { my_strncat(indextype," HASHED", CHARCOL_LEN); break; }
             case SQL_INDEX_OTHER:     /* 3 Normal, nothing special */
              { /* indextype = ""; */ break; } /* Do nothing. */
             default:
              {
                if(Type & SQL_INDEX_OBJECT_ID) /* 8 Kubl extension ? */
                 {
                   ...
                   if(Type) goto backwards; /* And go add rest. */
                ...
                sprintf(indextype+strlen(indextype),
                           " UNK-INDEX-TYPE(%d)",Type);
              }
           }
          ...
          if(strcmp(SCP(Index_Name),SCP(Previous_Index_Name)))
           { /* indname changed? */
             ...
             if(previous_one_was_primary_key)
              ...
              {
                if(il) { il->index_def = chestrdup(tmp1,where); }
          ...
          if((/* NOT kubl_mode && */ !strcmp(SCP(Index_Name),"PrimaryKey")))
           {
             ...
             if((tl->primary_key_type == NULL) ||
                empty_stringp(tl->primary_key_type))
              ...
              { /* indname changed? */
                if(tl->index_defs == NULL) /* First index definition? */
                 {
                   il = tl->index_defs
                      = (struct indexdeflist *)
                           chemalloc(sizeof(struct indexdeflist),where);
                ...
                sprintf(tmp1,"CREATE%s%s INDEX %s ON ",
                     (((cbNon_Unique != SQL_NULL_DATA) && NOT Non_Unique)
                            ? " UNIQUE" : ""),
                        indextype,
                        escapify(temp,SCP(Index_Name)));
                strcat(tmp1,
                        escapify_and_form_dest_name(temp,(sizeof(temp)-1),
                                                     SCP(tl->name)));
                strcat(tmp1,"(");
                strcat(tmp1,escapify(temp,SCP(Column_Name)));
...
/* We still have to save what we have collected to tmp1 on
   the last round of the loop, to the appropriate place */
       if(previous_one_was_primary_key)
        { tl->primary_key_def = chestrdup(tmp1,where); }
       else if(il && *tmp1) { il->index_def = chestrdup(tmp1,where); }
...
void print_datetime_col(char *timebinstr,int collen,int type)
{
    char temp[121];
    ...
    if(times_conform_to_odbc_flag)
	 ...
	 sprintf (temp, "%*s", collen, timebinstr);
        ...
        {
          sprintf(temp,
 "SHOULD NOT HAPPEN: print_date_col called(collen=%d,type=%d)",
                          collen,type);
        }
     }
    else /* Native KUBL mode. */
     {
       ...
       if(tm)
        ...
        {
          sprintf (temp,"INV-TIME(%d):%08lx:%08lx",
                     collen,tv.tv_sec,tv.tv_usec);
        }
     }
    ...
    switch (type)
      {
	case SQL_DATE:
	case SQL_TIMESTAMP: printf("stringdate('%s')",temp); break;
	case SQL_TIME: printf("stringtime('%s')",temp); break;
        default: printf ("%s", temp);
      }
    fflush(stdout);
}
 ...
 */
long print_string_col_aux(UCHAR *s, int length, int use_backslash_escapes,
     int is_foreach_blob, long chars_since_newline,
     char *end_token, char *blob_token)
{
    for(;length--; s++)
     {
       if('\n' == *s) { chars_since_newline = 0; }
       else { chars_since_newline++; }

       if((SQL_LONGVARCHAR == is_foreach_blob) &&
               maxlinelength_for_foreach &&
               (chars_since_newline >= maxlinelength_for_foreach))
        {
 ...
 */
       if((SQL_LONGVARBINARY == is_foreach_blob) &&
            (empty_stringp(s) || /* Either finished? Or... */
              (maxhexlength_for_foreach && /* time to print a newline? */
                (chars_since_newline >= maxhexlength_for_foreach))))
        {
          if(empty_stringp(s)) { break; }
          printf("\n");
          chars_since_newline = 0;
 ...
 */
int print_blob_col (UWORD n_col, struct coldeflist *cl, int really_do_it)
{
  ...
  int use_foreach_now =
      (use_foreach_for_blobs_flag && (0 != cl->col_is_nth_blob));
  char *token_to_use
        = ((really_do_it > 1) ? foreach_end_token : foreach_blob_token);
  ...
  if(NOT use_foreach_now)
   { putchar('\''); } /* The beginning single quote. */

  for (;;)
   {
     SDWORD n_recv;
     rc = SQLGetData(stmt, n_col, SQL_C_CHAR,
                         blob_buffer,
                         ((SDWORD) BLOB_BUFFER_SIZE),
                         & n_recv);
...
/* Tell calling function that everything is still all right, no panic: */
        if(SQL_NO_DATA_FOUND == rc) { rc = SQL_SUCCESS; }
        if(use_foreach_now) { printf("%s\n",token_to_use); }
        /* Otherwise, it is printed out as a string literal '': */
        else { fputs("'", stdout); } /* The ending quote. */
        fflush(stdout);
        return(rc);
      }
...
/* When inputting binary blobs as hexadecimal dump, this will get the
   real length hexdigit string to be printed: */
     ...
     if(got_n_bytes) { last_character = blob_buffer[got_n_bytes-1]; }
 ...
 */
     chars_since_newline = print_string_col_aux(UCP(blob_buffer),
          got_n_bytes,
          ((use_backslash_escapes_in_strings_flag || NOT(use_foreach_now))
             ? use_backslash_escapes_in_strings_flag : 1),
          (use_foreach_now ? type : 0), /* is foreach blob? */
          chars_since_newline,
          (use_foreach_now ? foreach_end_token : NULL),
          (use_foreach_now ? foreach_blob_token : NULL));
...
/* Print the last newline after hexadecimal dump, if it is necessary: */
         ...
         { putchar('\''); } /* The ending single quote. */
...
int print_table_row(struct tabledeflist *tl)
{
...
/* Was:
  ...
  printf("INSERT %s %s(%s)",
         tl->insert_mode,
         escapify_and_form_dest_name(temp,(sizeof(temp)-1),SCP(tl->name)),
         (tl->all_columns ? tl->all_columns : tl->columns));
...
/* We should print a warning here if the length tl->col_defs and n_out_cols
   don't match. */
  for (inx=0, cl = tl->col_defs;
       (inx < n_out_cols) && cl; inx ++)
   {
     int tp = cl->col_type;
     if(out_cols [inx].o_col_len == SQL_NULL_DATA)
      {
        fputs("NULL", stdout); fflush(stdout);
      }
     else
      {
       switch(tp)
        {
          ...
          case SQL_TIMESTAMP:
           {
             print_datetime_col(out_cols[inx].o_buffer,
                                out_cols[inx].o_col_len,tp);
             break;
           }
 ...
 */
                ...
                else if(SQL_SUCCESS != rc) { return(rc); }
              }
             else
              {
                ...
                print_string_col(UCP(temp),strlen(temp),
                                 use_backslash_escapes_in_strings_flag);
              }
             break;
           }
...
However, with Access, the length doesn't include the terminating zero. */
             if((SQL_VARCHAR == tp) && real_len &&
                  varchar_cols_returned_with_length_plus) /* With old Kubl */
              { real_len -= varchar_cols_returned_with_length_plus; }
#endif

             if(!cl->col_unquoted) { putchar('\''); }
             print_string_col(UCP(out_cols[inx].o_buffer),
                              real_len,
                              use_backslash_escapes_in_strings_flag);
             if(!cl->col_unquoted) { putchar('\''); }
             break;
     ...
     cl = cl->next;
     if(cl) { putc(',',stdout); } /* Not the last one? */
  ...
  if(n_blobs)
   {
     ...
     for(i=0; i < n_blobs; i++)
      {
        rc = print_blob_col(blob_colindexes_to_be_printed_out_later[i],
                            blob_columns_to_be_printed_out_later[i],
                            1+(i == (n_blobs-1))); /* 2 on last blob. */
        fflush(stdout);
        if((SQL_SUCCESS != rc)) { return(rc); } /* Brutal! */
      }
   }
...
/* The next two functions, escape_string and count_escaped_length
   ...
   to escape and how.
 */
unsigned char *escape_string(unsigned char *dest, unsigned char *src)
{
...
int do_generic_dest_statement(HSTMT *ptr_to_gen_stmt,
                              HDBC hdbc, /* Datadest (Kubl) connection */
                              HENV henv,
                              char *statement_text)
{
  /*RETCODE */ int rc; /* Return Code for SQL operations, signed short */
...
int link_generic_operation(HSTMT *ptr_to_link_stmt,
                            HDBC hdbc, /* Datadest (Kubl) connection */
                            HENV henv,
                            char *datasource,
                            char *second_arg,
                            char *third_arg,
                            char *output_template, /* With 7 %s 's */
                            char *statement_template) /* With 3 ?'s */
{
    static char *last_output_template = NULL;
    /* RETCODE*/ int rc; /* Return Code for SQL operations, signed short */
    SDWORD cb_datasource = SQL_NTS, cb_second_arg = SQL_NTS,
           cb_third_arg = SQL_NTS;
    ...
    printf(output_template,
            ...
            (second_arg ? "'" : ""),
            (third_arg ? "'" : ""),
            (third_arg ? SCP(escape_string(UCP(tmp2buf),UCP(third_arg))) : "NULL"),
            (third_arg ? "'" : ""));
    ...
    if(output_template != last_output_template) /* Statement changed? (or first time here) */
     {
       ...
       rc = SQLPrepare(*ptr_to_link_stmt,
                       UCP(statement_template),
                       SQL_NTS);
 ...
 */
       if(NULL == datasource) { cb_datasource = SQL_NULL_DATA; }
       rc = SQLBindParameter (*ptr_to_link_stmt,
                              ((UWORD)1), ((SWORD) SQL_PARAM_INPUT),
                              ((SWORD) SQL_C_CHAR), ((SWORD) SQL_CHAR),
                              ((UDWORD)0),((SWORD)0),
                              ((PTR) datasource),
                              ((SDWORD) 0), ((SDWORD *) &cb_datasource));
       IF_ERR_GO (*ptr_to_link_stmt, error, rc);

       if(NULL == second_arg) { cb_second_arg = SQL_NULL_DATA; }
       rc = SQLBindParameter (*ptr_to_link_stmt,
                              ((UWORD)2), ((SWORD) SQL_PARAM_INPUT),
                              ((SWORD) SQL_C_CHAR), ((SWORD) SQL_CHAR),
                              ((UDWORD)0),((SWORD)0),
                              ((PTR) second_arg),
                              ((SDWORD) 0), ((SDWORD *) &cb_second_arg));
       IF_ERR_GO (*ptr_to_link_stmt, error, rc);

       if(NULL == third_arg) { cb_third_arg = SQL_NULL_DATA; }
       rc = SQLBindParameter (*ptr_to_link_stmt,
                              ((UWORD)3), ((SWORD) SQL_PARAM_INPUT),
                              ((SWORD) SQL_C_CHAR), ((SWORD) SQL_CHAR),
                              ((UDWORD)0),((SWORD)0),
                              ((PTR) third_arg),
                              ((SDWORD) 0), ((SDWORD *) &cb_third_arg));
       IF_ERR_GO (*ptr_to_link_stmt, error, rc);
...
int link_remote_data_source(HSTMT *ptr_to_link_stmt,
                            HDBC hdbc, /* Datadest (Kubl) connection */
                            HENV henv,
                            char *datasource,
                            char *username,
...
int link_local_and_remote_table(HSTMT *ptr_to_link_stmt,
                                HDBC hdbc, /* Datadest (Kubl) connection */
                                HENV henv,
                                char *datasource,
                                char *local_name,
...
/* Note: the overflow checking of statement_text buffer is not yet
         really ready.
 */
int print_table_definition_banner(struct tabledeflist *tl)
{
  ...
  sprintf(where,"print_table_definition_banner(\"%s\")",
                (tl->name ? tl->name : "NULL"));

  table_destname=escapify_and_form_dest_name(tmp_for_destname,
                                             (sizeof(tmp_for_destname)-1),
                                             SCP(tl->name));
  ...
  if(vd_procedures_flag)
   {
     rc = link_local_and_remote_table(&datadest_link_stmt,
                                      ...
                                      henv,
                                      datasource,
                                (table_destname+('\\' == *table_destname)),
                                      NULL); /* Unlink the old one. */

/* No success, e.g. there exists a local table with the same name */
     if(SQL_SUCCESS != rc) { goto error; }
  ...
  if(0 == dont_issue_drop_commands_flag)
   {
     ...
     if(datadest_hdbc)
      {
        rc = do_generic_dest_statement(&datadest_gen_stmt,
                                       datadest_hdbc,
                                       henv,
                                       statement_text);
      }
  ...
  for(tmp1[0] = '\0', cl = tl->col_defs; cl; )
   {
     if(primary_key_by_all_means_flag
         ...
         && NO(tl->bestrow_columns)
         && (cl->col_type != SQL_LONGVARCHAR)
         && (cl->col_type != SQL_LONGVARBINARY)
       )
      {
        if(*tmp1) { strcat(tmp1,","); }
...
/* Then the datatype possibly with precision and NOT NULL definition. */
     strncpy(statement_ptr,
             get_sql_col_type_def(cl->col_type,cl->col_precision,
                                  cl->col_scale,cl->col_nullable),
             STMT_SPACE_REST
            );
     statement_ptr += strlen(statement_ptr);
     cl = cl->next;
/* Print a comma, in case this is not the last column, or even if it
   ...
   or a primary key definition for this table. */
     if(cl || tl->super_table
           || (tl->primary_key_def && *(tl->primary_key_def))
  ...
  if(tl->super_table)
   {
     ...
     statement_ptr += strlen(statement_ptr);
     strncpy(statement_ptr,
             escapify_and_form_dest_name(tmp,(sizeof(tmp)-1),
                                         SCP(tl->super_table)),
             STMT_SPACE_REST);
     statement_ptr += strlen(statement_ptr);
   }
 ...
 */
  if(tl->primary_key_def && *(tl->primary_key_def))
   ...
   {
     char *pk_cols = NULL;
     struct indexdeflist *first_unique_index
      = find_first_index_like(tl->index_defs,"CREATE UNIQUE ");
     if(first_unique_index
         && (NULL != (pk_cols = strchr(first_unique_index->index_def,'(')))
       )
      {
        pk_cols++; /* Past the opening parenthesis */
      }
     else if(tl->bestrow_columns) { pk_cols = tl->bestrow_columns; }
     else { pk_cols = tmp1; } /* Collected above, all non-blob columns */
  ...
  if(datadest_hdbc)
   {
     rc = do_generic_dest_statement(&datadest_gen_stmt,
                                    ...
                                    henv,
                                    statement_text);
     if(SQL_SUCCESS != rc) { goto error; } /* Create table failed? */
   }
  ...
  if(vd_procedures_flag)
   {
     ...
     rc = link_local_and_remote_table(&datadest_link_stmt,
                                ...
                                (table_destname+('\\' == *table_destname)),
                                      remote_name);
     if(SQL_SUCCESS != rc) { goto error; } /* Linking failed? */
   }

   if(nicer_output_flag) { putc('\n',stdout); }
   fflush(stdout);
   return(1);
error:
 ...
 */
int do_select_for_table(struct tabledeflist *tl, int for_contents_flag)
{
  int rc;
 ...
 */
  sprintf(tmp1, select_statement_template,
                (never_escape_tablenames_in_select
                    ? tl->name
                    : escapify(tmp,tl->name)));
     ...
     if(tl->sub_tables)
      {
        ...
        int enough_space =
             make_where_clause_for_table_with_children(tmp1,tl,
                                                       sizeof(tmp1)-10);
        if(NOT enough_space)
         {
           struct subtablelist *sl;
           fprintf(stdout,
 "-- do_select_for_table: Not enough space (only %d bytes) to hold:\n-- ",
                   sizeof(tmp1));
           puts(tmp1); /* This adds the newline ? Hope so. */
           fprintf(stdout,
 "-- Dumping the subtables with the REPLACING option instead.\n");
  ...
  if(rc == SQL_NO_DATA_FOUND) /* This shouldn't cause an error, but just */
   { goto error; } /* frees the handles and returns zero. */
  ...
  if(n_out_cols == 0)
   {
     fprintf(stdout,
        "-- Table %s has %d columns which is very strange indeed!\n",
        tl->name,n_out_cols);
     return(0);
   }
   ...
   {
     fprintf(stdout,
        "-- Table %s has %d columns > max. %d, truncated!\n",
...
int get_and_print_table_defition(struct tabledeflist *tl,int print_it_also)
{
  int n_cols;
  ...
  if(use_sql_describe_flag)
   {
     ...
     if(0 == n_cols) goto error;
     get_table_columns(tl,n_cols); /* Get them with SQLDescribeCol */
   }
  else { n_cols = get_table_columns(tl,0); } /* Get them with SQLColumns */

  if(print_it_also) { print_table_definition_banner(tl); }
 ...
 */
int print_table_contents(struct tabledeflist *tl)
{
  SWORD btype;
  UWORD inx;
  ...
  sprintf(where,"print_table_contents(\"%s\")",
              (tl->name ? tl->name : "NULL"));
  ...
  if(0 == use_sql_describe_flag) /* We still have to do select. */
   {
     ...
     if(0 == n_cols) goto error; /* Not really an error, just skip printing. */
...
/*   if(tl->super_table)   Always do it anyway */
  ...
  for(inx=1, cl = tl->col_defs; (inx <= n_out_cols) && cl;
                                inx ++, cl = cl->next)
   {
     ...
     if(NO times_to_strings_flag &&
          ((cl->col_type == SQL_TIMESTAMP) || (cl->col_type == SQL_DATE) || (cl->col_type == SQL_TIME))
       )
      { btype = SQL_C_TIMESTAMP; }
     else { btype = SQL_C_CHAR; }

     if(bind_blobs_flag
        || ((SQL_LONGVARCHAR!=cl->col_type)&&(SQL_LONGVARBINARY!=cl->col_type)))
      {
        rc = SQLBindCol(stmt,inx,btype,
                          out -> o_buffer,
                          ((SDWORD) maxcolumn_width),
                          &out -> o_col_len);
        IF_ERR_GO (stmt, error, rc);
      }
...
int print_indexdeflist(struct tabledeflist *tl)
{
    struct indexdeflist *il;
    int count=0;
 ...
 */
       if(NO only_table_definitions_flag &&
          NO dont_issue_drop_commands_flag)
...
int output_one_table(struct tabledeflist *this_table,int sublevel)
{
    int rows=0,indices=0,cols=0;
    struct subtablelist *sl;
    int save_only_table_definitions_flag;
    int save_only_index_definitions_flag;
    int save_only_contents_flag;
...
/* Print it only if this flag is on: */    only_table_definitions_flag);
    ...
    debug_fprintf(stdout,
             "-- Table %s %d columns, %d indices output, level %d.\n",
...
int output_tables_with(struct tabledeflist *tl,PFI_TABLEDEFLIST out_fun)
{
    for(; tl; tl = tl->next)
     {
       if(tl->super_table) { continue; } /* Skip subtables on top level */
       (out_fun)(tl,0);
     }
...
int output_tables(struct tabledeflist *tl)
{
...
int output_one_table_as_option(struct tabledeflist *this_table, int sublevel)
{
   int i;
   struct subtablelist *sl;
...
int output_tables_as_select_list(struct tabledeflist *tl, int list_size)
{
    int rc;

    printf("<SELECT NAME=\"S_TABLES\" MULTIPLE onchange=\"add_selected_items_to_the_text_elem(this,this.form.tablename)\" SIZE=%d>\n",
            list_size);
...
int output_tablelist_token(int list_size)
{
    if(flag_get_list_of_tables)
     ...
     {
       printf(
            "<INPUT TYPE=SUBMIT NAME=\"RTL\" VALUE=\"Show List\" "
...
char *next_delimiter(char *str)
{
    char *dp1,*dp2;

    dp1 = strchr(str,'&'); /* Stupid... */
    dp2 = strchr(str,'/');
    if(dp1 && (NO dp2 || (dp1 <= dp2))) { return(dp1); }
    if(dp2) { return(dp2); }
    return(NULL);
}
 ...
 */
char *append_to_specval(char *item, char *ptr2, char *where)
{
          if(*ptr2)
           {
             if(item) /* There are more than one type specifiers, */
              { /* and this is not the first one. */
                sprintf(tmp1,"%s,%s",SCP(item),
                                     SCP(parse_url_value(UCP(ptr2))));
...
char *parse_url_query_string(char *query_string) /* input */
{
    char *ptr1,*ptr2,*ptr3;
 ...
 */
       if(!strcasecmp(((char *)ptr1),"tablequalifier"))
        ...
        {
          if(*ptr2) tablename = SCP(append_to_specval(tablename,ptr2,where));
/* Was:   if(*ptr2) tablename = SCP(parse_url_value((UCHAR *)(ptr2))); */
        }
       else if(!strcasecmp(((char *)ptr1),"tabletype"))
        {
          if(*ptr2) tabletype = SCP(append_to_specval(tabletype,ptr2,where));
        }
        ...
        {
          if(*ptr2) unquoted_columns = SCP(append_to_specval(unquoted_columns,ptr2,where));
        }
       else if(!strcasecmp(((char *)ptr1),"username")
            || !strcasecmp(((char *)ptr1),"s_conuser"))
        {
          if(*ptr2)
       ...
       else if(!strcasecmp(((char *)ptr1),"password")
            || !strcasecmp(((char *)ptr1),"s_conpass"))
        {
          if(*ptr2)
       ...
       else if(!strcasecmp(((char *)ptr1),"datasource")
            || !strcasecmp(((char *)ptr1),"s_datasource"))
        {
          if(*ptr2)
             datasource = SCP(parse_url_value((UCHAR *)(ptr2)));
        }
       else if(!strcasecmp(((char *)ptr1),"datadest")
            || !strcasecmp(((char *)ptr1),"s_datadest"))
        {
          if(*ptr2)
   ...
   "%s-- Conversion option %s is invalid. N must be between 1 and %d!\n",
                     (parse_url_error_message ? parse_url_error_message
                                              : ""),
                     ptr1,MAX_CONVERSIONS);
             if(parse_url_error_message) { free(parse_url_error_message); }
             parse_url_error_message = chestrdup(tmp1,where);
           }
          else if(*ptr2)
           {
             conv_types_table[(2*n)+convdst] =
               SCP(parse_url_value(UCP(ptr2)));
             if(n > n_conv_types) { n_conv_types = n; } /* Keeps the max n encountered. */
           }
        }
       else if(!strcasecmp(((char *)ptr1),"init"))
        {
          if(*ptr2) init_statement = SCP(parse_url_value((UCHAR *)(ptr2)));
        }
       else if(!strcasecmp(((char *)ptr1),"RTL"))
        {
          if(*ptr2) { flag_get_list_of_tables = 1; }
        }
        ...
        { /* User wants to override the default select statement. */
          ...
          if(*ptr2)
            select_statement_template =
               SCP(parse_url_value((UCHAR *)(ptr2)));
        }
        ...
        {
          if(*ptr2)
            maxhexlength_for_foreach
              = atol(SCP(parse_url_value((UCHAR *)(ptr2))));
        }
          ...
          for(; *s; s++)
           {
             switch(*s) /* Stupid idea: tolower(*s) */
              {
                case 'D': { dbdump_debug_level++; break; }
                case 'B': { bind_blobs_flag = (1^oppo); break; }
                case 'A': { all_or_nothing_flag++; break; }
                case 'a':
                 {
                   never_escape_tablenames_in_select
                    = (surround_with_flag == 0);
                   surround_with_flag = '"';
                   break;
                 }
                case 'b':
 ...
 */
                ...
                case 'd': { use_sql_describe_flag = 1; break; }
                case 'K':
                 { kubl_default_conversions_flag = 1; break; }
                case 'P': { primary_key_by_all_means_flag++; break; }
                case 'U': { universal_transfer_flag++; break; }
                case 'V': { vd_procedures_flag++; break; }
                case 'N': { nicer_output_flag++; break; }
                case 'p':
                 {
                ...
                case 't': { only_table_definitions_flag = 1; break; }
                case 'i': { only_index_definitions_flag = 1; break; }
                case 'c': { only_contents_flag = 1; break; }
                case 'T': { auto_only_table_definitions_flag = 1; break; }
                case 'I': { auto_only_index_definitions_flag = 1; break; }
                case 'C': { auto_only_contents_flag = 1; break; }
                case 'q': { verbose_flag = 0; break; } /* Q as Quiet */
                case 'v': { verbose_flag = 1; break; } /* V as Verbose */
                case 'l': { list_data_sources_flag = 1; break; }
                case 'h': { dump_html_template_file_flag++; break; }
/* I.e. we can use -ne to force both use_singlequote_escape and
   ...
   See function print_string_col_aux. */
                case 'e':
                     { use_backslash_escapes_in_strings_flag++; break; }
                case 'n':
                     {
                ...
                case 'r': { dont_issue_drop_commands_flag = 1; break; }
/* -s: Keep all blobs as string literals, don't use foreach contraption: */
                case 's': { use_foreach_for_blobs_flag = 0; break; }
                default: { break; }
              }
           }
        }
...
int driver_has_necessary_sql_functions()
{
    ...
    if(0 == TablesExists)
     {
       fprintf(stderr,
        "-- Sorry, this driver doesn't implement SQLTables. Goodbye!\n");
       exit(1);
     }
    ...
    if(0 == ColumnsExists)
     {
       fprintf(stderr,
"-- Hmmm, this driver doesn't implement SQLColumns. Trying to use SQLDescribeCol instead.\n");
    ...
    if(0 == FunctionExists)
     {
       fprintf(stderr,
 "-- Haa! this driver doesn't implement SQLDescribeCol either. Goodbye!\n");
       exit(1);
     }
    ...
    if(0 == FunctionExists)
     {
       fprintf(stderr,
 "-- Sorry, this driver doesn't implement SQLStatistics. Can't get index information.\n");
...
int list_data_sources(int for_html, char *templatename, char *tagname)
{
    ...
    if(!strcmp(tagname,DATADEST_TOKEN)) { selected_datasource = ""; }
    ...
    if(for_html)
     {
       printf("<INPUT NAME=%s TYPE=TEXT VALUE=\"%s\">\n",
              (!strcmp(tagname,DATADEST_TOKEN) ? "S_DATADEST"
                                               : "S_DATASOURCE"),
              (selected_datasource ? selected_datasource : DEFAULT_DATASOURCE_IN_UNIX));
     }
    else
     {
       printf(
"Don't know how to list available data sources in Unix. Try %s or wherever"
...
/*    " onchange=\"select_option_with_value(this.form.OPCODE,'RTL');\n check_radiobutton_with_value(this.form.OPTION_qv,'q');\n this.form.submit();\""); */
      ...
      if(for_html)
        {
          printf("<OPTION%s VALUE=\"%s\">%s,   %s</OPTION>\n",
              ((selected_datasource && !strcmp(DSN,selected_datasource))
                       ? " SELECTED" : ""),
              DSN,DSN,DriverDescription);
        }
      else
        {
  ...
  if(for_html) { printf("</SELECT>\n"); }
  return(count);
error:
  if(for_html) { printf("</SELECT>\n"); }

  fprintf(stderr,
      "print_data_sources: SQLDataSources returned status %d, count=%d\n",
      rc,count);
  fflush(stderr);
...
{
    fprintf(stderr,
"DBDUMP - DataBase Dump, Version %s\n", DBDUMP_VERSION);
    fprintf(stderr,
"Usage: %s datasource [username [password]] spec=val spec=val ... -options\n",
            progname);
    fprintf(stderr,
"The specifiers are:\n"
...
"-K             Use Kubl default conversions (in addition to any of your own).\n");
    ...
    fprintf(stderr,"More?>"); getchar(); /* This is for pause. */
    fprintf(stderr,
"Options are:\n"
...
"-V Produce DB..vd_remote_data_source and DB..vd_remote_table calls.\n"
    ...
    fprintf(stderr,
"Use    %s -l   to list available datasources.\n",progname);
    fprintf(stderr,
"E.g. use\n"
...
"(or something like .../cgi-bin/dbdump or dbdump.cgi with Unix Web servers)\n"
    ...
    fprintf(stderr,
"Use    %s -h > %s   to dump out the internal HTML template to the\n",
        progname,DEFAULT_HTML_TEMPLATE_FILE);
    fprintf(stderr,
"       default template file, after which you can edit it to suit your taste.\n");
    fprintf(stderr,
"Use    %s -hh  to see how the internal template looks after expansion.\n",
        progname);
    fprintf(stderr,
"Please see http://www.kubl.com/ for more information about our products.\n");
    fflush(stderr);
}

int version_less_than(char *s,int major,int minor)
{
    int tmp;
    while(*s && NOT isdigit(*s)) { s++; } /* Skip non-digits. */
    if(empty_stringp(s)) { return(0); } /* Premature end? */
    tmp = atoi(s);
    if(tmp > major) { return(0); }
    else if(tmp < major) { return(1); }
    /* They are equivalent, we have to check the minor part also. */
    while(*s && (*s != '.')) { s++; } /* Search a period. */
    if(empty_stringp(s) || empty_stringp(++s))
     { return(0); } /* Not found or followed by nothing */
    tmp = atoi(s);
    return(tmp < minor);
}
...
int connect_to_datasource_or_dest(HDBC *ptr_to_hdbc,
                                  HENV henv,
                                  char *datasource,
                                  char *username,
                                  char *password)
 ...
 */
int check_for_failed_tables(struct tabledeflist *tl)
{
       ...
       if(tl->last_SQL_state)
        {
          if(0 == number_of_failed)
           {
             fprintf(stdout,
                 "-- The linking of the following tables failed for the following reasons:\n");
           }
          fprintf(stdout,"-- \"%s\" -> %s: %s\n",
                         tl->name,
                         tl->last_SQL_state, tl->last_SQL_message);
          number_of_failed++;
        }
     }
...
main (int argc, char ** argv)
{
  ...
  if((query_string = getenv("QUERY_STRING")) || getenv("HTTP_HOST")
       || getenv("SERVER_PROTOCOL")) /* Used as a CGI-script? */
     ...
     if(flag_get_list_of_tables || (!datasource || universal_transfer_flag))
      {
        ...
        if(!flag_get_list_of_tables)
         {
           output_html_file(universal_transfer_flag
                            ? DEFAULT_TRANSFER_TEMPLATE1
                            : DEFAULT_HTML_TEMPLATE_FILE);
           if(!universal_transfer_flag) { exit(0); }
         }
      }
     else /* Ordinary usage. */
      {
...
/* If the argument begins with a dash (-) then overwrite it with an
   ...
   parse_url_query_string. */
        if(((*(argv[i]) == '-') && (*(argv[i]) = '=')) ||
             strchr(argv[i],'=')) /* There's an equal sign? */
         {
           query_string = argv[i];
           query_string_error = parse_url_query_string(query_string);
           if(query_string_error) { goto arg_error; }
         }
        else
         {
           switch(++nth_non_option)
            {
              case 1: { datasource = argv[i]; break; }
              case 2: { username   = argv[i]; break; }
              case 3: { password   = argv[i]; break; }
            }
         }
  ...
  if(list_data_sources_flag)
   { list_data_sources(web_mode,"",""); exit(0); }

  if(dump_html_template_file_flag)
   {
     output_html_file(DEFAULT_HTML_TEMPLATE_FILE); exit(0);
   }
...
arg_error:
  ...
  if((NO datasource) && (0 == web_mode)) { usage(*argv); exit(0); }
 ...
 */
  ...
  if(NO only_table_definitions_flag &&
     NO only_index_definitions_flag &&
     NO only_contents_flag)
   { /* If ALL these flags has been left OFF, then turn them ALL ON */
     only_table_definitions_flag = only_index_definitions_flag
     ...
     for(i = 2; i <= (2*n_conv_types); i += 2)
      {
        fprintf(stdout,"-- From %s to %s\n",
                (conv_types_table[i] ? conv_types_table[i] : "NULL"),
                (conv_types_table[i+1] ? conv_types_table[i+1] : "NULL"));
      }
   }

  fflush(stdout);

  connect_to_datasource_or_dest(&hdbc,henv,
                                datasource,
                                username,
                                password);
...
error:
  ...
  if(datadest)
   {
     connect_to_datasource_or_dest(&datadest_hdbc,henv,
                                   datadest,
                                   dest_username,
                                   dest_password);
...

  {
    ...
    tl = all_tables_list = get_all_tables(tablequalifier,
                                 tableowner,
                                 tablename,
                                 tabletype,&count);
 ...
 */
    ...
    else
     {
       ...
       if(verbose_flag)
        {
          fprintf(stdout,
"-- SQLPrimaryKeys not implemented. Trying to find primary keys with SQLStatistics.\n");
        }
       SQLGetFunctions(hdbc, SQL_API_SQLSTATISTICS, &FunctionExists);
       if(NO FunctionExists)
        {
          fprintf(stderr,
           "-- Sorry, SQLStatistics not implemented. Exiting.\n");
          exit(1);
        }
     }
 ...
 */
             get_table_bestrow_columns(tl,
                       ((UWORD) ((primary_key_by_all_means_flag & 3)-1)),
                       ((UWORD) SQL_NULLABLE)); /* Or SQL_NO_NULLS */
             tl = tl->next;
           }
        }
     }
...
/*        debug_fprintf(stdout,
    ...
    if(verbose_flag)
     {
       fprintf(stdout,
          "-- Definitions of %d tables were read in.\n",count);
       fflush(stdout);
     }
       ...
       if(n_failed)
        {
          if(all_or_nothing_flag)
           {
             fprintf(stdout,
"-- %d linkages failed, discarding all the linkages done at this time.\n",
                     n_failed);
...
char *line_from_html_file(char *templatename)
{
 ...
 */
char *line_from_html_string(char *templatename)
{
...
int output_html_file(char *templatename)
{
/* When using option -h, then take it always from the memory. */
    if(dump_html_template_file_flag
        || NO(html_infp = fopen(templatename,"r")))
     {
 ...
 */
    ...
    else { output_html_page(line_from_html_file,templatename); }
    return(1);
}
...
"<FORM NAME=\"DBDUMP_OPTIONS\" METHOD=GET"
" onsubmit=\"return(set_options_according_to_action(this))\">\n"
