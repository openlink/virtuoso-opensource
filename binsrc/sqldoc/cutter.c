...
int cut_check_for_hit (cut_match_t *cm, char *line)
{
  int line_strlen;
  switch ((cm->cm_flags) & (CM_SHOULD_BEGIN_LINE | CM_SHOULD_END_LINE))
    {
    case 0:
      return (NULL != strstr (line, cm->cm_pattern));
    case CM_SHOULD_BEGIN_LINE:
      return (0 == strncmp (line, cm->cm_pattern, cm->cm_pattern_len));
    case CM_SHOULD_END_LINE:
      line_strlen = strlen (line);
      return (
	(line_strlen > cm->cm_pattern_len) &&
	(0 == memcmp (line+(line_strlen-cm->cm_pattern_len), cm->cm_pattern, cm->cm_pattern_len)) );
    case CM_SHOULD_BEGIN_LINE | CM_SHOULD_END_LINE:
      return (0 == strcmp (line, cm->cm_pattern));
    }
  return 0;
}
...
search_for_start:
      for (cm_ctr = 0; cm_ctr < env->ce_cfg.cc_block_starts_count; cm_ctr++)
	{
	  ...
	  int is_hit = cut_check_for_hit (curr_cm, line);
	  if (is_hit)
...
int main (int argc, const char *argv[])
{
  ...
  while (argctr < argc)
      ...
      if (!strncmp ("-BS", argv[argctr],3) && (argctr < (argc-1)))
	{
	  ...
	  env->ce_cfg.cc_block_starts_count += 1;
	  argctr += 2;
	  continue;
	}
      if (!strncmp ("-BH", argv[argctr],3) && (argctr < (argc-1)))
	{
	  ...
	  env->ce_cfg.cc_block_hits_count += 1;
	  argctr += 2;
	  continue;
	}
      if (!strncmp ("-BT", argv[argctr],3) && (argctr < (argc-1)))
	{
	  ...
	  new_cm->cm_flags = atoi (argv[argctr]+3);
	  new_cm->cm_score = 1;
	  env->ce_cfg.cc_block_ends_count += 1;
	  argctr += 2;
	  continue;
	}
