   ...
   public static void main(String args[])
   {
      try
      {
         ...
         for(int i = 0;i < 100;i++)
         {
            pstmt.setInt(1,i);
            pstmt.setInt(2,i);
            ((VirtuosoPreparedStatement)pstmt).addBatch();
            ((VirtuosoPreparedStatement)pstmt).clearParameters();
         }
         System.out.println("    PASSED");
         System.out.print("Execute batch");
         int[] res = ((VirtuosoPreparedStatement)pstmt).executeBatch();
         pstmt.close();
         for(int i = 0;i < 100;i++)
            if(res[i] != 1)
            {
         ...
         System.out.println("    PASSED");
         System.out.print("Execute batch");
         res = ((VirtuosoPreparedStatement)pstmt).executeBatch();
         pstmt.close();
         for(int i = 0;i < 100;i++)
            if(res[i] != 1)
            {
