...
virt_wide_to_ansi (TCHAR * in)
{
  ...
  len = WideCharToMultiByte(CP_ACP, 0, in, -1,
      NULL, 0, NULL , NULL );
  ...
  WideCharToMultiByte (CP_ACP, 0, in, -1,
      ret, len, NULL, NULL);
...
#ifdef DSN_TRANSLATION
# ifdef WIN32
	SQLGetPrivateProfileString (section, attrs[i].longName, _T (""),
	    attrs[i].data, attrs[i].maxLength, _T ("odbc.ini"));
# else
	valueW = NULL;
...
FillUpLoginDatabaseCombo (HWND hDlg)
{
  ...
  selection_index = (int)SendDlgItemMessage(hDlg, IDC_CHARSET, CB_FINDSTRINGEXACT, -1,
      (LPARAM)(attrs[oCHARSET].data[0] ? attrs[oCHARSET].data : DEFAULT_DATABASE_PER_USER));
  SendDlgItemMessage(hDlg, IDC_CHARSET, CB_SETCURSEL,
      selection_index == CB_ERR ? -1 : selection_index, (LPARAM)0);
  SQLFreeStmt (stmt, SQL_DROP);
  SQLDisconnect (driver_connect_dbc);
}
    ...
    LPARAM	lParam)
{
  ...
  switch (wMsg)
	...
	DSN = _T ("(File DSN)");
      ...
      DATABASE = (attrs[oDATABASE].data && _tcslen(attrs[oDATABASE].data))
	? attrs[oDATABASE].data : DEFAULT_DATABASE_PER_USER;
	...
	SendDlgItemMessage(hDlg, IDC_DATABASE, CB_ADDSTRING, 0, (LPARAM)DATABASE);
      SendDlgItemMessage (hDlg, IDC_DATABASE, CB_SELECTSTRING, 0,
	(LPARAM) DATABASE);
	...
	SetDlgItemText (hDlg, IDC_UID, UID=_T ("dba"));
	  ...
	  toupper (attrs[oENCRYPT].data[0]) == 'T' ||
	  attrs[oENCRYPT].data[0]  == '1'), 0);
#endif
#endif
...
DriverConnectDialog (void *hwnd)
{
  ...
  iRet = DialogBox (s_hModule, MAKEINTRESOURCE (DlgLogin),
      (HWND) hwnd, FDriverConnectProc);
    ...
    UWORD fDriverCompletion)
{
  ...
  if (fDriverCompletion == SQL_DRIVER_COMPLETE ||
    ...
    {
      if (!attrs[oUID].data || !attrs[oUID].data[0] || attrs[oUID].data[0] == ' ' ||
	  !attrs[oPWD].data || !attrs[oPWD].data[0] || attrs[oPWD].data[0] == ' ' ||
  ...
  con->con_pwd_cleartext = 0;
#endif
  FORCE_DMBS_NAMEW = attrs[oFORCE_DBMS_NAME].data &&
      _tcslen (attrs[oFORCE_DBMS_NAME].data) ?
        attrs[oFORCE_DBMS_NAME].data : NULL;
  if (FORCE_DMBS_NAMEW)
    {
  ...
  if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO)
      ...
      if (DAYLIGHTW)
	{
      ...
      if (-1 == StrCopyOut (cmd, szConnStrOut, cbConnStrOutMax,
          (u_short *) pcbConnStrOutMax))
	{
	...
	SWORD cbPWD)
{
  ...
  return virtodbc__SQLDriverConnect (hdbc, NULL, (SQLTCHAR *) cmd, SQL_NTS, NULL, 0, NULL,
      SQL_DRIVER_NOPROMPT);
#endif
}
