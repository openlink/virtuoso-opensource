...
bif_vt_batch_d_id (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (D_AT_END (&nid) || D_INITIAL (&nid) || D_PRESET (&nid)
      || (DV_LONG_INT == DV_TYPE_OF (new_d_id) && (uint32) unbox (new_d_id) >= 0xff000000))
    sqlr_new_error ("22023", "FT010", "vt_batch_d_id id's cannot be 0, -1 or -2");
...
"{\n"
"  declare str, text_id_col, kn, vt_name, pk_suits, _colname, func, ufunc, vi_column varchar;\n"
"  declare _coldtp smallint;\n"
"  declare k_id, pk_used, pk_parts integer;\n"
"  if (_lang is null)\n"
"    _lang := '*ini*';\n"
"  if (_enc is null)\n"
"    _enc := '*ini*';\n"
...
"      if (isstring (use_id) and _colname = DB.DBA.repl_undot_name (use_id) and dv_type_title (_coldtp) <> \'INTEGER\') \n"
"	signal (\'22023\', concat (\'the column \', use_id, \' is not an integer\'), \'FT024\'); \n"
"      if (k_id > 0) \n"
"	pk_suits := \'\'; \n"
"      else if (dv_type_title (_coldtp) = \'INTEGER\') \n"
"	pk_suits := _colname; \n"
"      pk_parts := pk_parts + 1; \n"
"    } \n"
"pk_check_done: \n"
...
"	  DB.DBA.execstr (sprintf (\'alter table \"%I\".\"%I\".\"%I\" add \"%I\" integer\', name_part (tb, 0), name_part (tb, 1), name_part (tb, 2), text_id_col));\n"
"	  \n"
"	  pk_used := null; \n"
"	} \n "
"    }\n"
...
"  str := sprintf (\'create table \"%I\".\"%I\".\"%I\" (VT_WORD varchar, VT_D_ID integer, VT_D_ID_2 integer, VT_DATA varchar, VT_LONG_DATA long varchar, primary key (VT_WORD, VT_D_ID))\',\n"
"  name_part (vt_name, 0), name_part (vt_name, 1), name_part (vt_name, 2));\n"
"  DB.DBA.execstr (str);\n"
...
"  DB.DBA.vt_create_update_log (tb, is_xml, defer_generation, obd, func, ufunc, _lang, _enc); \n"
...
"	\\\n\',\n"
"      sprintf (\n"
"	  \'insert into \"%I\".\"%I\".\"%I\" (VT_WORD, VT_D_ID, VT_D_ID_2, VT_DATA, VT_LONG_DATA)\n"
...
"  text_value := concat (\n"
"      sprintf (\n"
"	\'create procedure %s\"VT_PROCESS_WORD_BATCH_%s\" (inout word varchar, \\\n\' , dbpref, data_table_suffix),\n"
"	\'	inout wb any, inout n_w integer, inout n_ins integer, inout n_upd integer, \n"
"		 inout n_next integer) \n"
"	 {\n"
...
"		   if (0 = length (str1))\\\n\',\n"
"      sprintf (\n"
"	\'	     delete from \"%I\".\"%I\".\"%I\" where current of cr;\\\n\', name_part (full_words_table,0), name_part (full_words_table,1), name_part (full_words_table,2)),\n"
"	\'	  else\n"
"		     {\n"
...
"			   n_upd := n_upd + 1;\\\n\',\n"
"      sprintf (\n"
"	\'		   update \"%I\".\"%I\".\"%I\" set VT_D_ID = id1, VT_D_ID_2 = id2, VT_DATA = str1, VT_LONG_DATA = blob where current of cr;\\\n\', name_part (full_words_table,0), name_part (full_words_table,1), name_part (full_words_table,2)),\n"
"	\'		}\n"
"		       else\n"
"			 {\n"
"			   n_ins := n_ins + 1;\\\n\',\n"
...
"  text_value := concat (\n"
"      sprintf (\n"
"	\'create procedure %s\"VT_BATCH_REAL_PROCESS_%s\" (inout invd any)\\\n\', dbpref, data_table_suffix),\n"
"	\'{\n"
"	   declare n_w, n_ins, n_upd, n_next, inx integer;\n"
"	   inx := 0;\n"
...
"  text_value := concat (\n"
"      sprintf (\n"
"	\'create procedure %s\"VT_BATCH_PROCESS_%s\" (inout vtb any)\\\n\', dbpref, data_table_suffix),\n"
"	\'{\n{\n"
"	   declare invd any;\n"
...
"  declare inx, _dav_flag, is_pk integer; "
...
"  declare tb_suff, vtlog_suff varchar;"
"  declare _lang_enc_args varchar;"
...
"  select VI_COL, VI_ID_COL, VI_ID_IS_PK into datacol, keycol, is_pk from DB.DBA.SYS_VT_INDEX where VI_TABLE = tablename; "
"  if (datacol is null)"
"    signal ('42000','Misc. error upon update log creation. The free text index cannot be created.', 'FT026');"
"  _lang_enc_args := concat (\', \\\'\', _lang, \'\\\', \\\'\', _enc, \'\\\'\');\n"
"  commands := vector ( "
"      case when create_log_tb then \'create table <DB>.<DBA>.\"VTLOG_<VTLOGSUFF>\" (\"VTLOG_<KFU>\" any not null primary key, SNAPTIME datetime, DMLTYPE varchar (1), VT_GZ_WORDUMP long varbinary, VT_OFFBAND_DATA long varchar)\\\n\' else NULL end, "
"	either (isnull (is_pk), "
...
"	       { \\\n"
"		 if (<FN_HOOK_I>) { \\\n"
"		       <DB>.<DBA>.\"VT_BATCH_PROCESS_<SUFF>\" (vtb); \\\n"
"		     } \\\n"
...
"		VT_OFFBAND_DATA \\\n"
"	      from \\\n"
"		<DB>.<DBA>.\"VTLOG_<VTLOGSUFF>\" \\\n"
"	      order by \"VTLOG_<KFU>\" for update; \\\n"
" "
...
"		  if (_dmltype <> \\\'D\\\') \\\n"
"		    { \\\n"
"			select <RES_TYPE_COL> <DF> <OFF_DATA_COLS> into <DAV_RES_TYPE_VAR> _data <OFF_DATA_VARS> from <DB>.<DBA>.<TB> where <KF> = _vtlog_id; \\\n"
...
"	      <DB>.<DBA>.\"VT_BATCH_PROCESS_<SUFF>\" (vtb); \\\n"
"	      vtb := vt_batch (1001 <LANG_ENC>); \\\n"
"	      delete from <DB>.<DBA>.\"VTLOG_<VTLOGSUFF>\" where \"VTLOG_<KFU>\" <= start; \\\n"
"	      len := 0; \\\n"
"	      commit work; \\\n"
"	    } \\\n"
"	done: \\\n"
"	  <DB>.<DBA>.\"VT_BATCH_PROCESS_<SUFF>\" (vtb); \\\n"
"	  delete from <DB>.<DBA>.\"VTLOG_<VTLOGSUFF>\"; \\\n"
"	}\', "
"	either (isnull (is_pk), "
...
"	   index_cmd := sprintf ('create index \"%I\" on \"%I\".\"%I\".\"%I\" (\"%I\")', kn, name_part (tablename, 0), name_part (tablename, 1), name_part (tablename, 2), keycol);\n"
