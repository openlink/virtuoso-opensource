...
CE_NAME (col_pos_t * cpo, db_buf_t ce_first, int n_values, int n_bytes)
{
  ...
  if (CMP_NONE != sp->sp_min_op)
    {
      itc->itc_nth_key = sp->sp_min;
      dtp_cmp = 0;
      lower = itc_ce_value_offset (itc, ce, &body, &dtp_cmp);
      if (dtp_cmp > DVC_GREATER)
	return 0;
      if (DVC_DTP_GREATER == dtp_cmp)
	lower = -1;
      if (CMP_GT == sp->sp_min_op)
	lower++;
  ...
  if (CMP_NONE != sp->sp_max_op)
    {
      itc->itc_nth_key = sp->sp_max;
      dtp_cmp = 0;
      upper = itc_ce_value_offset (itc, ce, &body, &dtp_cmp);
      if (dtp_cmp > DVC_GREATER)
	return 0;
      if (DVC_DTP_GREATER == dtp_cmp)
	{
  ...
  while (ce_first < ce_end)
      ...
      else
	{
	  ...
	  for (inx = skip2; inx < last; inx++)
	    ...
	    repeating_place:
	      if (base + (int64) n >= lower && base + (int64) n <= upper)
		itc->itc_matches[itc->itc_match_out++] = last_row;
