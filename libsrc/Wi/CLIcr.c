...
stmt_row_bookmark (cli_stmt_t * stmt, caddr_t * row)
{
  ...
  bm = box_copy_tree (bm);
  sethash ((void *) (ptrlong) bmid,
	   stmt->stmt_bookmarks, (void *) bm);
  id_hash_set (stmt->stmt_bookmarks_rev, (caddr_t) &bm, (caddr_t) &bmid);
  sethash ((void *) (ptrlong) bmid,
	   con->con_bookmarks, (void *) bm);
  LEAVE_CON (con);
  return bmid;
}
...
stmt_process_rowset (cli_stmt_t * stmt, int ftype, SQLULEN * pcrow)
{
  ...
  stmt->stmt_rowset = (caddr_t **) dk_alloc_box (rssz * sizeof (caddr_t),
						 DV_ARRAY_OF_POINTER);
		     ...
		     SQLLEN bookmark_offset)
{
  ...
  if (SQL_CURSOR_FORWARD_ONLY == stmt->stmt_opts->so_cursor_type
      || stmt->stmt_compilation->sc_is_select != QT_SELECT)
    {
  ...
  stmt->stmt_future = PrpcFuture (stmt->stmt_connection->con_session,
				  &s_sql_extended_fetch,
				  stmt->stmt_id, fFetchType, irow, stmt->stmt_rowset_size,
				  stmt->stmt_opts->so_autocommit, bookmark);
...
set_pos_param_row (cli_stmt_t * stmt, int nth)
{
  ...
  int iparam = 0;
  caddr_t *row = (caddr_t *) dk_alloc_box_zero (n_cols * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
  ...
  for (iparam = 0; iparam < n_cols; iparam++)
      ...
      if (cb && cb->cb_place)
	{
	  ...
	  int rebind_offset = stmt->stmt_imp_row_descriptor ?
	       ...
	       *(stmt->stmt_imp_row_descriptor->d_bind_offset_ptr) : 0) : 0;
	  ...
	  if (length && SQL_IGNORE == *length)
	    ...
	    {
	      caddr_t v = buffer_to_dv (place, length, c_type, c_type, BHID (nth, iparam + 1), NULL,
	    ...
	    SQLUSMALLINT fLock)
{
  ...
  int irow = (int) _irow;
  STMT (stmt, hstmt);
  int n_rows = (int) (irow != 0 ? 1
  : (fOption == SQL_ADD ? stmt->stmt_rowset_size
     : stmt->stmt_rowset_fill));
  ...
  if (SQL_UPDATE == fOption || SQL_ADD == fOption)
      ...
      if (!params)
	{
	  if (0 == irow)
	    {
	      params = (caddr_t *) dk_alloc_box_zero (n_rows * sizeof (caddr_t),
						 DV_ARRAY_OF_POINTER);
	    ...
	    if (NULL == (params = set_pos_param_row (stmt, irow - 1)))
	      return SQL_ERROR;
  ...
  if (SQL_ERROR == rc)
    {
      err_queue_append (&stmt->stmt_error.err_queue,
			&sps->stmt_error.err_queue);
      return SQL_ERROR;
    }
  if (0 == irow)
    {
