...
sqlo_qc_position_texp (sqlo_t * so, query_cursor_t * qc)
{
  ...
  int nth = 0;
  from[0] =
      (ST *) t_list (5, TABLE_DOTTED, t_box_string (ot->ot_table->tb_name),
      NULL, t_box_num (ot->ot_u_id), t_box_num (ot->ot_g_id));
