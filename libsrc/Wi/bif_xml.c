...
size_t sgdfi_read (void *read_cd, char *buf, size_t bsize)
{
  sqlgetdata_fwd_iter_t *iter = (sqlgetdata_fwd_iter_t *) read_cd;
  SQLLEN len = 0, res;
  SQLGetData (iter->sgdfi_hstmt, iter->sgdfi_inx, iter->sgdfi_c_type, buf, bsize, &len);
  if (0 >= len)
    return 0;
  res = MIN ((size_t)len, (bsize - ((SQL_C_WCHAR == iter->sgdfi_c_type) ? sizeof (wchar_t) : 1)));
  return res;
}

xml_tree_ent_t *rst_xml_result (remote_stmt_t * rst, int inx, caddr_t * qst, int c_type, caddr_t *err_ret, caddr_t start, size_t start_len)
{
  dk_set_t top;
  caddr_t tree;
  vxml_parser_config_t config;
...
/* !!! FixMe!!! Edit xslt.c in order to process attributes inside preparing the sheet! Not here! */
  ...
  if (!rc)
	...
	*err_ret = srv_make_new_error ("22007", "XM003", "%.1500s", rc_msg);
        ...
        sqlr_new_error ("22007", "XM003", rc_msg);
      dk_free_box (rc_msg);
      return (NEW_DB_NULL);
    }
  XP_STRSES_FLUSH (&context);
  ...
  if (NULL == tree)
  ...
  else
    {
      ...
      xte->xe_doc.xd->xd_uri = box_dv_short_string ("");
      xte->xe_doc.xd->xd_dtd = dtd; /* The refcounter is incremented inside xml_make_tree */
...
bif_row_vector (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t * rbox;
  int res_fill = 0;
  caddr_t res[ENTITY_MAX_ATTRS];
  dbe_table_t * tb;
  query_instance_t * qi = (query_instance_t *) qst;
  long len, head_len;
  caddr_t row = bif_arg (qst, args, 0, XMLROWVECT);
  dtp_t rdtp = DV_TYPE_OF (row);
  it_cursor_t itc_auto;
  it_cursor_t * itc = &itc_auto;
  if (rdtp != DV_SHORT_CONT_STRING
      && rdtp != DV_LONG_CONT_STRING)
    sqlr_error ("XM220", "Expected _ROW string for row_vector");

  db_buf_length ((unsigned char *) row, &head_len, &len);
  itc->itc_position = 0;
  ITC_INIT (itc, qi->qi_space, qi->qi_trx);
  itc_make_row_map (itc, (db_buf_t) row);
  if (!itc->itc_row_map_key
      ||  !sch_is_subkey_incl (isp_schema (qi->qi_space), itc->itc_row_map_key->key_id, entity_key_id))
    sqlr_error ("XM001", "Not an entity subtable in row_vector");
  tb = itc->itc_row_map_key->key_table;
  e_name = (caddr_t) gethash ((void*)(unsigned long)  itc->itc_row_map_key->key_id, xml_global->xs_key_id_to_element);
  if (e_name && tb->tb_col_id_to_misc_id)
    {
      dk_hash_iterator_t hit;
      oid_t m_id;
      oid_t c_id;
      dk_hash_iterator (&hit, tb->tb_misc_id_to_col_id);
      while (dk_hit_next (&hit, (void**) &m_id, (void*) &c_id))
	{
	  xml_attr_t * attr = lt_xml_attr_by_id (NULL, m_id);
	  val = itc_box_column (itc, (db_buf_t) row, c_id, NULL);
	  res[res_fill++] = box_copy (attr->xa_name);
	  res[res_fill++] = val;
	}
    }
  if (!e_name)
    e_name = itc_box_column (itc, (db_buf_t) row, entity_name_col_id, NULL);
  else
    e_name = box_copy (e_name);
  e_level = itc_box_column (itc, (db_buf_t) row, entity_level_col_id, NULL);
  e_id = itc_box_column (itc, (db_buf_t) row, entity_id_col_id, NULL);
  e_wspace = itc_box_column (itc, (db_buf_t) row, entity_wspace_col_id, NULL);

  if (!itc->itc_row_map_key
      ||  !sch_is_subkey_incl (isp_schema (qi->qi_space), itc->itc_row_map_key->key_id, textfrag_key_id))
    {
      e_leading = itc_box_column (itc, (db_buf_t) row, entity_leading_col_id, NULL);
      e_trailing = itc_box_column (itc, (db_buf_t) row, entity_trailing_col_id, NULL);
      misc = itc_column (itc, (db_buf_t) row, entity_misc_col_id);
      db_buf_length (misc, &head_len, &len);
      misc += head_len;
      misc_end = misc + len;
      while (misc < misc_end)
	{
	  oid_t m_id = LONG_REF (misc);
	  misc += sizeof (int32);
	  val = itc_box_column (itc, (db_buf_t) row, 0, misc);
	  DB_BUF_TLEN (len, *misc, misc);
	  misc += len;
	  res[res_fill++] = box_copy (lt_xml_attr_by_id (NULL, m_id)->xa_name);
	  res[res_fill++] = val;
	}
      rbox = (caddr_t *) dk_alloc_box (res_fill * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
      memcpy (rbox, res, sizeof (caddr_t) * res_fill);
    }
  else
    {
      e_leading = itc_box_column (itc, (db_buf_t) row, textfrag_leading_col_id, NULL);
      e_trailing = itc_box_column (itc, (db_buf_t) row, entity_trailing_col_id, NULL);
      rbox = (caddr_t *) dk_alloc_box (0, DV_DB_NULL);
    }

  return (list (7, e_id, e_level, e_name, e_wspace, e_leading, e_trailing, rbox));
#else
  O12
  return 0; /*dummy */
#endif
...
#ifndef O12
void
bx_row_tag (caddr_t * qst, dk_session_t * out, caddr_t row,
	    close_tag_t ** ct_ret, int value_only, int e_level, int is_utf8)
{
  int e_table_name = 0;
  caddr_t e_name;
  db_buf_t e_leading;
  ...
  if (!itc->itc_row_map_key
      ||  !sch_is_subkey_incl (isp_schema (qi->qi_space), itc->itc_row_map_key->key_id, entity_key_id))
    sqlr_error ("XM001", "Not an entity subtable in row_vector");
  if (itc->itc_row_map_key->key_id == textfrag_key_id)
    {
      db_buf_t dv = itc_column (itc, (db_buf_t) row, entity_leading_col_id);
      if (dv && IS_STRING_DTP (dv[0]))
	bx_out_dv (qst, out, dv, src_charset);
      else
	{
	bx_out_dv (qst, out, itc_column (itc, (db_buf_t) row, textfrag_long_col_id), src_charset);
	}
      return;
    }
  ...
  if (value_only)
    {
      ...
      if (dv && IS_STRING_DTP (dv[0]))
	bx_out_dv (qst, out, dv, src_charset);
      bx_push_ct (ct_ret, e_level, NULL, itc_box_column (itc, (db_buf_t) row, entity_trailing_col_id, NULL));
      return;
    }
  tb = itc->itc_row_map_key->key_table;
  e_name = (caddr_t) gethash ((void*)(unsigned long) itc->itc_row_map_key->key_id, xml_global->xs_key_id_to_element);
  ...
  bx_push_ct (ct_ret, e_level, e_name, itc_box_column (itc, (db_buf_t) row, entity_trailing_col_id, NULL));

  session_buffered_write_char ('<', out);
  bx_out_q_name (qst, out, *ct_ret, e_name, 0, src_charset);
  if (e_table_name && tb->tb_col_id_to_misc_id)
    {
      dk_hash_iterator_t hit;
      oid_t m_id;
      oid_t c_id;
	...
	{
	  db_buf_t dv;
	  xml_attr_t * attr = lt_xml_attr_by_id (qi->qi_trx, m_id);
	  dv= itc_column (itc, (db_buf_t) row, c_id);
	  if (dv && DV_DB_NULL == dv[0])
	    continue;
	  SES_PRINT (out, " ");
	  bx_out_q_name (qst, out, *ct_ret, attr->xa_name, 1, src_charset);
	  SES_PRINT (out, "=\"");
	  bx_out_dv (qst, out, dv, src_charset);
	  SES_PRINT (out, "\"");
	}
  ...
  while (misc < misc_end)
    {
      oid_t m_id = LONG_REF (misc);
      xml_attr_t * xa = lt_xml_attr_by_id (qi->qi_trx, m_id);
      misc += sizeof (int32);
      if (misc[0] != DV_DB_NULL)
	{
	  SES_PRINT (out, " ");
	  if (xa)
	    bx_out_q_name (qst, out, *ct_ret, xa->xa_name, 1, src_charset);
	  else
	    SES_PRINT (out, "bad-attribute");
	  SES_PRINT (out, "=\"");
	  bx_out_dv (qst, out, misc, src_charset);
	  SES_PRINT (out, "\"");
	}
      DB_BUF_TLEN (len, *misc, misc);
      misc += len;
  ...
  session_buffered_write_char ('>', out);
  e_leading = itc_column (itc, (db_buf_t) row, entity_leading_col_id);
  if (e_leading && IS_STRING_DTP (e_leading[0]))
    bx_out_dv (qst, out, e_leading, src_charset);
}


caddr_t
bx_serialize (query_instance_t * qi, caddr_t id, dk_session_t * out,
	      int value_only)
{
  caddr_t row;
  int start_level = -1, level;
  close_tag_t * ct = NULL;
  local_cursor_t * lc;
  caddr_t err;

  err = qr_rec_exec (select_groupentity_qr, qi->qi_client, &lc, qi, NULL, 1,
		     ":0", box_copy(id), QRP_RAW);
  ...
  for (;;)
      ...
      if (!rc)
	{
	  ...
	  while (ct)p
	    ct = bx_pop_ct ((caddr_t *) qi, out, ct, QST_CHARSET(qi->qi_qst), 1);
	  lc_free (lc);
	  return NULL;
	}
      level = unbox (lc_nth_col (lc, 1));
      row = lc_nth_col (lc, 0);
      if (-1 == start_level)
	start_level = level;
      else if (level <= start_level)
	{
      ...
      if (ct && ct->ct_level >= level)
	{
	  while (ct->ct_level >= level)
	    ct = bx_pop_ct ((caddr_t *) qi, out, ct, QST_CHARSET(qi->qi_qst), 1);
	}
      bx_row_tag ((caddr_t*) qi, out, row, &ct, value_only, level, 0);
      if (1 == value_only)
	value_only = 0;
    }
}
...
caddr_t
bif_xml_serialize (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  caddr_t val;
  caddr_t id = bif_bin_arg (qst, args, 0, "getxml");
  long fl = bif_long_arg (qst, args, 1, "getxml");
  dk_session_t * out = strses_allocate ();
  caddr_t err = bx_serialize ((query_instance_t*) qst,  id, out, fl);
  if (err != (caddr_t) SQL_SUCCESS)
    {
      strses_free (out);
      sqlr_resignal (err);
  ...
  if (!STRSES_CAN_BE_STRING (out))
    {
      *err_ret = STRSES_LENGTH_ERROR ("getxml");
      val = NULL;
    }
  else
    val = strses_string (out);
  strses_free (out);
  return val;
}
#endif
...
bif_xml_validate_dtd (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  do
	...
	sqlr_error ("42000", "Unable to validate BLOB of length %d bytes", blob_arg_len);
...
#ifndef O12
  bif_define (XMLSERIALIZEFNC, bif_xml_serialize);
  bif_set_uses_index (bif_xml_serialize);
#endif
