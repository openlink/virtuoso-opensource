...
/* procedures */
%token AGGREGATE PROCEDURE FUNCTION OUT_L HANDLER IF THEN ELSE ELSEIF WHILE
%token BEGINX ENDX RETURN CALL RETURNS DO EXCLUSIVE PREFETCH
%token SQLSTATE SQLWARNING SQLEXCEPTION FOUND EXIT RESIGNAL
...
%token BEGIN_FN_X BEGIN_CALL_X BEGIN_EQCALL_X BEGIN_OJ_X BEGIN_U_X CONVERT CASE WHEN THEN IDENTITY LEFT RIGHT FULL OUTER
...
%token TIMESTAMP_FUNC SQL_TSI IDENTIFIED EXTRACT
%token KWD_TAG
...
ordering_spec
	: scalar_exp opt_asc_desc
		{ $$ = t_listst (3, ORDER_BY, INTEGERP ($1) ? (caddr_t) unbox ($1) : (caddr_t) $1, (ptrlong) $2);  }
