	...
	SWORD FAR * pfNullable)
{
  ...
  if (BOX_ELEMENTS (sc) > 3 && sc->sc_params)
	...
	*pcbColDef = unbox (pd->pd_prec);
      if (pibScale)
	*pibScale = (SWORD) unbox (pd->pd_scale);
      if (pfNullable)
	...
	UNALIGNED SQLINTEGER * StringLengthPtr)
{
  ...
  switch (fOption)
    ...
    case SQL_NO_CHAR_C_ESCAPE:
      if (pvParam)
	*(SWORD *) pvParam = (SWORD) con->con_defs.cdef_no_char_c_escape;
      break;
    case SQL_CHARSET:
	{
	  char *chrs_name = "";
    ...
    case SQL_PWD_CLEARTEXT:
      if (pvParam)
	*(SWORD *) pvParam = (SWORD) con->con_pwd_cleartext;
      break;
      ...
      if (pvParam)
	{
	  ...
	  *(SWORD *) pvParam = (SWORD) SESSION_IS_INPROCESS (con->con_session);
#else
	  *(SWORD *) pvParam = (SWORD) 0;
#endif
	}
      break;
    }
  ...
  switch (fOption)
      ...
      if (CON_CONNECTED (con))
	{
	  if (con->con_qualifier && vParam && strcmp ((const char *) con->con_qualifier, (char *)vParam))
	    {
	      HSTMT stmt;
	      RETCODE rc = virtodbc__SQLAllocStmt (hdbc, &stmt);
	      if (SQL_SUCCESS != rc)
		return rc;
      ...
      UWORD FAR * pfExists)
{
  ...
  if (fFunction == SQL_API_ALL_FUNCTIONS)
    memcpy (pfExists, &functions, 100 * sizeof (UWORD));
#if (ODBCVER >= 0x0300)
	...
	SWORD FAR * pcbInfoValue)
{
  ...
  switch (fInfoType)
    ...
    case SQL_DRIVER_HSTMT:
      intres = (SQLLEN) rgbInfoValue;	/* Or *((HSTMT *)rgbInfoValue) ? */
      break;
#endif
...
*/
  ...
  if (strres)
      ...
      if (rgbInfoValue)
	{
      ...
      if (pcbInfoValue)
	{
	  *pcbInfoValue = (SWORD) strlen (strres);
	}
	...
	SWORD FAR * pcbInfoValue)
{
  ...
  switch (fInfoType)
    ...
    case SQL_XOPEN_CLI_YEAR:
	  {
	    RETCODE rc;
	    NDEFINE_OUTPUT_NONCHAR_NARROW (rgbInfoValue, cbInfoValueMax, pcbInfoValue, con, SWORD);
	...
	HSTMT hstmt)
{
  ...
  while (!stmt->stmt_at_end)
    {
      RETCODE ret = virtodbc__SQLFetch (hstmt, 0);
      if (ret == SQL_ERROR)
	{
	...
	SWORD FAR * pcpar)
{
  ...
  if (BOX_ELEMENTS (sc) > 3 && sc->sc_params)
    {
      if (pcpar)
	*pcpar = (SWORD) BOX_ELEMENTS (sc->sc_params);
      return SQL_SUCCESS;
    }
  NOT_IMPL_FUN (hstmt,
...
{
  SWORD t = SQL_C_CHAR;
  virtodbc__SQLDescribeCol ((HSTMT) stmt, (UWORD) nth, NULL, 0, NULL, &t,
		    NULL, NULL, NULL);
  return t;
}
	...
	PTR FAR * prgbValue)
{
  ...
  if (STS_LOCAL_DAE == stmt->stmt_status)
      ...
      if (!stmt->stmt_current_dae)
	{
	  if (SQL_API_SQLEXECDIRECT == stmt->stmt_pending.p_api)
	    {
	      rc = virtodbc__SQLExecDirect ((HSTMT) stmt, NULL, SQL_NTS);
	      if (SQL_NEED_DATA == rc)
		{
	  ...
	  if (SQL_API_SQLSETPOS == stmt->stmt_pending.p_api)
	    ...
	    return (virtodbc__SQLSetPos ((HSTMT) stmt,
					 (UWORD) stmt->stmt_pending.psp_irow,
					 (UWORD) stmt->stmt_pending.psp_op,
					 SQL_LOCK_NO_CHANGE));
