...
#ifndef O12
void
sqlc_text_node (sql_comp_t * sc, comp_table_t * ct)
{
  int ctype;
  ST **args = sqlc_contains_args (ct->ct_text_pred->pred_text, &ctype);
  state_slot_t *text_id = NULL;
  if (ct->ct_is_text_order)
    {
      /* make a col predicate to drive the ts, then generate a text node that will instantiate the variable  */
      t_NEW_VARZ (col_pred_t, colp);
      text_id = sqlc_new_temp (sc, "text_id", DV_LONG_INT);
      colp->colp_op = BOP_EQ;
      ct->ct_index = tb_text_key (ct->ct_table);
      colp->colp_col = (dbe_column_t *) ct->ct_index->key_parts->data;
      colp->colp_ssl = text_id;
      t_set_push (&ct->ct_col_preds, (void *) colp);
    }
  else
    {
      dbe_column_t *col = (dbe_column_t *) tb_text_key (ct->ct_table)->key_parts->data;
      ST *ref = sqlc_ct_col_ref (ct, col->col_name);
      col_ref_rec_t *cr = sqlc_col_ref_rec (sc, ref, 1);
      text_id = sqlc_col_ref_rec_ssl (sc, cr);
    }
  {
    dk_set_t code = ct->ct_text_pre_code;
    SQL_NODE_INIT (text_node_t, txs, txs_input, txs_free);
    txs->txs_cached_string = sqlc_new_temp (sc, "xpath_cached_exp_string", DV_SHORT_STRING);
    txs->txs_cached_compiled_tree = sqlc_new_temp (sc, "xpath_cached_tree", DV_ARRAY_OF_POINTER);
    txs->txs_cached_runtime_options = sqlc_new_temp (sc, "xpath_cached_runtime_options", DV_ARRAY_OF_POINTER);
    txs->txs_table = tb_text_key (ct->ct_table)->key_text_table;
    txs->txs_d_id = text_id;
    txs->txs_is_driving = ct->ct_is_text_order;
    if (ctype == 'x')
      {
	txs->txs_xpath_text_exp = sqlc_new_temp (sc, "xpath_text_exp", DV_SHORT_STRING);
	ct->ct_is_xcontains = 1;
      }
    txs->txs_text_exp = scalar_exp_generate (sc, args[1], &code);
    txs->txs_main_range_out = ct->ct_main_range_out_ssl;
    txs->txs_attr_range_out = ct->ct_attr_range_out_ssl;
    txs->txs_offband = ct->ct_text_offband_ssls;
    txs->txs_desc= ct->ct_text_desc_ssl;
    txs->txs_score_limit = ct->ct_text_score_limit_ssl;
    txs->txs_init_id = ct->ct_text_start_ssl;
    txs->txs_end_id = ct->ct_text_end_ssl;
    /* IvAn/SmartXContains/001025 Added text_node_t::txs_why_ranges member */
    switch(ctype)
      {
      case 'c':
	if(ct->ct_main_range_out_ssl)
	...
	break;
      default:
	GPF_T1("internal error during compilation of text node");
      }
    txs->txs_score = ct->ct_text_score_ssl;
    txs->txs_sst = sqlc_new_temp (sc, "text search", DV_TEXT_SEARCH);
    txs->src_gen.src_pre_code = code_to_cv (sc, code);
    ct->ct_text_node = (data_source_t *) txs;
  }
}


void
sqlc_is_text_only (sql_comp_t * sc, comp_table_t * ct)
{
  text_node_t * txs = (text_node_t *) ct->ct_text_node;
  table_source_t * ts = ct->ct_ts;
  key_source_t * order_ks = ct->ct_ts->ts_order_ks;
  if (!ts->ts_main_ks
      && !ts->ts_order_ks->ks_row_spec && !ct->ct_xpath_node)
    {
      dbe_column_t * col;
	...
	return;
      ...
      if (col == (dbe_column_t *) tb_text_key (ct->ct_table)->key_parts->data)
	{
	  ct->ct_text_only = 1;
	  if (ct->ct_is_text_order)
...
void
sqlc_xpath_node (sql_comp_t * sc, comp_table_t * ct)
{
  int ctype = 0;
  ST **args = ct->ct_text_pred
  ? sqlc_contains_args (ct->ct_text_pred->pred_text, &ctype) : NULL;
  dk_set_t code = NULL;
  SQL_NODE_INIT (xpath_node_t, xn, xn_input, xn_free);
  if (!args || ctype == 'c')
    args = sqlc_contains_args (ct->ct_xpath_pred->pred_text, &ctype);
  if (!args)
    sqlc_error (sc->sc_cc, "37000", "misplaced %s", sqlo_spec_predicate_name (ctype));
  xn->xn_output_val = ct->ct_xpath_value_ssl;
  xn->xn_text_col = ct->ct_text_ssl;
  xn->xn_base_uri = ct->ct_base_uri_ssl;
  xn->xn_predicate_type = ctype;
  if ((ctype == 'p') || (ctype == 'q'))
    xn->xn_exp_for_xqr_text = scalar_exp_generate (sc, args[1], &code);
  xn->xn_xqi = sqlc_new_temp (sc, "text search", DV_XQI);
  xn->xn_compiled_xqr_text = sqlc_new_temp (sc, "xp_text", DV_SHORT_STRING);
  xn->xn_compiled_xqr = sqlc_new_temp (sc, "xp_xqr", DV_XPATH_QUERY);
  xn->src_gen.src_pre_code = code_to_cv (sc, code);
  ct->ct_xpath_node = (data_source_t *) xn;
  if (ct->ct_text_node && ct->ct_is_xcontains)
    {
      ((text_node_t *) ct->ct_text_node)->txs_xpath_node = xn;
      xn->xn_text_node = (text_node_t *) ct->ct_text_node;
    }
}


void
sqlc_xpath_temp (sql_comp_t * sc)
{
  int inx;
  query_t *qr = sc->sc_cc->cc_query;
  if (qr->qr_xp_temp)
    return;
  qr->qr_xp_temp = (state_slot_t **) dk_alloc_box (8 * sizeof (caddr_t), DV_ARRAY_OF_LONG);
  for (inx = 0; inx < 8; inx++)
    qr->qr_xp_temp[inx] = sqlc_new_temp (sc, "xp_temp", DV_UNKNOWN);
}


void
sqlc_xpath_col (sql_comp_t * sc, comp_table_t * ct, ST ** args, int nth, char ctype)
{
  dbe_column_t *text_col;
  dk_set_t code = NULL;
  col_ref_rec_t *crr;
  ct->ct_text_ssl = scalar_exp_generate (sc, args[0], &code);
  text_col = ct->ct_text_ssl->ssl_column;
  if (!text_col)
    sqlc_error (sc->sc_cc, "37000", "The first argument of %s must be a column", sqlo_spec_predicate_name (ctype));
  if (text_col->col_xml_base_uri)
    {
      ST *ucol = (ST *) t_list (3, COL_DOTTED, t_box_copy (ct->ct_prefix), t_box_copy (text_col->col_xml_base_uri));
      ct->ct_base_uri_ssl = scalar_exp_generate (sc, ucol, &code);
      sqlc_temp_tree (sc, (caddr_t) ucol);
    }
  sqlc_xpath_temp (sc);
  ...
  if (-1 == nth)
	...
	nth = 2;
      else if (BOX_ELEMENTS (args) >= 4 && ST_P (args[3], COL_DOTTED))
	nth = 3;
      else
	return;
    }
  if (!ST_P (args[nth], COL_DOTTED))
    sqlc_error (sc->sc_cc, "37000", "XPATH output must be a column ref in %s", sqlo_spec_predicate_name (ctype));
  crr = sqlc_virtual_col_crr (sc, ct, args[nth]->_.col_ref.name, DV_SHORT_STRING);
  ct->ct_xpath_value_ssl = crr->crr_ssl;
}


void
sqlc_check_ft_offband (sql_comp_t * sc, comp_table_t * ct, ST ** args, char ctype)
{
  int inx;
  dk_set_t off = NULL;
  DO_BOX (ST*, arg, inx, args)
	...
	{
	  int oinx;
	  col_ref_rec_t * sc_crr;
	  dbe_column_t ** ocols = tb_text_key (ct->ct_table)->key_text_col->col_offband_cols;
	  if (BOX_ELEMENTS_INT (args) <= inx + 1 || !ST_P (args[inx + 1], COL_DOTTED))
	    sqlc_error (sc->sc_cc, "37000", "offband in contains must be a column name");
	  if (!ocols)
	    sqlc_error (sc->sc_cc, "37000", "The table does not have offband text columns");
	  DO_BOX (dbe_column_t *, ocol, oinx,ocols)
	    {
	      if (0 == stricmp (ocol->col_name, args[inx + 1]->_.col_ref.name))
		{
		  sc_crr = sqlc_virtual_col_crr (sc, ct, ocol->col_name,
		      ocol->col_sqt.sqt_dtp);
		  dk_set_push (&off, (void*) sc_crr->crr_ssl);
		  dk_set_push (&off, (void*) (ptrlong) oinx);
		  goto offb_done;
		}
	    }
	  END_DO_BOX;
	  sqlc_error (sc->sc_cc, "37000", "No offband column %s", args[inx + 1]->_.col_ref.name);
	offb_done:
	  continue;
	}
      if ((0 == stricmp ((char *)arg, "ranges")) || (0 == stricmp ((char *)arg, "main_ranges")))
	{
	  col_ref_rec_t * sc_crr;
	  if (BOX_ELEMENTS_INT (args) <= inx + 1 || !ST_P (args[inx + 1], COL_DOTTED))
	    sqlc_error (sc->sc_cc, "37000", "The %s argument of %s must reference a column", (char *)arg, sqlo_spec_predicate_name (ctype));
	  sc_crr = sqlc_virtual_col_crr (sc, ct, args[inx + 1]->_.col_ref.name, DV_ARRAY_OF_POINTER);
	  ct->ct_main_range_out_ssl = sc_crr->crr_ssl;
	  continue;
	}
      if (0 == stricmp ((char *)arg, "attr_ranges"))
	{
	  col_ref_rec_t * sc_crr;
	  if (BOX_ELEMENTS_INT (args) <= inx + 1 || !ST_P (args[inx + 1], COL_DOTTED))
	    sqlc_error (sc->sc_cc, "37000", "The attr_ranges argument of %s must reference a column", sqlo_spec_predicate_name (ctype));
	  sc_crr = sqlc_virtual_col_crr (sc, ct, args[inx + 1]->_.col_ref.name, DV_ARRAY_OF_POINTER);
	  ct->ct_attr_range_out_ssl = sc_crr->crr_ssl;
	  continue;
	}
      if ((0 == stricmp ((char *)arg, "desc")) || (0 == stricmp ((char *)arg, "descending")))
	{
	  ct->ct_text_desc_ssl = ssl_new_constant (sc->sc_cc, box_num (1));
	  continue;
	}
      if (0 == stricmp ((char *) arg, "start_id"))
	{
	  if (BOX_ELEMENTS_INT (args) <= inx + 1)
	    sqlc_error (sc->sc_cc, "37000", "contains start_id option must have an argument");
	  sqlc_mark_pred_deps (sc, NULL, args[inx + 1]);
	  ct->ct_text_start_ssl = scalar_exp_generate (sc, args[inx + 1], &ct->ct_text_pre_code);
	  continue;
	}
      if (0 == stricmp ((char *) arg, "end_id"))
	{
	  ...
	  sqlc_mark_pred_deps (sc, NULL, args[inx + 1]);
	  ct->ct_text_end_ssl = scalar_exp_generate (sc, args[inx + 1], &ct->ct_text_pre_code);
	  continue;
	}
      if (0 == stricmp ((char *) arg, "score_limit"))
	{
	  ...
	  sqlc_mark_pred_deps (sc, NULL, args[inx + 1]);
	  ct->ct_text_score_limit_ssl = scalar_exp_generate (sc, args[inx + 1], &ct->ct_text_pre_code);
	  continue;
	}
    }
  END_DO_BOX;
  if (off)
    ct->ct_text_offband_ssls =(state_slot_t **)  list_to_array (off);
}


void
sqlc_implied_columns (sql_comp_t * sc)
{
  DO_SET (predicate_t *, pred, &sc->sc_preds)
  {
    int ctype;
    ST **args = sqlc_contains_args (pred->pred_text, &ctype);
    if (args)
      {
	col_ref_rec_t *cr, *sc_crr;
	comp_table_t *ct;
	if (!ST_P (args[0], COL_DOTTED))
	  sqlc_error (sc->sc_cc, "37000", "The first argument of %s must be a column", sqlo_spec_predicate_name (ctype));
	cr = sqlc_col_or_param (sc, args[0], 0);
	if (!cr->crr_ct)
	  sqlc_error (sc->sc_cc, "37000", "The first argument of %s must reference a column", sqlo_spec_predicate_name (ctype));
	ct = cr->crr_ct;
	if (ctype == 'c' || ctype == 'x')
	  {
	    if (!tb_text_key (ct->ct_table))
	      sqlc_error (sc->sc_cc, "37000", "Table referenced in %s does not have a text index", sqlo_spec_predicate_name (ctype));
	    sc_crr = sqlc_virtual_col_crr (sc, ct, "SCORE", DV_LONG_INT);
	    ct->ct_text_score_ssl = sc_crr->crr_ssl;
	    if (ctype == 'x' || ctype == 'c')
	      sqlc_check_ft_offband (sc, ct, args, (char) ctype);
	    if ((ctype == 'x') || (NULL != ct->ct_main_range_out_ssl))
	      {
		sqlc_xpath_col (sc, ct, args, -1, ctype);
		if (NULL == ct->ct_main_range_out_ssl)
		  ct->ct_main_range_out_ssl = sqlc_new_temp (sc, "xcontains_main_ranges", DV_ARRAY_OF_POINTER);
		if (NULL == ct->ct_attr_range_out_ssl)
		  ct->ct_attr_range_out_ssl = sqlc_new_temp (sc, "xcontains_attr_ranges", DV_ARRAY_OF_POINTER);
	      }
	  }
	else
	  {
	    sqlc_xpath_col (sc, ct, args, 2, ctype);
	  }
      }
  }
  END_DO_SET ();
...
void
ks_make_misc_accelerator (sql_comp_t * sc, comp_table_t * ct, key_source_t * ks)
{
#ifndef O12
  /* sort the referenced misc's */
  misc_accelerator_t *ma;
  int inx;
  state_slot_t **out_slots;
  oid_t *out_ids;
  col_ref_rec_t *misc_crrs[MAX_MISC_REFS];
  int misc_fill = 0;
  DO_SET (col_ref_rec_t *, crr, &sc->sc_col_ref_recs)
  {
    if (crr->crr_ct == ct
	&& IS_BOX_POINTER (crr->crr_dbe_col)
	&& crr->crr_dbe_col->col_is_misc)
      misc_crrs[misc_fill++] = crr;
  }
  END_DO_SET ();
  if (0 == misc_fill)
    return;
  ma = (misc_accelerator_t *) dk_alloc (sizeof (misc_accelerator_t));
  memset (ma, 0, sizeof (misc_accelerator_t));
  buf_bsort ((buffer_desc_t **) & misc_crrs, misc_fill, (sort_key_func_t) misc_crr_key);

  out_slots = (state_slot_t **) dk_alloc_box (sizeof (caddr_t) * misc_fill, DV_ARRAY_OF_LONG);
  memset (out_slots, 0, box_length ((caddr_t) out_slots));
  out_ids = (oid_t *) dk_alloc_box (sizeof (oid_t) * misc_fill, DV_ARRAY_OF_LONG);
  memset (out_ids, 0, box_length ((caddr_t) out_slots));
  for (inx = 0; inx < misc_fill; inx++)
    {
      if (misc_crrs[inx]->crr_ssl)
	{
	  out_slots[inx] = misc_crrs[inx]->crr_ssl;
	  out_ids[inx] = misc_crrs[inx]->crr_dbe_col->col_id;
	}
    }
  ma->ma_ids = out_ids;
  ma->ma_out_slots = out_slots;
  ma->ma_specs = (search_spec_t ***) dk_alloc_box (misc_fill * sizeof (caddr_t),
      DV_ARRAY_OF_POINTER);
  for (inx = 0; inx < misc_fill; inx++)
    {
      ma->ma_specs[inx] = ks_col_specs (ks, misc_crrs[inx]->crr_dbe_col, 0);
    }
...
#ifndef O12
void
ks_ancestor_scan (sql_comp_t * sc, comp_table_t * ct, key_source_t * ks)
{
  char ct_id[MAX_NAME_LEN * 2 + 10];
  ST **args = sqlc_ancestor_args (ct->ct_ancestor_pred->pred_text);
  col_ref_rec_t *l_crr, *r_crr;
  int axis;
  NEW_VARZ (ancestor_scan_t, as);

  ks->ks_key = ct->ct_table->tb_primary_key;
  ks->ks_ancestor_scan = as;
  l_crr = sqlc_col_ref_rec (sc, args[0], 1);
  r_crr = sqlc_col_ref_rec (sc, args[1], 1);
  axis = unbox (args[2]);
  if (ct != r_crr->crr_ct)
    {
      /* the joined table is the ancestor, not the child */
      col_ref_rec_t *t = l_crr;
      axis = -axis;
      l_crr = r_crr;
      r_crr = t;
    }
  CT_ID (l_crr->crr_ct, ct_id);
  as->as_init_pl_name = box_string (ct_id);

  as->as_axis = axis;

  /*  if (n_args > 3)
   *   as->as_subscript = scalar_exp_generate (sc, args[3], &code); */
  as->as_current_level = sqlc_new_temp (sc, "anc_level", DV_LONG_INT);
  as->as_subscript = sqlc_new_temp (sc, "subscript", DV_LONG_INT);
...
#ifndef O12
void
ins_arrange_misc (sql_comp_t * sc, insert_node_t * ins)
{
  int inx;
  int fill;
  int any_misc = 0;
  misc_asg_t asg[UPD_MAX_COLS];
  misc_asg_t *asgp[UPD_MAX_COLS];
  if (BOX_ELEMENTS (ins->ins_col_ids) > UPD_MAX_COLS)
    sqlc_error (sc->sc_cc, "37000", "Too many columns in update");

  DO_BOX (oid_t, id, inx, ins->ins_col_ids)
  {
    asg[inx].asg_col_id = id;
    asg[inx].asg_ssl = (state_slot_t *) dk_set_nth (ins->ins_values, inx);
    asgp[inx] = &asg[inx];
    if (IS_MISC_ID (id))
      any_misc = 1;
  }
  ...
  for (inx = 0; inx < fill; inx++)
    {
      dk_set_set_nth (ins->ins_values, inx, (void *) asgp[inx]->asg_ssl);
      ins->ins_col_ids[inx] = asgp[inx]->asg_col_id;
    }
}
#endif
