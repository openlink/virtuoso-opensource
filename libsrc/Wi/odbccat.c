...
#if !defined (WIN32)
void
get_odbc_ini (char * szPath, size_t max_szPath)
{
  char *value;
  if (!szPath)
    return;
  if ((value = getenv ("ODBCINI")) != NULL)
    snprintf (szPath, max_szPath, "%s", value);
  else
    {
      if ((value = getenv ("HOME")) != NULL)
	snprintf (szPath, max_szPath, "%s/.odbc.ini", value);
      else
	strcpy_size_ck (szPath, "odbc.ini", max_szPath);
    }
}
#endif
...
/* sql_config_data_sources (Driver name char, type user/system char, DSN name char,
			    Description char, Server char, Database Name char, User char,
			    Password char, Other Parameters char)			*/
...
odbc_cat_init (void)
{
  ...
  bif_define ("sql_get_type_info", bif_sql_gettypeinfo); /*the convention mismash fixed, so we'll leave the old one for compatibility */
