...
int
itc_cl_local (it_cursor_t * itc, buffer_desc_t * buf)
{
  /* if ordered, put the copy into the parsed in basketr of the req clo
   * if not ordered and this is ks last, set the env and run local code and setp
   * if not ordered  and next clb with space, put the stuff there.  If full, return to start the next clb
   * if not ordered otherwise, put the stuff in the parsed in basket */
  dbe_key_t *key;
  int bytes = 11;
  cll_in_box_t *clib = (cll_in_box_t *) itc->itc_search_params[itc->itc_search_par_fill];
  cl_op_t *set_clo = (cl_op_t *) itc->itc_search_params[itc->itc_search_par_fill + 1];
  key_source_t *ks = itc->itc_ks;
  itc->itc_is_on_row = 1;
  if (!ks || (!ks->ks_is_last && !ks->ks_next_clb))
    {
      /* the general case.  Either ordered or non-last and followed by non-buffered in unordered.  Put the row in results */
      int row_bytes;
      cl_op_t *row = clo_make_row (clib, itc, buf, set_clo, &row_bytes);
      clib->clib_local_bytes += row_bytes;
      basket_add (&clib->clib_in_parsed, (void *) row);
      if (IS_MAX_ROWS (set_clo->_.select.max_rows))
	itc->itc_cl_set_done = 1;
      if (ks->ks_ts->ts_is_unique || clib->clib_local_bytes > cl_res_buffer_bytes || itc->itc_cl_set_done)
	{
	  /*in_printf (("local batch of ts %d full\n", itc->itc_ks->ks_ts->clb.clb_fill)); */
	  itc->itc_cl_batch_done = 1;
	  return DVC_MATCH;
	}
      if (itc->itc_owns_page == buf->bd_page || itc->itc_isolation != ISO_REPEATABLE)
	return DVC_LESS;	/* keep looping, no return needed */
      return DVC_MATCH;
    }
  /* now we have unordered in ks_last place or as non-last with immediately following feedable clb */
  key = itc->itc_row_key;
  if (ks->ks_out_cols)
    {
      int inx = 0;
	...
	if (om[inx].om_is_null)
	  ...
	  {
	    /* for leading key part of bitmap inx, set them only when first time on row */
	    if (!itc->itc_insert_key->key_is_bitmap || itc->itc_bp.bp_new_on_row)
	      itc_qst_set_column (itc, buf, &om[inx].om_cl, itc->itc_out_state, ssl);
	    bytes += box_serial_length (QST_GET_V (itc->itc_out_state, ssl), 0);
	  }
	inx++;
      }
      END_DO_SET ();
  ...
  if (clib->clib_local_bytes > 5000)
    {
      /* if unordered local op, check every now and then, then reset */
      itc_cl_others_ready (itc);
      clib->clib_local_bytes_cum += clib->clib_local_bytes;
      clib->clib_local_bytes = 0;
    }
  if (itc->itc_cl_batch_done)
    {
      /*cl_printf ((" Local batch of %d bytes ends when remote data available\n", clib->clib_local_bytes_cum + clib->clib_local_bytes)); */
      clib->clib_local_bytes_cum = 0;
    }

  if (!ks->ks_ts->clb.clb_nth_context || QST_INT (itc->itc_out_state, ks->ks_ts->clb.clb_nth_context) != set_clo->clo_nth_param_row)
    {
      itc_cluster_t *itcl = ((cl_op_t *) QST_GET_V (itc->itc_out_state, ks->ks_itcl))->_.itcl.itcl;
      cl_ts_set_context (ks->ks_ts, itcl, itc->itc_out_state, set_clo->clo_nth_param_row);
    }
