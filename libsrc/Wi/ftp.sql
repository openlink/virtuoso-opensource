...
{
  -- dbg_obj_princ ('FTP_NOR_DIR (', dir1, dir2, ')');
   -- TODO check for ..
   if ("LEFT" (dir2, 1) <> '/')
     dir2 := dir1 || dir2 || '/';
   else
   ...
   dir2 := replace (dir2, '//', '/');
  -- dbg_obj_princ ('FTP_NOR_DIR has made ', dir2);
}
;
...
-- Parameter check
  ...
  FTP_COMMAND (ses, concat ('stor ', _remote), vector (150));
...
--	temp := temp + length (_read);
--  	dbg_obj_print (' _read ', length (_read), ' - ', temp);
        if (length (_read) = 0)
          {
             ses_disconnect (data_ses);
             return;
					       ...
					       inout listen any, inout rest_pos integer)
{
  ...
  -- _upd_res := DAV_SEARCH_ID (f_name, 'R', _user, _pass);
