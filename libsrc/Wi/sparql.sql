...
create procedure DB.DBA.RDF_TRIPLES_TO_HTML_NICE_MICRODATA (inout triples any, inout ses any)
{
  ...
  if (0 = tcount)
    {
      http ('<!DOCTYPE html>\n');
      http ('<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
<head><title>Empty HTML Microdata document</title></head><body>
<p>This document is empty and basically useless. It is generated by a web service that can make some statements in HTML Microdata format.
This time the service made zero such statements, sorry.</p></body></html>', ses);
      return;
  ...
  http ('<!DOCTYPE html>\n');
  http ('<html xmlns="http://www.w3.org/1999/xhtml" lang="en"', ses);
  http ('>\n<head><title>HTML Based Entity Description (with embedded Microdata)</title>
<style>
table { border: 1px solid black; empty-cells: hide; text-align: left; vertical-align: top; padding-top: 0.5em; }
tbody td, th { padding-top: 0.5em; }
tbody tr:nth-child(odd) { background-color: #eee; }
  ...
  http ('\n<table><thead><tr><th>Subject</th><th>Predicate</th><th>Object</th></tr></thead><tbody>\n', ses);
  for (tctr := 0; tctr < tcount; tctr := tctr + 1)
    {
      ...
      if (prev_subj is null or (subj <> prev_subj))
        {
          ...
          if (about_path is not null)
            {
              if ('' = split[1])
		http (sprintf ('<a href="%H">%V</a>&nbsp;(<a href="%s%H">/about</a>)', val, split[2], about_path, val), ses);
              else if (isstring (split[0]))
		http (sprintf ('<a href="%H">%V:%V</a>&nbsp;(<a href="%s%H">/about</a>)', val, split[0], split[2], about_path, val), ses);
              else
		http (sprintf ('<a href="%H">%V%V</a>&nbsp;(<a href="%s%H">/about</a>)', val, split[1], split[2], about_path, val), ses);
            }
          else if (describe_path is not null)
            {
              if ('' = split[1])
		http (sprintf ('<a href="%H">%V</a>&nbsp;(<a href="%s%U">/describe</a>)', val, split[2], describe_path, val), ses);
              else if (isstring (split[0]))
		http (sprintf ('<a href="%H">%V:%V</a>&nbsp;(<a href="%s%U">/describe</a>)', val, split[0], split[2], describe_path, val), ses);
              else
		http (sprintf ('<a href="%H">%V%V</a>&nbsp;(<a href="%s%U">/describe</a>)', val, split[1], split[2], describe_path, val), ses);
            }
          else
            {
              if ('' = split[1])
      ...
      if (isvector (o_split))
	    ...
	    http (sprintf ('<a itemprop="%V" href="%H">%V%V</a>', p_itemprop, o_href, o_split[1], o_split[2]), ses);
          if (about_path is not null)
            http (sprintf ('(<a href="%s%H">/about</a>)', about_path, o_href), ses);
        }
      else
        {
          declare sqlval any;
