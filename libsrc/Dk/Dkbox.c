...
uint32 big_endian_box_length (const void *box)
{
  ...
  switch (tag)
	...
	cpair->unc_refcounted = blk->unb_next;
      else
        {
          ...
          while (prev_in_chain->unb_next != blk) prev_in_chain = prev_in_chain->unb_next;
...
box_t
DBG_NAME(box_copy_tree) (DBG_PARAMS cbox_t box)
{
  uint32 inx, len;
  box_t *copy;
  dtp_t tag;
...
box_t DBG_NAME (box_dv_short_concat) (DBG_PARAMS ccaddr_t box1, ccaddr_t box2)
{
...
caddr_t DBG_NAME(box_vsprintf) (DBG_PARAMS size_t buflen_eval, const char *format, va_list tail)
{
  char *tmpbuf;
  int res_len;
  caddr_t res;
...
caddr_t box_sprintf_impl (size_t buflen_eval, const char *format, ...)
{
  va_list tail;
  caddr_t res;
...
char *DBG_NAME(box_dv_ubuf) (DBG_PARAMS size_t buf_strlen)
{
#ifdef MALLOC_DEBUG
  caddr_t uname;
  buf_strlen++;
...
box_t DBG_NAME(box_dv_uname_from_ubuf) (DBG_PARAMS char *text)
{
  uint32 hash;
  char *uname;
...
box_t DBG_NAME(box_dv_uname_string) (DBG_PARAMS const char *string)
{
  return DBG_NAME(box_dv_uname_nchars) (DBG_ARGS string, strlen(string));
}

box_t DBG_NAME(box_dv_uname_substr) (DBG_PARAMS ccaddr_t str, int n1, int n2)
{
  int lstr = (int)(box_length (str)) - 1;
  int lres;
  if (n2 > lstr)
...
caddr_t box_mem_wrapper_copy_hook (caddr_t mw_arg)
{
...
void dk_box_initialize(void)
{
  ...
  /* Uname trick is... well, a trick. If broken, it will cause never-catched errors. Thus check it always. */
  {
    ...
    for (ctr = UNAME_TABLE_SIZE * 2; ctr--; /* no step*/)
      { /* alloc 1 */
        char buf[20]; sprintf (buf, "uname%d", ctr);
...
*/
#undef box_string
box_t box_string (const char *string) { return dbg_box_string (__FILE__, __LINE__, string); }
#undef box_dv_short_string
box_t box_dv_short_string (const char *string) { return dbg_box_dv_short_string (__FILE__, __LINE__, string); }
#undef box_dv_short_nchars
box_t box_dv_short_nchars (const char *buf, size_t buf_len) { return dbg_box_dv_short_nchars (__FILE__, __LINE__, buf, buf_len); }
#undef box_dv_short_nchars_reuse
box_t box_dv_short_nchars_reuse (const char *buf, size_t buf_len, box_t replace) { return dbg_box_dv_short_nchars_reuse (__FILE__, __LINE__, buf, buf_len, replace); }
#undef box_dv_short_substr
box_t box_dv_short_substr (ccaddr_t box, int n1, int n2) { return dbg_box_dv_short_substr (__FILE__, __LINE__, box, n1, n2); }
#undef box_dv_short_concat
box_t box_dv_short_concat (ccaddr_t box1, ccaddr_t box2) { return dbg_box_dv_short_concat (__FILE__, __LINE__, box1, box2); }
#undef box_copy
box_t box_copy (cbox_t box) { return dbg_box_copy (__FILE__, __LINE__, box); }
#undef box_copy_tree
...
box_t box_dv_uname_from_ubuf (char *ubuf) { return dbg_box_dv_uname_from_ubuf (__FILE__, __LINE__, ubuf); }
#undef box_dv_uname_string
box_t box_dv_uname_string (const char *string) { return dbg_box_dv_uname_string (__FILE__, __LINE__, string); }
#undef box_dv_uname_nchars
box_t box_dv_uname_nchars (const char *buf, size_t buf_len) { return dbg_box_dv_uname_nchars (__FILE__, __LINE__, buf, buf_len); }
#undef box_dv_uname_substr
box_t box_dv_uname_substr (ccaddr_t box, int n1, int n2) { return dbg_box_dv_uname_substr (__FILE__, __LINE__, box, n1, n2); }
#undef box_double
box_t box_double (double d) { return dbg_box_double (__FILE__, __LINE__, d); }
#undef box_float
box_t box_float (float d) { return dbg_box_float (__FILE__, __LINE__, d); }
#undef box_vsprintf
caddr_t box_vsprintf (size_t buflen_eval, const char *format, va_list tail) { return dbg_box_vsprintf (__FILE__, __LINE__, buflen_eval, format, tail); }
#undef box_sprintf
caddr_t box_sprintf (size_t buflen_eval, const char *format, ...)
{
  va_list tail;
  caddr_t res;
  va_start (tail, format);
